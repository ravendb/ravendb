	...
	public class StartupTaskTests : RavenTest
	{
		...
		public void CleanerStartupTaskShouldRemoveDatabasesAfterIdleTimeout()
		{
			using (var store = NewDocumentStore())
			{
				...
				Assert.NotNull(store.DatabaseCommands.Get(Constants.RavenDatabasePathPrefix + "Northwind"));
				Assert.NotNull(store.DatabaseCommands.Get(Constants.RavenDatabasePathPrefix + "Northwind2"));

				store.ServerIfEmbedded
					.ServerStartupTasks.OfType<LiveTestResourceCleanerStartupTask>().First()
					.ExecuteCleanup(null);

				Assert.Null(store.DatabaseCommands.Get(Constants.RavenDatabasePathPrefix + "Northwind"));
				Assert.NotNull(store.DatabaseCommands.Get(Constants.RavenDatabasePathPrefix + "Northwind2"));
				...
				Assert.Null(store.DatabaseCommands.Get(Constants.RavenDatabasePathPrefix + "Northwind2"));
				Assert.NotNull(store.DatabaseCommands.Get(Constants.RavenDatabasePathPrefix + "Northwind3"));
			}
		}

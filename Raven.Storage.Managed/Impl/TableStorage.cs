...
namespace Raven.Storage.Managed.Impl
{
    public class TableStorage : Munin.Database
    {
        ...
        {
            Details = Add(new Table(JTokenComparer.Instance, "Detauls"));

            Identity = Add(new Table(new ModifiedJTokenComparer(x => x.Value<string>("name")), "Identity"));

            Attachments = Add(new Table(new ModifiedJTokenComparer(x => x.Value<string>("key")), "Attachments")
            {
                {"ByEtag", x => new ComparableByteArray(x.Value<byte[]>("etag"))},
            });

            Documents = Add(new Table(new ModifiedJTokenComparer(x => x.Value<string>("key")), "Documents")
            {
            ...
            DocumentsModifiedByTransactions =
                Add(new Table(new ModifiedJTokenComparer(x => new JObject {{"key", x.Value<string>("key")}}),
                              "DocumentsModifiedByTransactions")
                {
            ...
            Transactions =
                Add(new Table(new ModifiedJTokenComparer(x => x.Value<byte[]>("txId")), "Transactions"));

            IndexingStats =
                Add(new Table(new ModifiedJTokenComparer(x => x.Value<string>("index")), "IndexingStats"));


            MappedResults = Add(new Table(JTokenComparer.Instance, "MappedResults")
            {
            ...
            Queues = Add(new Table(new ModifiedJTokenComparer(x => new JObject
            {
            ...
            Tasks = Add(new Table(new ModifiedJTokenComparer(x => new JObject
            {

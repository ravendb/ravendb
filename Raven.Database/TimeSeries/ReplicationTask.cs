...
namespace Raven.Database.TimeSeries
{
	public class ReplicationTask : IDisposable
	{
        ...
        private readonly object waitForTimeSeriesUpdate = new object();
        private int actualWorkTimeSeries;
        private int replicatedWorkTimeSeries; // represents the last actualWorkTimeSeries value that was checked in the last replication iteration

		private readonly ConcurrentDictionary<string, TimeSeriesDestinationStats> destinationsStats =
            new ConcurrentDictionary<string, TimeSeriesDestinationStats>(StringComparer.OrdinalIgnoreCase);
		...
		{
			lock (waitForTimeSeriesUpdate)
			{
				Interlocked.Increment(ref actualWorkTimeSeries);
				Monitor.PulseAll(waitForTimeSeriesUpdate);
			}
		}
		...
		{
			if (Thread.VolatileRead(ref actualWorkTimeSeries) != replicatedWorkTimeSeries)
			{
				replicatedWorkTimeSeries = actualWorkTimeSeries;
				return true;
			}
			lock (waitForTimeSeriesUpdate)
			{
		...
		public void SendReplicationToAllServers(bool runningBecauseOfDataModifications)
		{
			try
			{
				...
				var destinationForReplication = replicationDestinations.Where(destination => 
					(!runningBecauseOfDataModifications || IsNotFailing(destination.TimeSeriesUrl, currentReplicationAttempts)) && !destination.Disabled);
		...
		{
			var dest = destination.TimeSeriesUrl;
			...
			var replicationTask = Task.Factory.StartNew(
				() =>
				{
					...
					try
					{
						if (ReplicateTo(destination)) SignalUpdate();
					}
					catch (Exception e)
					{
		...
		private bool ReplicateTo(TimeSeriesReplicationDestination destination)
		{
			...
			try
			{
				...
				switch (TryReplicate(destination, out lastEtag, out lastError))
				{
					case ReplicationResult.Success:
                        ...
                        RecordSuccess(destination.TimeSeriesUrl, lastReplicatedEtag: lastEtag, lastReplicatedLastModified: replicationTime);
                        storage.MetricsTimeSeries.OutgoingReplications.Mark();
						result = true;
						break;
					case ReplicationResult.NotReplicated:
						//TODO: Record not replicated
                        RecordSuccess(destination.TimeSeriesUrl, SystemTime.UtcNow);
						break;
					default:
						RecordFailure(destination.TimeSeriesUrl, lastError);
                        storage.MetricsTimeSeries.OutgoingReplications.Mark();
						break;
				}
			...
			{
				Log.ErrorException("Error occured replicating to: " + destination.TimeSeriesUrl, ex);
				RecordFailure(destination.TimeSeriesUrl, ex.Message);
				return false;
			}
			finally
			{
                ...
                storage.MetricsTimeSeries.GetReplicationDurationHistogram(destination.TimeSeriesUrl).Update(elapsedMicroseconds);
				var holder = activeReplicationTasks.GetOrAdd(destination.TimeSeriesUrl, s => new SemaphoreSlim(0, 1));
				holder.Release();
			}
		}
		...
		private List<TimeSeriesReplicationDestination> GetReplicationDestinations()
		{
			TimeSeriesReplicationDocument replicationData;
        ...
        {
            TimeSeriesDestinationStats destinationStats;
        ...
        {
            var destStats = destinationsStats.GetOrAdd(destinationUrl, new TimeSeriesDestinationStats { Url = destinationUrl });
		...
		private void NotifySibling(BlockingCollection<RavenConnectionStringOptions> collection)
		{
			...
			while (true)
			{
				...
				try
				{
					var url = connectionStringOptions.Url + "/cs/" + storage.Name + "/replication/heartbeat?from=" + Uri.EscapeDataString(storage.TimeSeriesUrl);
					var request = httpRavenRequestFactory.Create(url, HttpMethods.Post, connectionStringOptions);
        ...
        {
            var stats = destinationsStats.GetOrAdd(url, new TimeSeriesDestinationStats { Url = url });
        ...
        public ConcurrentDictionary<string, TimeSeriesDestinationStats> DestinationStats
        {

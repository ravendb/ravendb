    ...
    public class ClusterManagementHttpClient : IDisposable
    {
        ...
        internal Task<Action<HttpClient>> HandleUnauthorizedResponseAsync(HttpResponseMessage unauthorizedResponse, NodeConnectionInfo nodeConnectionInfo)
        {
            ...
            if (string.IsNullOrEmpty(oauthSource))
                oauthSource = nodeConnectionInfo.AbsoluteUri + "/OAuth/API-Key";

            return GetAuthenticator(nodeConnectionInfo).DoOAuthRequestAsync(nodeConnectionInfo.AbsoluteUri, oauthSource, nodeConnectionInfo.ApiKey);
        }
        ...
        {
            var url = leaderNode.AbsoluteUri + "admin/cluster/join";
        ...
        {
            var url = node.AbsoluteUri + "cluster/replicationState";
        ...
        public async Task<ConnectivityStatus> CheckConnectivity(NodeConnectionInfo node)
        {
            try
            {
                var url = node.AbsoluteUri + "stats";
        ...
        {
            var url = node.AbsoluteUri + "admin/cluster/commands/database/" + Uri.EscapeDataString(databaseName) + "?hardDelete=" + hardDelete;
        ...
        {
            var url = node.AbsoluteUri + action;
        ...
        {
            var url = nodeConnectionInfo.AbsoluteUri + "admin/cluster/canJoin?topologyId=" + raftEngine.CurrentTopology.TopologyId;
        ...
        {
            var url = leaderNode.AbsoluteUri + "admin/cluster/update";
            using (var request = CreateRequest(leaderNode, url, HttpMethods.Post))
            {
        ...
        {
            var url = leaderNode.AbsoluteUri + "admin/cluster/leave?name=" + leavingNode.Name;
        ...
        {
            var url = node.AbsoluteUri + "admin/cluster/initialize-new-cluster" + (clusterId.HasValue ? "?id=" + clusterId.Value : string.Empty);
        ...
        {
            return _securedAuthenticatorCache.GetOrAdd(info.AbsoluteUri, _ => new SecuredAuthenticator(autoRefreshToken: false));
        }

        internal ReturnToQueue GetConnection(NodeConnectionInfo nodeConnection, out HttpClient result)
        {
            var connectionQueue = _httpClientsCache.GetOrAdd(nodeConnection.AbsoluteUri, _ => new ConcurrentQueue<HttpClient>());
        ...
        public async Task<bool> SendVotingModeChangeRequestAsync(NodeConnectionInfo nodeConnectionInfo, bool isVoting)
        {
            ...
            do
            {
                try
                {
                    ...
                    var url = $"{leaderNode.AbsoluteUri}admin/cluster/changeVotingMode?isVoting={isVoting}";
                    using (var request = CreateRequest(leaderNode, url, HttpMethods.Post))
                    {

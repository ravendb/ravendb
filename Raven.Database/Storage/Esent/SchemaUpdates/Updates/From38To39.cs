...
namespace Raven.Storage.Esent.SchemaUpdates.Updates
{
	public class From38To39 : ISchemaUpdate
	{
		...
		{
			using (var table = new Table(session, dbid, "tasks", OpenTableGrbit.None))
			{
				Api.JetDeleteIndex(session, table, "mergables_by_task_type");
				Api.JetDeleteColumn(session, table, "supports_merging");

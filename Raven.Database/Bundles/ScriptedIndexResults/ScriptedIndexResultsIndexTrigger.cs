...
namespace Raven.Database.Bundles.ScriptedIndexResults
{
	...
	public class ScriptedIndexResultsIndexTrigger : AbstractIndexUpdateTrigger
	{
		...
		public class Batcher : AbstractIndexUpdateTriggerBatcher
		{
			...
			private readonly Dictionary<string, Document> created = new Dictionary<string, Document>(StringComparer.InvariantCultureIgnoreCase);
			...
			public override void Dispose()
			{
				...
				if (string.IsNullOrEmpty(scriptedIndexResults.IndexScript) == false)
				{
					...
					{
						var ravenJObject = CreateJsonDocumentFromLuceneDocument(kvp.Value);
						patcher.Apply(ravenJObject, new ScriptedPatchRequest
						{
			...
			{
				public ScriptedIndexResultsJsonPatcher(DocumentDatabase database) : base(database)
				{
				}

...
namespace Raven.Database.FileSystem.Synchronization
{
    public class SynchronizationQueue
    {
            ...
            try
            {
                ...
                foreach (var pendingWork in pendingForDestination)
                {
                        ...
                        Buffers.Compare(workItem.FileETag.ToByteArray(), pendingWork.FileETag.ToByteArray()) > 0)
                    {
                        ...
                        if (Log.IsDebugEnabled)
                            Log.Debug("{0} for a file {1} and a destination {2} was already existed in a pending queue but with older ETag, it's metadata has been refreshed",
                                workItem.GetType().Name, workItem.FileName, destinationFileSystemUrl);
                ...
                if (Log.IsDebugEnabled)
                    Log.Debug("{0} for a file {1} and a destination {2} was enqueued", workItem.GetType().Name, workItem.FileName, destinationFileSystemUrl);
            }
            finally
            {
        ...
        public void SynchronizationStarted(SynchronizationWorkItem work, string destinationFileSystemUrl)
        {
            ...
            {
                if (Log.IsDebugEnabled)
                    Log.Debug("File '{0}' with ETag {1} was added to an active synchronization queue for a destination {2}", work.FileName, work.FileETag, destinationFileSystemUrl);
            }
        }
        ...
        public void SynchronizationFinished(SynchronizationWorkItem work, string destinationFileSystemUrl)
        {
            ...
            {
                if (Log.IsDebugEnabled)
                    Log.Debug("File '{0}' with ETag {1} was removed from an active synchronization queue for a destination {2}", work.FileName, work.FileETag, destinationFileSystemUrl);
            }
        }

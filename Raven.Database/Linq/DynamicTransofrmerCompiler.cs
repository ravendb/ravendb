	...
	public class DynamicTransformerCompiler : DynamicCompilerBase
	{
		...
		private void TransformToClass()
		{
			if (transformerDefinition.TransformResults == null)
				throw new InvalidOperationException("Cannot compile a transformer without a transformer function");
			...
			{
				translatorDeclaration = QueryParsingUtils.GetVariableDeclarationForLinqQuery(transformerDefinition.TransformResults, requiresSelectNewAnonymousType: false);
			}
			else
			{
				translatorDeclaration = QueryParsingUtils.GetVariableDeclarationForLinqMethods(transformerDefinition.TransformResults, requiresSelectNewAnonymousType: false);
			}

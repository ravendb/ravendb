...
namespace Raven.Database.Server.Security.OAuth
{
	public class OAuthApiKeyResponder : AbstractRequestResponder
	{
		...
		private AccessToken GetAccessTokenFromApiKey(string apiKeyName, string apiSecret)
		{
			...
			if (!AuthenticateClient.Authenticate(Database, apiKeyName, apiSecret, out authorizedDatabases))
			{
				if ((Database == SystemDatabase ||
					 !AuthenticateClient.Authenticate(SystemDatabase, apiKeyName, apiSecret, out authorizedDatabases)))
					return null;
			}

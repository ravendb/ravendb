...
namespace Raven.Powershell
{
    ...
    public class BinaryRestoreDatabaseInClusterCmdlet : Cmdlet
    {
        ...
        private bool shouldGenerateNewId = true;
        ...
        [Parameter(ValueFromPipelineByPropertyName = true, HelpMessage = "If false, generate new low-level storage Id on restore. Otherwise, leave the old low-level storage Id in place.")]
        public SwitchParameter DontGenerateNewId
        {
            get { return shouldGenerateNewId; }
            set { shouldGenerateNewId = !value; }
        }
        ...
        protected override void ProcessRecord()
        {
            try
            {
                ...
                })
                {
                    ...
                    var globalReplicationDestinationsJson =
                        store.DatabaseCommands.Get(Constants.Global.ReplicationDestinationsDocumentName);

                    if (globalReplicationDestinationsJson == null)
                    {
                        throw new NotFoundException("Failed to fetch document id = " + Constants.Global.ReplicationDestinationsDocumentName);
                    }

                    var globalReplicationDestinations = globalReplicationDestinationsJson.DataAsJson
                        .JsonDeserialization<ReplicationDocument>();
                    ...
                    foreach (var node in replicationDestinations)
                    {
                        ...
                        var restoreOperation = curNodeStore.DatabaseCommands.GlobalAdmin.StartRestore(
                            new DatabaseRestoreRequest
                            {
                                BackupLocation = BackupLocation,
                                DatabaseName = DatabaseName,
                                DisableReplicationDestinations = true,
                                GenerateNewDatabaseId = shouldGenerateNewId
                            });
                    ...
                    foreach (var node in replicationDestinations)
                    {
                        ...
                        }.Initialize())
                        {
                            ...
                            {
                                var sources =
                                    session.Advanced.LoadStartingWith<RavenJObject>("Raven/Replication/Sources/",
                            ...
                            using (var session = curNodeStore.OpenSession())
                            {
                                foreach (var replicationDestination in replicationDestinations.Where(x =>
                                    x.Url != node.Url))
                                {
                                    ...
                                    {
                                        replicationDestinationUrl =
                                            replicationDestinationUrl.Substring(0,
                                                replicationDestinationUrl.Length - 1);
                                    }

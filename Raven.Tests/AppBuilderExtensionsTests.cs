...
namespace Raven.Tests
{
    public class AppBuilderExtensionsTests : RavenTest
    {
        ...
        public void When_HostOnAppDisposing_key_not_exist_then_should_not_throw()
        {
            ...
            configuration.SetSetting(InMemoryRavenConfiguration.GetKey(x => x.Core.DataDirectory), path);
            configuration.SetSetting(InMemoryRavenConfiguration.GetKey(x => x.FileSystem.DataDirectory), Path.Combine(path, "FileSystem"));

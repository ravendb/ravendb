	...
	public class CanPageThroughReduceResults : RavenTest
	{
		...
		public void Test()
		{
			...
			using (var store = new DocumentStore{Url = "http://localhost:8079/"}.Initialize())
			{
				...
				{
                    new SmugglerApi(new RavenConnectionStringOptions { Url = store.Url }).ImportData(new SmugglerImportOptions { FromStream = stream }, new SmugglerOptionsBase()).Wait(TimeSpan.FromSeconds(15));
				}

...
namespace Raven.Tests.MailingList
{
    public class NonHttpBackupRestore : RavenTest
    {
        ...
        public void CanImportFromDumpFile()
        {
            ...
            {
                var dumper = new DataDumper(store.DocumentDatabase);
                var stats = dumper.ImportData(options);

                Assert.Equal(1, stats.Attachments);
                Assert.Equal(2, stats.Documents); // Will also have a SysDoc/HiLo.
                Assert.Equal(1, stats.Indexes);
        ...
        public void ImportReplacesAnExistingDatabase()
        {
            ...
            using (var store = NewDocumentStore())
            {
                ...
                using (var session = store.OpenSession())
                {
                    ...
                    // The newly added attachment has been removed.
                    Assert.Null(store.DatabaseCommands.GetAttachment("Attachments/2"));

                    // Original person has been restored.
                    Assert.NotNull(session.Query<Person, PeopleByName>().Customize(x => x.WaitForNonStaleResults()).Single(x => x.Name == "Sean"));

                    // The newly added person has been removed.
                    Assert.False(session.Query<Person, PeopleByName>().Customize(x => x.WaitForNonStaleResults()).Any(x => x.Name == "Gillian"));
                }
            }
        }

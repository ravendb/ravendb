		...
		public void ShouldWork()
		{
			...
			using (var documentStore = new EmbeddableDocumentStore())
			{
				...
				using (var session = documentStore.OpenSession())
				{
					...
					session.Query<FeedItem>().Customize(x => x.WaitForNonStaleResultsAsOfNow()).Statistics(out stats)
						.Where(fi => fi.RelatedProfiles.Any(rp => rp.In(profiles))).ToArray();
				}
			}
		} 
	}

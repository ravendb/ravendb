	...
	public class RavenDB_2387 : RavenTestBase
	{
		...
		public void Updates_within_dtc_should_be_reindexed_correctly()
		{
			using (var store = NewDocumentStore(requestedStorage: "esent"))
			{
				...
				using (var session = store.OpenSession())
				{
					var objects = session.Advanced.LuceneQuery<object>("ChildWithParentDocument").WhereEquals("Data", "Foo").ToList();
				...
				using (var session = store.OpenSession())
				{
					var objects = session.Advanced.LuceneQuery<object>("ChildWithParentDocument").WhereEquals("Data", "Bar").ToList();
					Assert.Equal(1, objects.Count);
				}

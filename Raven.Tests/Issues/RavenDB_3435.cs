...
namespace Raven.Tests.Issues
{
	public class RavenDB_3435 : ReplicationBase
	{
		...
		public void Resolution_of_conflict_should_delete_all_conflict_files_with_mocked_cluster()
		{
			...
			{
				storeB.DatabaseCommands.EnsureDatabaseExists(TestDatabaseName);

				var serverUrl = servers[0].Server.Configuration.ServerUrl;
				...
				var url = string.Format("{0}databases/{1}/replication/replicateDocs?from=http%3A%2F%2Fmichael%3A9000%2Fdatabases%2FtestDB&dbid=b2f4bdf5-9bc2-46bf-a173-8c441c5b3b5a&count=2", serverUrl, TestDatabaseName);
				...
				WaitForReplication(storeB,"users/1",TestDatabaseName);
				...
				url = string.Format("{0}databases/{1}/replication/replicateDocs?from=http%3A%2F%2Fmichael%3A9000%2Fdatabases%2FtestDB&dbid=b2f4bdf5-9bc2-46bf-a173-8c441c5b3b5a&count=1", serverUrl, TestDatabaseName);
		...
		private static void ChangeDocument(IDocumentStore store, string id, string newName)
		{
			using (var session = store.OpenSession(TestDatabaseName))
			{

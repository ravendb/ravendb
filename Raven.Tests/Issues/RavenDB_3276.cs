...
namespace Raven.Tests.Issues
{
	public class RavenDB_3276 : RavenTestBase
	{
		...
		public void Direct_request_with_empty_json_property_should_result_in_422_http_code()
		{
			...
			{
				var url = String.Format("{0}/databases/TestDB/bulk_docs", store.Url);
				...
				Assert.Throws<InvalidOperationException>(() =>
				{
					...
					Assert.Equal(422, (int)request.ResponseStatusCode);
				});
			}
		}
		...
		public async Task Dictionary_with_empty_string_as_key_should_fail_bulk_insert_request()
		{
			...
			using (var store = NewRemoteDocumentStore(databaseName:"TestDB"))
			{
				var url = String.Format("{0}/databases/TestDB", store.Url);
				using (ConnectionOptions.Expect100Continue(new Uri(url)))
				{
					var operationUrl = "/bulkInsert?&operationId=" + Guid.NewGuid();
					var createHttpJsonRequestParams = new CreateHttpJsonRequestParams(null, url + operationUrl, "POST",
						new OperationCredentials(String.Empty, CredentialCache.DefaultNetworkCredentials), new DocumentConvention());
					var request = jsonRequestFactory.CreateHttpJsonRequest(createHttpJsonRequestParams);

					using (var requestStream = await request.GetRawRequestStream())
					{

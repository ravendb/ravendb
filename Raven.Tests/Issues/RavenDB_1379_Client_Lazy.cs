	        ...
	        using (var documentStore = NewRemoteDocumentStore(requestedStorage: requestedStorage))
	        {
	            ...
	            using (var session = documentStore.OpenSession())
	            {
	                var fetchedDocuments = session.Advanced.Lazily
                        .LoadStartingWith<JsonDocument>("FooBar", string.Empty, 0, 4, string.Empty)
                        .Value
                        .ToList();

                    //var foundDocKeys = fetchedDocuments.Select(doc => doc["@metadata"].Value<string>("@id")).ToList();
	            ...
	            {
                    var fetchedDocuments = session.Advanced.Lazily
                        .LoadStartingWith<RavenJObject>("FooBar", string.Empty, 4, 4, string.Empty)
                        .Value
                        .ToList();
	            ...
	            {
                    var fetchedDocuments = session.Advanced.Lazily
                        .LoadStartingWith<RavenJObject>("FooBar", string.Empty, 8, 4, string.Empty)
                        .Value
                        .ToList();

	...
	public class RavenDB2537 : RavenTest
	{
		...
		public void ShutdownDatabaseDuringPreparedTransaction()
		{
			using (var store = NewDocumentStore(requestedStorage: "esent", runInMemory: false))
			{
				...
				store.DocumentDatabase.Put("test", null, new RavenJObject{{"Exists", true}}, new RavenJObject(), transactionInformation);

				Assert.False(store.DocumentDatabase.Get("test", null).DataAsJson.Value<bool>("Exists"));

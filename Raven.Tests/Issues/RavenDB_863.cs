	...
	public class RavenDB_863 : RavenTest
	{
		...
		public void NumberOfLoadedItemsToReduceShouldBeLimited(string storageType)
		{
			using (var storage = NewTransactionalStorage(requestedStorage: storageType))
			{
				...
				storage.Batch(accessor =>
				{
					var results = accessor.MapReduce.GetItemsToReduce(new GetItemsToReduceParams("test", new[] { "a", "b" }, 0, true, 2, new List<object>(), new HashSet<Tuple<string, int>>(), new List<string>())).ToList();
		...
		public void LimitOfLoadedItemsShouldNotBreakInTheMiddleOfBucket(string storageType)
		{
			using (var storage = NewTransactionalStorage(requestedStorage: storageType))
			{
				...
				storage.Batch(accessor =>
				{
					var results = accessor.MapReduce.GetItemsToReduce(new GetItemsToReduceParams("test", new[] { "a", "b" }, 0, true, 3, new List<object>(), new HashSet<Tuple<string, int>>(), new List<string>())).ToList();

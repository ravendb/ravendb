	...
	public class ApiKey : RemoteClientTest
	{
		...
		public void CanAuthAsAdminAgainstTenantDb()
		{
			using (var server = GetNewServer(enableAuthentication: true))
			{
				...
				{
					store.JsonRequestFactory.CreateHttpJsonRequest(new CreateHttpJsonRequestParams(null, store.Url + "/databases/test/admin/changeDbId", "POST", new OperationMetadata.OperationCredentials("dbadmin/ThisIsMySecret", null), store.Conventions))
						.ExecuteRequest();// can do admin stuff

					var httpJsonRequest = store.JsonRequestFactory.CreateHttpJsonRequest(new CreateHttpJsonRequestParams(null, store.Url + "/databases/test/debug/user-info", "GET", new OperationMetadata.OperationCredentials("dbadmin/ThisIsMySecret", null), store.Conventions));

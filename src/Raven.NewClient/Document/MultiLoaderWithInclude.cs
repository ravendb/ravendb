...
namespace Raven.NewClient.Client.Document
{
    ...
    public class MultiLoaderWithInclude<T> : ILoaderWithInclude<T>
    {
        ...
        /// <returns></returns>
        public IDictionary<string, T> Load(params string[] ids)
        {
        ...
        /// <param name="ids">The ids.</param>
        /// <returns></returns>
        public IDictionary<string, T> Load(IEnumerable<string> ids)
        {
        ...
        /// Or whatever your conventions specify.
        /// </remarks>
        public IDictionary<string, T> Load(params ValueType[] ids)
        {
        ...
        /// Or whatever your conventions specify.
        /// </remarks>
        public IDictionary<string, T> Load(IEnumerable<ValueType> ids)
        {
            var documentKeys = ids.Select(id => session.Conventions.FindFullDocumentKeyFromNonStringIdentifier(id, typeof(T), false));
            return Load(documentKeys);
        }
        ...
        /// <param name="ids">The ids.</param>
        public IDictionary<string, TResult> Load<TResult>(params string[] ids)
        {
        ...
        /// <typeparam name="TResult"></typeparam>
        /// <param name="ids">The ids.</param>
        public IDictionary<string, TResult> Load<TResult>(IEnumerable<string> ids)
        {
        ...
        public IDictionary<string, TResult> Load<TResult>(params ValueType[] ids)
        {
            var documentKeys = ids.Select(id => session.Conventions.FindFullDocumentKeyFromNonStringIdentifier(id, typeof(T), false));
            return Load<TResult>(documentKeys);
        }

        public IDictionary<string, TResult> Load<TResult>(IEnumerable<ValueType> ids)
        {
            var documentKeys = ids.Select(id => session.Conventions.FindFullDocumentKeyFromNonStringIdentifier(id, typeof(T), false));
            return Load<TResult>(documentKeys);
        }
            ...
            where TTransformer : AbstractTransformerCreationTask, new()
        {
            ...
            return session.LoadInternal<TResult>(new[] { id }, includes.ToArray(), transformer, configuration.TransformerParameters).Values.FirstOrDefault();
        }

        public IDictionary<string, TResult> Load<TTransformer, TResult>(IEnumerable<string> ids, Action<ILoadConfiguration> configure = null)
            where TTransformer : AbstractTransformerCreationTask, new()
        {
            var transformer = new TTransformer().TransformerName;
            var configuration = new RavenLoadConfiguration();

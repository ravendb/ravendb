...
class amazonSqsTaskTestMode {
    ...
    runTest() {
        ...
        if (testValid && parentValid) {
            ...
            eventsCollector.default.reportEvent("amazon-sqs-etl", "test-script");

            new testQueueEtlCommand(this.db(), dto, "AmazonSqs")
                .execute()
...
class editAmazonSqsEtlTask extends viewModelBase {
    view = require("views/database/tasks/editAmazonSqsEtlTask.html");
    optionsPerQueueEtlView = require("views/database/tasks/optionsPerQueueEtl.html");
    connectionStringView = require("views/database/settings/connectionStringAmazonSqs.html");
    ...
    test: amazonSqsTaskTestMode;

    editedEtl = ko.observable<ongoingTaskAmazonSqsEtlEditModel>();
        ...
        if (args.taskId) {
            ...
            getOngoingTaskInfoCommand.forQueueEtl(this.activeDatabase(), args.taskId)
                ...
                .done((result: Raven.Client.Documents.Operations.OngoingTasks.OngoingTaskQueueEtl) => {
                    this.editedEtl(new ongoingTaskAmazonSqsEtlEditModel(result));
            ...
            this.editedEtl(ongoingTaskAmazonSqsEtlEditModel.empty());
    ...
    compositionComplete() {
        ...
        $('.edit-amazon-sqs-etl-task [data-toggle="tooltip"]').tooltip();
    }
    ...
    private getAllConnectionStrings() {
        return new getConnectionStringsCommand(this.activeDatabase())
            ...
            .done((result: Raven.Client.Documents.Operations.ConnectionStrings.GetConnectionStringsResult) => {
                ...
                const amazonSqsStrings = queueConnectionStrings.filter(x => x.BrokerType === "AmazonSqs");
                this.etlConnectionStringsDetails(_.sortBy(amazonSqsStrings, (x: Raven.Client.Documents.Operations.ETL.Queue.QueueConnectionString) => x.Name.toUpperCase()));
            });
    }
    ...
    private initObservables() {
        ...
        const dtoProvider = () => {
            ...
            if (!dto.Name) {
                dto.Name = "Test Amazon SQS ETL Task"; // assign fake name
            }
            return dto;
        };

        this.test = new amazonSqsTaskTestMode(this.activeDatabase, () => {
    ...
    onTestConnection() {
        eventsCollector.default.reportEvent("amazon-sqs-connection-string", "test-connection");
        this.spinners.test(true);
        ...
        if (this.createNewConnectionString()) {
            ...
            getConnectionStringInfoCommand.forAmazonSqsEtl(this.activeDatabase(), this.editedEtl().connectionStringName())
                .execute()
                .done((result: Raven.Client.Documents.Operations.ConnectionStrings.GetConnectionStringsResult) => {
                    new connectionStringAmazonSqsModel(result.QueueConnectionStrings[this.editedEtl().connectionStringName()], true, [])
                        .testConnection(this.activeDatabase())
    ...
    saveEtl() {
        ...
        savingNewStringAction.done(()=> {
            eventsCollector.default.reportEvent("amazon-sqs-etl", "save");
    ...
    private findNameForNewTransformation() {
        const scriptsWithPrefix = this.editedEtl().transformationScripts().filter(script => {
            return script.name().startsWith(editAmazonSqsEtlTask.scriptNamePrefix);
        });

        const maxNumber = _.max(scriptsWithPrefix
            .map(x => x.name().substr(editAmazonSqsEtlTask.scriptNamePrefix.length))
            .map(x => _.toInteger(x))) || 0;

        return editAmazonSqsEtlTask.scriptNamePrefix + (maxNumber + 1);
    }
    ...
    syntaxHelp() {
        const viewmodel = new transformationScriptSyntax("AmazonSqs");

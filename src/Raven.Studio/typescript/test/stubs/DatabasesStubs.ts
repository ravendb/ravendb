...
import { DatabaseSharedInfo, ShardedDatabaseInfo } from "components/models/databases";

export class DatabasesStubs {
    static databaseSharedInfo(isSharded = false): DatabaseSharedInfo {
        if (isSharded) {
            return DatabasesStubs.shardedDatabaseInfo();
        } else {
            return DatabasesStubs.nonShardedDatabaseInfo();
        }
    }

    static nonShardedDatabaseInfo(): DatabaseSharedInfo {
        return {
            name: "some-name",
            encrypted: false,
            sharded: false,
            nodes: [
                {
                    tag: "A",
                    nodeUrl: "http://live-test.ravendb.net",
                    type: "Member",
    ...
    static shardedDatabaseInfo(): ShardedDatabaseInfo {
        return {
            name: "some-name",
            encrypted: false,
            sharded: true,

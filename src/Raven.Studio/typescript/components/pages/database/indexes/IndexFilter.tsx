        ...
        return (
            <div>
                <small className="on-base-background">
                    All <strong>Index Status</strong> options are unchecked. Please select options under <strong>'Index Status'</strong> to view indexes list.
                </small>
            </div>
        )
    }
    
    const indexingErrorsOnlyPart = filter.showOnlyIndexesWithIndexingErrors ? <>, with <strong>indexing errors only</strong>,</> : "";
    
    const firstPart = indexesCount
        ? <>Displaying <strong>{indexesCount}</strong> {pluralizeHelpers.pluralize(indexesCount, "index", "indexes", true)}
...
export default function IndexFilter(props: IndexFilterProps) {
    ...
    const toggleStatus = (status: IndexStatus) => {
        props.setFilter(f => ({
            ...f,
            status: filter.status.includes(status) ? filter.status.filter(x => x !== status) : filter.status.concat(status)
        }));
    };
    ...
    return (
        <div className="btn-group-label" data-label="Filter">
            <input type="text" accessKey="/" className="form-control" placeholder="Index Name" title="Filter indexes" value={filter.searchText} onChange={onSearchTextChange}/>
            <div className="dropdown dropdown-right">
                <button className={classNames("btn btn-default dropdown-toggle", { active: hasAnyStateFilter(filter) })} type="button" data-toggle="dropdown">
                ...
                <DropdownPanel className="settings-menu">
                    <div className="settings-item">
                        <div className="margin-left margin-right margin-right-sm">
                            ...
                            <IndexFilterStatusItem toggleStatus={() => toggleStatus("ErrorOrFaulty")} checked={filter.status.includes("ErrorOrFaulty")} label="Error / Faulty" toggleClass="toggle-danger" />
                            <IndexFilterStatusItem toggleStatus={() => toggleStatus("Stale")} checked={filter.status.includes("Stale")} label="Stale" toggleClass="toggle-warning" />
                            <IndexFilterStatusItem toggleStatus={() => toggleStatus("RollingDeployment")} checked={filter.status.includes("RollingDeployment")} label="Rolling deployment" toggleClass="toggle-warning" />
                            <IndexFilterStatusItem toggleStatus={() => toggleStatus("Paused")} checked={filter.status.includes("Paused")} label="Paused" toggleClass="toggle-warning" />
                            <IndexFilterStatusItem toggleStatus={() => toggleStatus("Disabled")} checked={filter.status.includes("Disabled")} label="Disabled" toggleClass="toggle-warning" />
                            <IndexFilterStatusItem toggleStatus={() => toggleStatus("Idle")} checked={filter.status.includes("Idle")} label="Idle" toggleClass="toggle-warning" />
                        </div>
                            ...
                            <div className="flex-horizontal">
                                ...
                                <div className="flex-noshrink">
                                    <div className="toggle toggle-danger">
                                        <input type="checkbox" className="styled" checked={filter.showOnlyIndexesWithIndexingErrors} onChange={toggleIndexesWithErrors} />
                                        <label/>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="flex-horizontal">
                                ...
                                <div className="flex-noshrink">
                                    <div className="toggle toggle-info">
                                        <input type="checkbox" className="styled" checked={filter.autoRefresh} onChange={toggleAutoRefresh} />
                                        <label/>
                                    </div>
                                </div>
                            </div>

...
class getTransformersCommand extends commandBase {
    ...
    execute(): JQueryPromise<Array<Raven.Client.Indexing.TransformerDefinition>> {
        const args = {
            start: this.skip,
            pageSize: this.take
        };
        const url = endpoints.databases.transformer.transformers + this.urlEncodeArgs(args);
        const extractor = (response: resultsDto<Raven.Client.Indexing.TransformerDefinition>) => response.Results;

...
export async function getSuggestionsForParseTree(
    parser: RqlParser, parseTree: ProgContext, symbolTableFn: () => SymbolTable, caretPosition: CaretPosition, writtenPart: string): Promise<autoCompleteWordList[]> {
    ...
    for (const provider of providers) {
        if (provider.collect) {
            completions.push(...provider.collect(scanner, candidates, parser, writtenPart));
        }
        if (provider.collectAsync) {
            const providerCompletions = await provider.collectAsync(scanner, candidates, parser, writtenPart);

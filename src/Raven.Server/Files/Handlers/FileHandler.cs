...
namespace Raven.Server.Files.Handlers
{
    ...
    {
        [RavenAction("/filesystem|s|fs/#1#docs", "HEAD", "/databases/{databaseName:string}/docs?id={documentId:string}")]
        public Task Head()
        {
            ...
            {
                var document = Constants.Database.DocumentsStorage.Get(context, id);
                if (document == null)
        ...
        [RavenAction("/databases/#1#docs", "GET", "/databases/{databaseName:string}/docs?id={documentId:string|multiple}&include={fieldName:string|optional|multiple}&transformer={transformerName:string|optional}")]
        public Task Get()
        {
            ...
            {
                transformer = Constants.Database.TransformerStore.GetTransformer(transformerName);
                if (transformer == null)
        ...
        [RavenAction("/databases/#1#docs", "POST", "/databases/{databaseName:string}/docs body{documentsIds:string[]}")]
        ...
        private void GetDocuments(DocumentsOperationContext context, Transformer transformer, bool metadataOnly)
        {
            ...
            var start = GetStart();
            var pageSize = GetPageSize(Constants.Database.Configuration.Core.MaxPageSize);

            IEnumerable<Document> documents;
            if (etag != null)
            {
                documents = Constants.Database.DocumentsStorage.GetDocumentsFrom(context, etag.Value, start, pageSize);
            }
            else if (HttpContext.Request.Query.ContainsKey("startsWith"))
            {
               documents = Constants.Database.DocumentsStorage.GetDocumentsStartingWith(context,
            ...
            {
                documents = Constants.Database.DocumentsStorage.GetDocumentsInReverseEtagOrder(context, start, pageSize);
            }
            ...
            using (var writer = new BlittableJsonTextWriter(context, ResponseBodyStream()))
            {
                ...
                if (transformer != null)
                {
                    ...
                    using (var scope = transformer.OpenTransformationScope(transformerParameters, null, Constants.Database.DocumentsStorage,
                        Constants.Database.TransformerStore, context))
                    {
                ...
                writer.WriteInteger(Constants.Database.DocumentsStorage.NextPage);
        ...
        private void GetDocumentsById(DocumentsOperationContext context, StringValues ids, Transformer transformer, bool metadataOnly)
        {
            ...
            var includeDocs = new IncludeDocumentsCommand(Constants.Database.DocumentsStorage, context, includePaths);
            ...
            if (transformer != null)
            {
                ...
                using (var scope = transformer.OpenTransformationScope(transformerParameters, includeDocs, Constants.Database.DocumentsStorage, Constants.Database.TransformerStore, context))
                {
        ...
        private unsafe long ComputeAllDocumentsEtag(DocumentsOperationContext context)
        {
            ...
            buffer[1] = Constants.Database.DocumentsStorage.GetNumberOfDocuments(context);
        ...
        [RavenAction("/databases/#1#docs", "DELETE", "/databases/{databaseName:string}/docs?id={documentId:string}")]
        public async Task Delete()
        {
            ...
            using (ContextPool.AllocateOperationContext(out context))
            {
                ...
                await Constants.Database.TxMerger.Enqueue(cmd);
        ...
        [RavenAction("/databases/#1#docs", "PUT", "/databases/{databaseName:string}/docs?id={documentId:string}")]
        ...
        [RavenAction("/databases/#1#docs", "PATCH", "/databases/{databaseName:string}/docs?id={documentId:string}&test={isTestOnly:bool|optional(false)} body{ Patch:PatchRequest, PatchIfMissing:PatchRequest }")]
        public Task Patch()
        {
            var id = GetQueryStringValueAndAssertIfSingleAndNotEmpty("id");
            ...
            using (ContextPool.AllocateOperationContext(out context))
            {
                ...
                try
                {
                    ...
                    {
                        patchResult = Constants.Database.Patch.Apply(context, id, etag, patch, patchIfMissing, skipPatchIfEtagMismatch, debugMode: isDebugOnly);
        ...
        [RavenAction("/databases/#1#docs/class", "GET")]

    ...
    public class OngoingTasksHandler : DatabaseRequestHandler
    {
        ...
        public async Task UpdateExternalReplication()
        {
            ...
            using (ServerStore.ContextPool.AllocateOperationContext(out TransactionOperationContext context))
            {
                ...
                using (context.OpenReadTransaction())
                {
                    /*Console.WriteLine("Oren 3 done waiting for index " + index + " " + ServerStore.Engine.GetLastCommitIndex(context) + " " +
                        ServerStore.Cluster._rachisLogIndexNotifications._lastModifiedIndex);*/

                    var record = ServerStore.Cluster.ReadDatabase(context, Database.Name);
                    if (record.ExternalReplication.Count == 0)
                    {
                        Console.WriteLine("Oren 2: Early return!!!!");
                    }

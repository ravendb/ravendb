...
internal sealed class ProxyCommandResponseBehavior : AbstractCommandResponseBehavior
{
    ...
    public override async ValueTask<bool> TryHandleUnsuccessfulResponseAsync<TResult>(JsonOperationContext context, RavenCommand<TResult> command, HttpResponseMessage response, AbstractCommandResponseBehavior.CommandUnsuccessfulResponseBehavior unsuccessfulResponseBehavior)
    {
        HttpResponseHelper.CopyStatusCode(response, _response);
        HttpResponseHelper.CopyHeaders(response, _response);
        await HttpResponseHelper.CopyContentAsync(response, _response).ConfigureAwait(false);

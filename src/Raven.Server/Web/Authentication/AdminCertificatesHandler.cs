...
namespace Raven.Server.Web.Authentication
{
    public class AdminCertificatesHandler : RequestHandler
    {
        ...
        public Task Put()
        {
            return PutCertificate(Constants.Certificates.Prefix);
        }
        ...
        protected async Task PutCertificate(string prefix)
        {
            // one of the first admin action is to create a certificate, so let
            // us also use that to indicate that we are the seed node
            ServerStore.EnsureNotPassive();
            ...
            using (var certificateJson = ctx.ReadForDisk(RequestBodyStream(), "put-certificate"))
            {
                ...
                try
                {
                    await PutCertificateCollectionInCluster(prefix, certificate, certBytes, certificate.Password, ServerStore, ctx);
                }
                catch (Exception e)
                {
        ...
        private static void ValidateCertificateDefinition(CertificateDefinition certificate, ServerStore serverStore)
        {

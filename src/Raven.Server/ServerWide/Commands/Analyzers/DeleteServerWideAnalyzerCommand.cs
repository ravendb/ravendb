...
namespace Raven.Server.ServerWide.Commands.Analyzers
{
    public sealed class DeleteServerWideAnalyzerCommand : DeleteValueCommand
    {
        ...
        {
            context.Transaction.InnerTransaction.LowLevelTransaction.BeforeCommitFinalization += (_,_) => AnalyzerCompilationCache.Instance.RemoveServerWideItem(PutServerWideAnalyzerCommand.ExtractName(Name));
        }
    }
}

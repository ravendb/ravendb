...
public ref struct DirectlyToStreamInterpolatedStringHandler
    {
        // Implementation note:
        ...
        private readonly long _initIndex;
        ...
        public DirectlyToStreamInterpolatedStringHandler(int literalLength, int formattedCount, MemoryStream memoryStream)
        {
            _provider = null;
            ...
            _initIndex = memoryStream.Position;
        }
            ...
            if (value is IFormattable)
            {
                ...
                if (value is ISpanFormattable)
                {
                    while (true)
                    {
                        ...
                        int memoryStreamPosition = (int)_memoryStream.Position;
                        Span<byte> spanBuffer = buffer.AsSpan(memoryStreamPosition, ((int)_memoryStream.Length) - memoryStreamPosition);
                        Span<char> chars = MemoryMarshal.Cast<byte, char>(spanBuffer);
                        if (((ISpanFormattable)value).TryFormat(chars, out var charsWritten, default, _provider))
                        {
                            _memoryStream.Position += charsWritten * sizeof(char);
                            break;
                        }
                        _memoryStream.SetLength(Math.Max(128, 2 * _memoryStream.Length));
                    }
                    
                    // while (!) // constrained call avoiding boxing for value types
                    // {
                    //     Grow();
                    // }
        ...
        {
            ReadOnlySpan<byte> buffer = MemoryMarshal.Cast<char, byte>(value);
        ...
        public void AppendFormatted(string? value)
        {
            ...
            if (!_hasCustomFormatter && value is not null)
            {
                ReadOnlySpan<char> charSpan = value.AsSpan();
                AppendFormatted(charSpan);
            }
            else
            {
        ...
        private void GrowCore(uint requiredMinCapacity)
        {
            ...
            uint newCapacity = Math.Max(requiredMinCapacity, (uint)_chars.Length * 2);

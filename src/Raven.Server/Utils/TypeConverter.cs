...
namespace Raven.Server.Utils
{
    internal static class TypeConverter
    {
        ...
        private static IEnumerable<object> EnumerateArray(object root, JsArray arr, bool flattenArrays, bool forIndexing, int recursiveLevel, bool canTryJsStringToDateConversion, Engine engine, JsonOperationContext context)
        {
            foreach (var val in arr)
            {
                yield return ToBlittableSupportedType(root, val, flattenArrays, forIndexing, recursiveLevel, canTryJsStringToDateConversion, engine, context, out _);
            }
        }

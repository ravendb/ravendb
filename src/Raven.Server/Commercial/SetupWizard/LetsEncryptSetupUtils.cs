...
{
        public static async Task<byte[]> Setup(SetupInfo setupInfo,  SetupProgressAndResult progress, bool registerTcpDnsRecords, string acmeUrl, string directoryPath, CancellationToken token)
        {
            progress.Processed++;
            ...
            var acmeClient = new LetsEncryptClient(acmeUrl, directoryPath);

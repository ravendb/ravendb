        ...
        public void HandleDatabaseRecordChange(DatabaseRecord databaseRecord)
        {
            ...
            using (context.OpenReadTransaction())
            {
                foreach (var subscriptionStateKvp in _subscriptionConnectionStates)
                {
                    ...
                    {
                        DropSubscriptionConnection(subscriptionStateKvp.Key, new SubscriptionClosedException($"The subscription {subscriptionName} query has been modified, connection must be restarted", tryToReconnect: true));
                        continue;
                    }

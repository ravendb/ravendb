...
{
    public abstract class ElasticSearchDocumentTransformerBase<T> : EtlTransformer<ElasticSearchItem, ElasticSearchIndexWithRecords, EtlStatsScope, EtlPerformanceOperation, T>
        where T : struct, IJsHandle<T>
    {
        ...
        protected ElasticSearchDocumentTransformerBase(Transformation transformation, DocumentDatabase database, DocumentsOperationContext context, ElasticSearchEtlConfiguration config)
        ...
        protected override void LoadToFunction(string indexName, ScriptRunnerResult<T> document)
        {
            if (indexName == null)
            ...
            {
                result.Modifications = new DynamicJsonValue { [index.DocumentIdProperty] = ElasticSearchEtl.LowerCaseDocumentIdProperty(Current.Document.Id) };
            }

...
{
    public abstract class OlapDocumentTransformerBase<T> : EtlTransformer<ToOlapItem, OlapTransformedItems, OlapEtlStatsScope, OlapEtlPerformanceOperation, T>
        where T : struct, IJsHandle<T>
    {
        ...
        private T _noPartition;

        protected OlapDocumentTransformerBase(Transformation transformation, DocumentDatabase database, DocumentsOperationContext context, OlapEtlConfiguration config)
        ...
        protected override void LoadToFunction(string tableName, ScriptRunnerResult<T> colsAsObject)
        {
        }

        private void LoadToFunction(string tableName, string key, ScriptRunnerResult<T> runnerResult, List<string> partitions = null)
        {
            if (key == null)
        ...
        private T LoadToFunctionTranslator(string name, T[] args)
        {
            var methodSignature = $"loadTo{name}(key, obj)";
        ...
        private T LoadToFunctionTranslatorInternal(string name, T key, T obj, string methodSignature)
        {
        ...
        //TODO: egor handle the exceptions (we cannot throw inside script in v8)
        private T PartitionBy(T self, T[] args)
        {
            if (args.Length == 0)
            ...
            else
            {
                ...
                //TODO: because it expects jsvalue but we pass T so it jsut process it as object.
                //jsArr = EngineHandle.FromObjectGen(args);
            }

            T o;
            //TODO: egor we cannot dispose it here, we return it (need to clone/ dispose afterwards)
            /*using (jsArr)
            {*/
        ...
        private T NoPartition(T self, T[] args)
        {
            if (args.Length != 0)

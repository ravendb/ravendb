...
public class AllNodesExecutor : AbstractExecutor
{
    ...
    public AllNodesExecutor(ServerStore store, string database) : base(store)
    {
        ...
        UpdateExecutors(_clusterTopology);
    }
    ...
    private void RegisterForTopologyChange()
    {
        ...
        task.ContinueWith(_ =>
        {
            ...
            UpdateExecutors(topology);
        }, TaskContinuationOptions.OnlyOnRanToCompletion);
    }

    private void UpdateExecutors(ClusterTopology current)
    {
        ...
        lock (this)
        {
            foreach (var node in current.AllNodes)
            {
                ...
                {
                    _current[tag] = RequestExecutor.CreateForSingleNodeWithoutConfigurationUpdates(url, _database, _store.Server.Certificate.Certificate, DocumentConventions.DefaultForServer);
                    continue;
                }
            ...
            {
                FullRange = Enumerable.Range(0, current.AllNodes.Count).ToArray(),

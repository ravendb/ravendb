...
{
    internal sealed class ShardedTimeSeriesHandlerProcessorForGetTimeSeriesStats : AbstractTimeSeriesHandlerProcessorForGetTimeSeriesStats<ShardedDatabaseRequestHandler, TransactionOperationContext>
    {
        ...
        protected override async ValueTask GetTimeSeriesStatsAndWriteAsync(TransactionOperationContext context, string docId)
        {
            ...
            await RequestHandler.ShardExecutor.ExecuteSingleShardAsync(new ProxyCommand<TimeSeriesStatistics>(op, RequestHandler.HttpContext), shardNumber);
        }
    }
}

...
public class ShardedMapReduceStreamingEnumerator : MergedEnumerator<BlittableJsonReaderObject>
{
    ...
    {
        DocumentsComparer.RetrieveConfigurationForDocumentsComparer(databaseContext, queryStats.IndexName, out var defaultNullsSortMode, out var acceptMissing);
        return new DocumentsComparer(reduceKeys.ToArray(), extractFromData: true, hasOrderByRandom: false, defaultNullsSortMode, acceptMissing);
    }

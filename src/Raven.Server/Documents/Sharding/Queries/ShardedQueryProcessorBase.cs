    ...
    where TCombinedResult : QueryResultServerSide<BlittableJsonReaderObject>
{
    ...
    protected void ReduceResults(ref TCombinedResult result, QueryTimingsScope scope)
    {
        ...
        using (scope?.For(nameof(QueryTimingsScope.Names.Reduce)))
        {
            ...
            {
                DocumentsComparer.RetrieveConfigurationForDocumentsComparer(RequestHandler.DatabaseContext, result.IndexName, out var defaultNullsSortMode, out bool acceptMissing);

                // apply ordering after the re-reduce of a map-reduce index
                result.Results.Sort(new DocumentsComparer(orderByFields, extractFromData: true, Query.Metadata.HasOrderByRandom, defaultNullsSortMode, acceptMissing));
            }
        }
    }

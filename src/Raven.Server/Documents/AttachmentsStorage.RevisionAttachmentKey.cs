...
// cv.Version bytes (Legacy row) or its Blake2b-128 (Hashed row). `[attHash44]`
...
// (Legacy rows = rawCv, Hashed rows = hash22). On the wire the revCv slot is
// always rawCv: senders rebuild to Legacy form before emit
// (`BuildLegacyAttachmentCompositeForExternalEmit`) and receivers reject
...
{
    //   HashComposite     -- parent revision Blake2b-128 in the revCv slot (canonical / Hashed row)
    //   RawComposite      -- parent revision cv.Version bytes in the revCv slot (Legacy row / wire)
    //   RevisionVersion   -- parent revision's cv.Version bytes. Threaded in from the caller's
    //                        RevisionKey.Raw (or the wire-parsed rawCv slice on the receive path).
    //                        Written into AttachmentsTable.RevisionVersion (field 11 on v7.2) on RA Put,
    //                        and TombstoneTable.RevisionVersion (field 9) on RAT Create.
    ...
    internal readonly struct RevisionAttachmentKeyScope : IDisposable
    {
        private readonly ByteStringContext<ByteStringMemoryCache>.InternalScope _withHashScope;
        private readonly ByteStringContext<ByteStringMemoryCache>.InternalScope _withRawCvScope;
        private readonly ByteStringContext<ByteStringMemoryCache>.InternalScope _revisionVersionScope;
    ...
    public unsafe partial class AttachmentsStorage
    {
        ...
        private static RevisionAttachmentKeyScope BuildPair(
            ...
            out RevisionAttachmentKey pair)
        {
            var withHashScope = build(context, revisionKey.Hash, out Slice withHash);
        ...
        public static string ExtractRevCvSegment(LazyStringValue key)
        {
            ...
            if (cvSegmentSize == RevisionsStorage.RevisionKeyHashSize)
                throw new InvalidOperationException(
        ...
        internal bool TryReadRevisionAttachmentByKey(Table table, in RevisionAttachmentKey pair, out TableValueReader tvr)
            => _documentDatabase.DocumentsStorage.RevisionsStorage.DualForm.TryRead(
                pair.HashComposite, pair.RawComposite, table.SeekOnePrimaryKeyPrefix, out tvr);

        internal void DeleteRevisionAttachmentByKey(Table table, in RevisionAttachmentKey pair)
            => _documentDatabase.DocumentsStorage.RevisionsStorage.DualForm.Apply(
                pair.HashComposite, pair.RawComposite, table.DeleteByKey);
            ...
            out RevisionAttachmentKey pair)
        {
            ...
            if (parts.CvSegmentSize == RevisionsStorage.RevisionKeyHashSize)
                throw new InvalidOperationException(
            ...
            using (Slice.External(context.Allocator, src + parts.AttHashStart, parts.AttHashSize, out Slice base64Hash))
            using (RevisionsStorage.GetRevisionKeyHashSlice(context.Allocator, rawCvSlice, out Slice hashSlice, out Slice prefixedHashSlice))
            {
                var synthRevisionKey = new RevisionKey(hashSlice, prefixedHashSlice, rawCvSlice);
        ...
        internal bool TryReadRevisionAttachmentTombstoneByKey(Table tombstoneTable, in RevisionAttachmentKey pair, out TableValueReader tvr)
            => _documentDatabase.DocumentsStorage.RevisionsStorage.DualForm.TryRead(
                pair.HashComposite, pair.RawComposite, tombstoneTable.ReadByKey, out tvr);
        ...
        internal bool VerifyRevisionAttachmentTombstoneExists(Table tombstoneTable, in RevisionAttachmentKey pair)
            => _documentDatabase.DocumentsStorage.RevisionsStorage.DualForm.Exists(
                pair.HashComposite, pair.RawComposite, tombstoneTable.VerifyKeyExists);
            ...
            string expectedChangeVector, string changeVector, long lastModifiedTicks)
        {
            ...
            bool found = dualForm.TryRead(pair.HashComposite, pair.RawComposite, table.ReadByKey, out TableValueReader tvr);

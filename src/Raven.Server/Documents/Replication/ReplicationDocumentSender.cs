...
namespace Raven.Server.Documents.Replication
{
    public class ReplicationDocumentSender : IDisposable
    {
        ...
        private bool CanContinueBatch(ReplicationState state, ref long next)
        {
            ...
            if (_stats.Storage.CurrentStats.InputCount > 0 && _stats.Storage.CurrentStats.InputCount % 16384 == 0)
            {
        ...
        private bool ShouldSkip(ReplicationBatchItem item, OutgoingReplicationStatsScope stats, SkippedReplicationItemsInfo skippedReplicationItemsInfo)
        {
            ...
            switch (item)
            {
                case DocumentReplicationItem document:
                    if (document.Flags.Contain(DocumentFlags.Artificial))
                    {
                    ...
                    if (document.Flags.Contain(DocumentFlags.Revision) || document.Flags.Contain(DocumentFlags.DeleteRevision))
                    {
                        ...
                        if (document.Flags.Contain(DocumentFlags.Conflicted) ||
                            document.Flags.Contain(DocumentFlags.Resolved) ||
                            (document.Flags.Contain(DocumentFlags.FromClusterTransaction)))
                        {
                            return false;
                        }
                    }

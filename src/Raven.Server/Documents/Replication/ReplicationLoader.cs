        ...
        public long GetMinimalEtagForTombstoneCleanupWithHubReplication(Dictionary<string, LastTombstoneInfo> result, string collection)
        {
            ...
            using (ctx.OpenReadTransaction())
            {
                ...
                if (lastCleanUp.AddMinutes(time) > Database.Time.GetUtcNow())
                {
                    ...
                    {
                        result.Add("Hub Replication", new LastTombstoneInfo("Hub Replication", collection, _hubInfoForCleaner.LastEtag));
                    }
            ...
            using (context.OpenReadTransaction())
            {
                ...
                if (maxTombstone.LastModified <= lastDateToSave)
                {
                    ...
                    result.Add("Hub Replication", new LastTombstoneInfo("", collection, max));
                    return max;
                }
                ...
                if (minTombstone.LastModified > lastDateToSave)
                {
                    ...
                    result.Add("Hub Replication", new LastTombstoneInfo("Hub Replication", collection, minTombstone.Etag - 1));
                ...
                while (true)
                {
                    ...
                    if (newEtag == oldEtag)
                    {
                        ...
                        result.Add("Hub Replication", new LastTombstoneInfo("Hub Replication", collection, min));
                        return min;
                    }

...
namespace Raven.Server.Documents.Indexes.MapReduce.Static
{
    public class MapReduceIndex : MapReduceIndexBase<MapReduceIndexDefinition, IndexField>
    {
            ...
            where TStaticIndex : MapReduceIndex
        {
            TStaticIndex instance;
            if (typeof(TStaticIndex) == typeof(MapReduceIndex))
                instance = (TStaticIndex)CreateIndexInstance<MapReduceIndex, StaticIndexBase>(definition, documentDatabase.Configuration, IndexDefinitionBaseServerSide.IndexVersion.CurrentVersion, (staticMapIndexDefinition, staticIndex) => new MapReduceIndex(staticMapIndexDefinition, staticIndex));
            else if (typeof(TStaticIndex) == typeof(MapReduceTimeSeriesIndex))
                instance = (TStaticIndex)(MapReduceIndex)CreateIndexInstance<MapReduceTimeSeriesIndex, StaticTimeSeriesIndexBase>(definition, documentDatabase.Configuration, IndexDefinitionBaseServerSide.IndexVersion.CurrentVersion, (staticMapIndexDefinition, staticIndex) => new MapReduceTimeSeriesIndex(staticMapIndexDefinition, staticIndex));
            else if (typeof(TStaticIndex) == typeof(MapReduceCountersIndex))
                instance = (TStaticIndex)(MapReduceIndex)CreateIndexInstance<MapReduceCountersIndex, StaticCountersIndexBase>(definition, documentDatabase.Configuration, IndexDefinitionBaseServerSide.IndexVersion.CurrentVersion, (staticMapIndexDefinition, staticIndex) => new MapReduceCountersIndex(staticMapIndexDefinition, staticIndex));
            else
                throw new NotSupportedException($"Not supported index type {typeof(TStaticIndex).Name}");
        ...
        private static TStaticIndex CreateIndexInstance<TStaticIndex, TStaticIndexBase>(IndexDefinition definition, RavenConfiguration configuration, long indexVersion, Func<MapReduceIndexDefinition, TStaticIndexBase, TStaticIndex> factory)
            where TStaticIndex : MapReduceIndex
            where TStaticIndexBase : AbstractStaticIndexBase
        {
            var staticMapIndexDefinition = CreateIndexDefinition<TStaticIndexBase>(definition, configuration, indexVersion, out var staticIndex);
            return factory(staticMapIndexDefinition, staticIndex);
        }

        private static MapReduceIndexDefinition CreateIndexDefinition<TStaticIndexBase>(IndexDefinition definition, RavenConfiguration configuration, long indexVersion, out TStaticIndexBase staticIndex)
            where TStaticIndexBase : AbstractStaticIndexBase
        {

...
namespace Raven.Server.Documents.Indexes.Static
{
    public class DynamicBlittableJson : DynamicObject, IEnumerable<object>, IBlittableJsonContainer
    {
        ...
        private bool TryGetByName(string name, out object result)
        {
            ...
            if (string.Compare(name, Constants.Indexing.Fields.DocumentIdFieldName, StringComparison.OrdinalIgnoreCase) == 0 || 
                string.Compare(name, "Id", StringComparison.OrdinalIgnoreCase) == 0 )
            {
            ...
            if (result == null && string.Compare(name, "HasValue", StringComparison.OrdinalIgnoreCase) == 0 )
            {
                result = getResult;
                return true;
            }
            ...
            if (string.Compare(name,Constants.Metadata.Key, StringComparison.OrdinalIgnoreCase) == 0)
            {
        ...
        public object this[string key]
        {
            get
            {
                if (string.Compare(Constants.Headers.LastModified, key, StringComparison.OrdinalIgnoreCase) == 0) // TODO - avoid two headers for last doc modification

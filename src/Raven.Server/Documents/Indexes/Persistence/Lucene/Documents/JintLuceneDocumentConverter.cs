            ...
            IWriteOperationBuffer writeBuffer, object sourceDocument)
        {
                    ...
                    if (isObject)
                    {
                        ...
                        if (val != null)
                        {
                            ...
                            else
                            {
                                ...
                                numberOfCreatedFields = GetRegularFields(instance, field, CreateValueForIndexing(value, propertyBoost), indexContext, out _, sourceDocument);
                        ...
                        if (objectValue.HasOwnProperty(SpatialPropertyName) && objectValue.TryGetValue(SpatialPropertyName, out var inner))
                        {
                            ...
                            numberOfCreatedFields = GetRegularFields(instance, field, CreateValueForIndexing(spatial, propertyBoost), indexContext, out _, sourceDocument);
            ...
            int ProcessAsJson(JsValue actualValue, IndexField field, float? propertyBoost)
            {
                var value = TypeConverter.ToBlittableSupportedType(actualValue, flattenArrays: false, forIndexing: true, engine: documentToProcess.Engine,
                    context: indexContext);
                return GetRegularFields(instance, field, CreateValueForIndexing(value, propertyBoost), indexContext, out _, sourceDocument);
            }

...
{
    public sealed class JsLuceneDocumentConverter : JsLuceneDocumentConverterBase
    {
        public JsLuceneDocumentConverter(MapIndex index, bool storeValue = false)
            : base(index, index.Definition.IndexDefinition, storeValue: storeValue)
        {
        }

        public JsLuceneDocumentConverter(MapReduceIndex index, bool storeValue = false)
            : base(index, index.Definition.IndexDefinition, storeValue: storeValue)
        {
        }
    }
        ...
        protected override int GetFields<T>(T instance, LazyStringValue key, LazyStringValue sourceDocumentId, object document, JsonOperationContext indexContext, IWriteOperationBuffer writeBuffer)
        {
            ...
            foreach (var (propertyName, jsPropertyValue) in documentToProcess.GetOwnProperties())
            {
                ...
                using (jsPropertyValue)
                {
                    ...
                    value = TypeConverter.ToBlittableSupportedType(jsPropertyValue, flattenArrays: false, forIndexing: true, engine: documentToProcess.Engine, context: indexContext);
                }

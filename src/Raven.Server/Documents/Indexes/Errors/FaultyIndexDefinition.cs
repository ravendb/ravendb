...
namespace Raven.Server.Documents.Indexes.Errors
{
    public sealed class FaultyIndexDefinition : IndexDefinitionBaseServerSide<IndexField>
    {
        ...
        public FaultyIndexDefinition(string name, IEnumerable<string> collections, IndexLockMode lockMode, IndexPriority priority, IndexState state, IndexField[] mapFields, IndexDefinition definition)
            : base(name, collections, lockMode, priority, state, mapFields, IndexVersion.CurrentVersion, definition.DeploymentMode, definition.ClusterState, definition.ArchivedDataProcessingBehavior.GetValueOrDefault())
        {

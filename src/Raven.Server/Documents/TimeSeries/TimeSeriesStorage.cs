...
namespace Raven.Server.Documents.TimeSeries
{
    public unsafe class TimeSeriesStorage
    {
            ...
            using (var slicer = new TimeSeriesSliceHolder(context, documentId, name))
            {
                ...
                bool TryDeleteRange(out DateTime? next)
                {
                    ...
                    using (var holder = new TimeSeriesSegmentHolder(this, context, documentId, name, collectionName, baseline))
                    {
                        ...
                        if(TryMergeIncrementResults(context, holder))
                            holder.ReloadCurrentSegment();
        ...
        public class TimeSeriesSegmentHolder : IDisposable
        {
            ...
            private void AddValueInternal(DateTime time, Span<double> values, Span<byte> tagSlice, ref TimeSeriesValuesSegment segment, ulong status)
            {
                ...
                if (inRange == false || segment.Append(_context.Allocator, (int)timestampDiff, values, tagSlice, status) == false)
                {
                    ...
                    if (segmentLastTimestamp == BaselineDate)
                    {
            ...
            private bool TryMergeEntries(ref TimeSeriesValuesSegment timeSeriesSegment)
            {
                using (_context.Allocator.Allocate(timeSeriesSegment.NumberOfBytes, out var currentSegmentBuffer))
                {
                    Memory.Copy(currentSegmentBuffer.Ptr, timeSeriesSegment.Ptr, timeSeriesSegment.NumberOfBytes);
                    var readOnlySegment = new TimeSeriesValuesSegment(currentSegmentBuffer.Ptr, timeSeriesSegment.NumberOfBytes);
                    var newSegment = new TimeSeriesValuesSegment(timeSeriesSegment.Ptr, MaxSegmentSize);
                    newSegment.Initialize(timeSeriesSegment.NumberOfValues);
                    ReloadCurrentSegment();

                    double[] values = new double[readOnlySegment.NumberOfValues];
                    ...
                    bool success = newSegment.Append(innerTransactionAllocator, 0,
                        new Span<double>(values),
                        SliceHolder.TagAsSpan(tag),
                        TimeSeriesValuesSegment.Live
                    );

                    if (success == false || newSegment.NumberOfBytes >= readOnlySegment.NumberOfBytes)
                        return false;
        ...
        {
            _sortedAppends ??= new SortedDictionary<TimeSeriesOperation.AppendOperation, TimeSeriesOperation.AppendOperation>(TimeSeriesOperation.AppendOperationComparer.Instance);
            try
            {
                foreach (var operation in toAppend)
                {
                    _sortedAppends[operation] = operation;
                }

                var holder = new SingleResult();

                return AppendTimestamp(context, documentId, collection, name, _sortedAppends.Keys.Select(ToResult), changeVectorFromReplication);
        ...
        {
            // TODO: sort operations according to timestamps & tags
                    ...
                    while (retry)
                    {
                        ...
                        using (var slicer = new TimeSeriesSliceHolder(context, documentId, name, collection).WithBaseline(current.Timestamp))
                        {
                            ...
                            if (EnsureNumberOfValues(segmentHolder.ReadOnlySegment.NumberOfValues, ref current))
                            {
                                ...
                                if (TryAppendToCurrentSegment(context, segmentHolder, appendEnumerator, current, incrementResults , out var newValueFetched))
                                    break;
                            ...
                            retry = SplitSegment(context, segmentHolder, appendEnumerator, current, incrementResults);
                        }
                    }
                }
        ...
        private static bool TryMergeIncrementResults(DocumentsOperationContext context, TimeSeriesSegmentHolder segmentHolder)
        {
            if (segmentHolder.ReadOnlySegment.NumberOfLiveEntries < 2)
                return false;

            Memory.Set(segmentHolder.SliceHolder.SegmentBuffer.Ptr, 0, MaxSegmentSize);
            var newSegment = new TimeSeriesValuesSegment(segmentHolder.SliceHolder.SegmentBuffer.Ptr, MaxSegmentSize);
            newSegment.Initialize(segmentHolder.ReadOnlySegment.NumberOfValues);
            segmentHolder.ReloadCurrentSegment();

            SortedDictionary<LazyStringValue, double[]> valuesByTag = null;
            var currentTime = DateTime.MinValue;
            ByteStringContext innerTransactionAllocator = context.Transaction.InnerTransaction.Allocator;
            double[] values = null;
            ...
            foreach (var segmentValue in segmentHolder.ReadOnlySegment.YieldAllValues(context, segmentHolder.BaselineDate, includeDead: true))
            {
                ...
                {
                    if (valuesByTag.Count > 0 && FlushValues() == false)
                        return false;
                    currentTime = segmentValue.Timestamp;
                }

                if (key == null) // dead entry or append operation with empty tag 
                {
                    var deltaInMs = ((currentTime - segmentHolder.BaselineDate).Ticks / 10_000);
                    bool success;
                    if (segmentValue.Status == TimeSeriesValuesSegment.Dead)
                    {
                        success = newSegment.Append(innerTransactionAllocator, (int)deltaInMs,
                            segmentHolder.ReadOnlySegment.NumberOfValues, 
                            Span<byte>.Empty, 
                            TimeSeriesValuesSegment.Dead
                        );
                    }
                    else
                    {
                        success = newSegment.Append(innerTransactionAllocator, (int)deltaInMs,
                            segmentValue.Values.Span,
                            Span<byte>.Empty,
                            TimeSeriesValuesSegment.Live
                        );
                    }
                ...
                if (valuesByTag.TryGetValue(key, out var existing) == false)
                {
                    if (values == null || values.Length < segmentValue.Values.Length)
                    {
                        values = new double[segmentValue.Values.Length];
                    }
                    else
                    {
                    ...
                    valuesByTag[key] = existing = values;
                }

                Span<double> valuesSpan = segmentValue.Values.Span;
                for (int i = 0; i < segmentValue.Values.Length; i++)
                {
                    existing[i] += valuesSpan[i];
                }
            }
            ...
            if (newSegment.NumberOfBytes >= segmentHolder.ReadOnlySegment.NumberOfBytes)
                return false;
            ...
            {
                var deltaInMs = ((currentTime - segmentHolder.BaselineDate).Ticks / 10_000);
                if (deltaInMs < 0) return false;

                foreach (var valueByTag in valuesByTag)
                {
                    bool success = newSegment.Append(innerTransactionAllocator, (int)deltaInMs,
                        new Span<double>(valueByTag.Value),
                        segmentHolder.SliceHolder.TagAsSpan(valueByTag.Key),
                        TimeSeriesValuesSegment.Live
                    );
            ...
            bool incrementOperation)
        {
            ...
            bool incrementFromReplication = current?.Tag.StartsWith(PositivePrefix) == true || current?.Tag.StartsWith(NegativePrefix) == true;
                ...
                using (context.Allocator.Allocate(readOnlySegment.NumberOfValues * sizeof(TimestampState), out var stateBuffer))
                {
                    ...
                    using (var enumerator = readOnlySegment.GetEnumerator(context.Allocator))
                    {
                        while (enumerator.MoveNext(out var currentTimestamp, currentValues, state, ref currentTag, out var localStatus))
                        {
                            ...
                            while (true)
                            {
                                if (incrementOperation == false && incrementFromReplication == false)
                                {
                                ...
                                {
                                    var compareIncrement = CompareIncrement(currentTime, currentTag, currentValues, current, nextSegmentBaseline, timeSeriesSegment);
                                    if (compareIncrement == CompareResult.Equal)
                                    {
                                        if (timeSeriesSegment.FromReplication)
                                        {
                                            if (current.Tag.StartsWith(PositivePrefix)) // increment operation - take the higher value
                                            {
                                                if (currentValues.SequenceCompareTo(current.Values.Span) >= 0)
                                                {
                                                    timeSeriesSegment.AddExistingValue(currentTime, updatedValues, currentTag.AsSpan(), ref splitSegment, localStatus);
                                                }
                                                else
                                            ...
                                            {
                                                if (currentValues.SequenceCompareTo(current.Values.Span) > 0) // decrement operation - take the lower value
                                                {
                                                    segmentChanged = true;
                                                    timeSeriesSegment.AddNewValue(current, ref splitSegment);
                                                }
                                        ...
                                        Span<double> incrementValues;
                                        var remoteValues = current.Values.Span;

                                        if (remoteValues.Length < currentValues.Length)
                                        {
                                            incrementValues = currentValues;
                                            for (int i = 0; i < remoteValues.Length; i++)
                                            {
                                                incrementValues[i] += remoteValues[i];
                                            }
                                        }
                                        else
                                        {
                                            incrementValues = remoteValues;
                                            for (int i = 0; i < currentValues.Length; i++)
                                            {
                                                incrementValues[i] += currentValues[i];
                                            }
                                        }

                                        segmentChanged = true;
                                        timeSeriesSegment.AddNewValue(currentTime, incrementValues, currentTag.AsSpan(), ref splitSegment, localStatus);
                                    ...
                                    if (compareIncrement == CompareResult.Local)
                                    {
                                        timeSeriesSegment.AddExistingValue(currentTime, updatedValues, currentTag.AsSpan(), ref splitSegment, localStatus);
                                        break;
                                    }
                    ...
                    if (retryAppend && 
                        ...
                        incrementOperation == false && incrementFromReplication == false)
                    {
                        timeSeriesSegment.AddNewValue(current, ref splitSegment);
                        segmentChanged = true;
                        retryAppend = false;
        ...
        private static CompareResult CompareIncrement(DateTime localTime, TimeSeriesValuesSegment.TagPointer localTag, Span<double> localValues, 
            SingleResult remote, DateTime? nextSegmentBaseline, TimeSeriesSegmentHolder holder)
        {
            ...
            if (localTime == remote.Timestamp)
            {
                var compareTags = CompareByTags(localTag, holder.SliceHolder.TagAsSpan(remote.Tag));

                /*if (compareTags == CompareResult.Equal)
                {
                    if(holder.FromReplication == false)
                        return CompareResult.Equal; // if not from replication, remote value is an update

                    if (remote.Tag.StartsWith(PositivePrefix)) // increment operation - take the higher value
                    {
                        if (localValues.SequenceCompareTo(remote.Values.Span) >= 0)
                            return CompareResult.Local;

                        return CompareResult.Remote;
                    }

                    if (localValues.SequenceCompareTo(remote.Values.Span) > 0) // decrement operation - take the lower value
                        return CompareResult.Remote;
        ...
        private static bool OverwriteExistingSegment(DocumentsOperationContext context, TimeSeriesSegmentHolder segmentHolder, SingleResult current)
        {
            var originalBaseline = segmentHolder.BaselineDate;
            var additionalValueSize = Math.Max(0, current.Values.Length - segmentHolder.ReadOnlySegment.NumberOfValues);
            var newNumberOfValues = additionalValueSize + segmentHolder.ReadOnlySegment.NumberOfValues;

            using (context.Allocator.Allocate(segmentHolder.ReadOnlySegment.NumberOfBytes, out var currentSegmentBuffer))
            {
                Memory.Copy(currentSegmentBuffer.Ptr, segmentHolder.ReadOnlySegment.Ptr, segmentHolder.ReadOnlySegment.NumberOfBytes);
                var readOnlySegment = new TimeSeriesValuesSegment(currentSegmentBuffer.Ptr, segmentHolder.ReadOnlySegment.NumberOfBytes);
                var newSegment = new TimeSeriesValuesSegment(segmentHolder.SliceHolder.SegmentBuffer.Ptr, MaxSegmentSize);
                newSegment.Initialize(newNumberOfValues);

                using (context.Allocator.Allocate((newNumberOfValues) * sizeof(double), out var valuesBuffer))
                using (context.Allocator.Allocate(newNumberOfValues * sizeof(TimestampState), out var stateBuffer))
                {
                    Memory.Set(valuesBuffer.Ptr, 0, valuesBuffer.Length);
                    Memory.Set(stateBuffer.Ptr, 0, stateBuffer.Length);

                    Span<double> localValues = current.Values.Span;
                    var localState = new Span<TimestampState>(stateBuffer.Ptr, newNumberOfValues);
                    var localTag = new TimeSeriesValuesSegment.TagPointer();
                    bool isAppend = false;
                    ...
                    using (var enumerator = readOnlySegment.GetEnumerator(context.Allocator))
                    {
                        ...
                        {
                            var currentLocalTime = originalBaseline.AddMilliseconds(localTimestamp);
                            var deltaFromStart = ((currentLocalTime - originalBaseline).Ticks / 10_000);

                            if (currentLocalTime != current.Timestamp)
                            {
                                newSegment.Append(context.Allocator, (int)deltaFromStart, localValues, localTag.AsSpan(), localStatus);
                                continue;
                            }
                            ...
                            if (newSegment.Append(context.Allocator, (int)deltaFromStart, current.Values.Span, 
                                segmentHolder.SliceHolder.TagAsSpan(current.Tag), current.Status) == false)
                                return false;
        ...
        private SortedDictionary<TimeSeriesOperation.AppendOperation, TimeSeriesOperation.AppendOperation> _sortedAppends;

...
namespace Raven.Server.Documents.Revisions
{
    public class RevisionsStorage
    {
            ...
            RevisionsCollectionConfiguration configuration, long revisionsCount, NonPersistentDocumentFlags nonPersistentFlags, string changeVector, long lastModifiedTicks, out long currentRevisionsCount, bool deletedDoc = false, bool skipForceCreated = false)
        {
            ...
            else if (configuration.MinimumRevisionsToKeep.HasValue) // obey the configuration.MinimumRevisionsToKeep (and the configuration.MaximumRevisionsToDeleteUponDocumentUpdate)
            {
            ...
            List<(string ChangeVector, long Etag, DocumentFlags Flags, string CollectionName)> revisionsToRemove)
        {
            ...
            foreach (var revisionInfo in revisionsToRemove)
            {
                using (Slice.From(context.Allocator, revisionInfo.ChangeVector, out var keySlice))
                {
                    ...
                    if (revisionInfo.Flags.Contain(DocumentFlags.HasAttachments))
                    {

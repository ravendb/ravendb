...
{
    //   HashComposite   = [lowerDocId][RS][24-byte prefixed hash]
    //   RawComposite    = [lowerDocId][RS][rawCv             ]
    //   RevisionVersion = [rawCv]
    //   DocIdSlice      = [lowerDocId]
    ...
    internal sealed unsafe class RevisionTombstoneKeyScope : IDisposable
    {
        ...
        private void BuildFromExternalComposite(DocumentsOperationContext context, LazyStringValue compositeKey, string collection)
        {
            ...
            if (_trailingSize == RevisionsStorage.RevisionKeySize)
        ...
        // Hash the trailing into a RevisionKey; `prefixedHash` is also embedded in the composite by BuildHashedKey.
        private RevisionKey BuildRevisionKeyFromTrailing(out Slice prefixedHash)
        {
            Register(RevisionsStorage.GetRevisionKeyHashSlice(
                _allocator, _trailingSlice, out prefixedHash));
            return new RevisionKey(prefixedHash, _trailingSlice);
        }

        // [docId][RS][prefixedHash] key (LIVE + RECEIVE). Embeds the full 24B prefixed hash; the byte-skip
        // prefix encoding keeps it free of 0x1E, so it is safe inside the composite cv slot.
        private void BuildHashedKey()
        {
            RevisionKey revisionKey = BuildRevisionKeyFromTrailing(out Slice prefixedHash);
            Register(BuildCompositeSlice(prefixedHash, out Slice hashComposite));
        ...
        private void BuildEmitKey(Tombstone tombstone)
        {
            ...
            else if (_trailingSize == RevisionsStorage.RevisionKeySize)
            {
                throw new InvalidOperationException(
    ...
    public partial class RevisionsStorage
    {
        ...
        internal bool TryReadRevisionTombstone(Table table, in RevisionTombstoneKey tombstoneKey, out TableValueReader tvr)
            => DualForm.TryRead(table, tombstoneKey.HashComposite, tombstoneKey.RawComposite, out tvr);

        internal bool VerifyRevisionTombstoneExists(Table table, in RevisionTombstoneKey tombstoneKey)
            => DualForm.Exists(table, tombstoneKey.HashComposite, tombstoneKey.RawComposite);
        ...
        internal void DeleteRevisionTombstoneByKey(Table table, in RevisionTombstoneKey tombstoneKey)
            => DualForm.Delete(table, tombstoneKey.HashComposite, tombstoneKey.RawComposite);

...
namespace Raven.Server.Smuggler.Migration
{
    public sealed class Migrator
    {
        ...
        public Migrator(MigrationConfigurationBase configuration, ServerStore serverStore)
        {
            ...
            var httpClientHandler = DefaultRavenHttpClientFactory.CreateHttpMessageHandler(_serverStore.Server.Certificate.ClientCertificate, setSslProtocols: false, useHttpDecompression: false, hasExplicitlySetDecompressionUsage: false, DocumentConventions.DefaultForServer.HttpPooledConnectionLifetime, DocumentConventions.DefaultForServer.HttpPooledConnectionIdleTimeout);
        ...
        public async Task MigrateDatabases(List<DatabaseMigrationSettings> databases, AuthorizationStatus authorizationStatus)
        {
            ...
            if (databases == null || databases.Count == 0)
            {
                ...
                if (_buildMajorVersion != MajorVersion.V4 && _buildMajorVersion != MajorVersion.V5 && _buildMajorVersion != MajorVersion.V6 && _buildMajorVersion != MajorVersion.V7 && _buildMajorVersion != MajorVersion.V8 && _buildMajorVersion != MajorVersion.GreaterThanCurrent)
                {

            ...
            using (step.DocumentsStorage.ContextPool.AllocateOperationContext(out DocumentsOperationContext context))
            {
                ...
                if (readTable != null)
                {
                    ...
                    foreach (var read in readTable.SeekByPrimaryKey(Slices.BeforeAllKeys, 0))
                    {
                        ...
                        {
                            var collection = TableValueToString(context, (int)CollectionsTable.Name, readCopy);
                            ...
                            using (writeTable.Allocate(out TableValueBuilder write))
                            {
                                ...
                                var pk = readCopy.Read((int)CollectionsTable.Name, out int size);
            ...
            using (step.DocumentsStorage.ContextPool.AllocateOperationContext(out DocumentsOperationContext context))
            {
                foreach (var collection in step.DocumentsStorage.GetTombstoneCollections(step.ReadTx))
                {
                    ...
                    foreach (var read in readTable.SeekForwardFrom(Raven.Server.Documents.Schemas.Tombstones.CollectionEtagsIndex, 0, 0))
                    {
                        ...
                        using (TableValueReaderUtil.CloneTableValueReader(context, ref readCopy))
                        {
                            var type = *(Tombstone.TombstoneType*)readCopy.Read((int)TombstoneTable.Type, out _);
                            var oldCollection = TableValueToString(context, (int)TombstoneTable.Collection, read);
                            using (DocumentIdWorker.GetStringPreserveCase(context, oldCollection, out Slice collectionSlice))
                            using (writeTable.Allocate(out TableValueBuilder write))
                            {
                                write.Add(readCopy.Read((int)TombstoneTable.LowerId, out int size), size);
                                write.Add(readCopy.Read((int)TombstoneTable.Etag, out size), size);
                                write.Add(readCopy.Read((int)TombstoneTable.DeletedEtag, out size), size);
                                write.Add(readCopy.Read((int)TombstoneTable.TransactionMarker, out size), size);
                                write.Add(readCopy.Read((int)TombstoneTable.Type, out size), size);
                                if (type == Tombstone.TombstoneType.Attachment)
                                {
                                    write.Add(readCopy.Read((int)TombstoneTable.Collection, out size), size);
                                }
                                else
                                {
                                    write.Add(collectionSlice);
                                }
                                write.Add(readCopy.Read((int)TombstoneTable.Flags, out size), size);
                                write.Add(readCopy.Read((int)TombstoneTable.ChangeVector, out size), size);
                                write.Add(readCopy.Read((int)TombstoneTable.LastModified, out size), size);
            ...
            using (step.DocumentsStorage.ContextPool.AllocateOperationContext(out DocumentsOperationContext context))
            {
                ...
                if (readTable != null)
                {
                    ...
                    foreach (var read in readTable.SeekByPrimaryKey(Slices.BeforeAllKeys, 0))
                    {
                        ...
                        using (TableValueReaderUtil.CloneTableValueReader(context, ref readCopy))
                        {
                            var oldCollection = TableValueToString(context, (int)ConflictsTable.Collection, read);
                            using (DocumentIdWorker.GetStringPreserveCase(context, oldCollection, out Slice collectionSlice))
                            using (writeTable.Allocate(out TableValueBuilder write))
                            {
                                write.Add(readCopy.Read((int)ConflictsTable.LowerId, out int size), size);
                                write.Add(readCopy.Read((int)ConflictsTable.RecordSeparator, out size), size);
                                write.Add(readCopy.Read((int)ConflictsTable.ChangeVector, out size), size);
                                write.Add(readCopy.Read((int)ConflictsTable.Id, out size), size);
                                write.Add(readCopy.Read((int)ConflictsTable.Data, out size), size);
                                write.Add(readCopy.Read((int)ConflictsTable.Etag, out size), size);
                                write.Add(collectionSlice);
                                write.Add(readCopy.Read((int)ConflictsTable.LastModified, out size), size);
                                write.Add(readCopy.Read((int)ConflictsTable.Flags, out size), size);
                                writeTable.Set(write);
                            }
                        }
                    }

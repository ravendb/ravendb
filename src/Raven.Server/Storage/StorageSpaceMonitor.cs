            ...
            while (current != null)
            {
                ...
                try
                {
                    foreach (var item in database.GetAllStoragesEnvironment())
                    {
                        ...
                        var dataDisk = DiskHelper.GetDiskSpaceInfo(options.BasePath.FullPath, driveInfo?.BasePath);

                        AddEnvironmentIfLowSpace(dataDisk);

                        if (options.JournalPath != null)
                        {
                            var journalDisk = DiskHelper.GetDiskSpaceInfo(options.JournalPath.FullPath, driveInfo?.JournalPath);
                        ...
                        {
                            var tempDisk = DiskHelper.GetDiskSpaceInfo(options.TempPath.FullPath, driveInfo?.TempPath);

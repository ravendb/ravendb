...
namespace Raven.Server.Monitoring
{
    public sealed class GcThreadContentionDetector : IDisposable
    {
        ...
        private readonly RavenLogger _logger = RavenLogManager.Instance.GetLoggerForServer<GcThreadContentionDetector>();
        ...
        private void CheckGcThreadContention(object state)
        {
            ...
            catch (Exception e)
            {
                if (_logger.IsWarnEnabled)
                    _logger.Warn("Failed to check if there might be GC threads contention", e);
            }
            finally
            {
                _timer?.Dispose();
        ...
        private void RaiseAlert(int totalCores, int utilizedCores)
        {
            ...
            var alert = AlertRaised.Create(
                ...
                details.Message,
                AlertReason.GcThreadContention,
            ...
            if (_logger.IsWarnEnabled)
            {
                _logger.Warn($"GC thread contention detected: {totalCores} total cores but only {utilizedCores} utilized core(s). " +
        ...
        {
            _notificationCenter.Dismiss(AlertRaised.GetKey(AlertReason.GcThreadContention, null));
        }

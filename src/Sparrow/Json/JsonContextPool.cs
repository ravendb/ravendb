...
namespace Sparrow.Json
{
    public sealed class JsonContextPool : JsonContextPoolBase<JsonOperationContext>
    {
        ...
        public JsonContextPool(Size? maxContextSizeToKeep, IRavenLogger logger)
            : this(maxContextSizeToKeep, PlatformDetails.Is32Bits == false ? 8 * 1024 : 2 * 1024, logger)
        {
        }

        internal JsonContextPool(Size? maxContextSizeToKeep, int maxNumberOfAllocatedStringValuesPerContext, IRavenLogger logger)
            : base(maxContextSizeToKeep, logger)
        {

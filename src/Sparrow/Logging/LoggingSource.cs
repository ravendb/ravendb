...
namespace Sparrow.Logging
{
    public sealed class LoggingSource
    {
        ...
        private readonly SingleProducerSingleConsumerCircularQueue<WebSocketMessageEntry>[] _pooledMessageEntries;
        private readonly SingleProducerSingleConsumerCircularQueue<WebSocketMessageEntry>[] _activeMessages;
        ...
        public LoggingSource(LogMode logMode, string path, string name, TimeSpan retentionTime, long retentionSize, bool compress = false)
        {
            ...
            _pooledMessageEntries = new SingleProducerSingleConsumerCircularQueue<WebSocketMessageEntry>[Environment.ProcessorCount];
            _activeMessages = new SingleProducerSingleConsumerCircularQueue<WebSocketMessageEntry>[Environment.ProcessorCount];
            for (int i = 0; i < _pooledMessageEntries.Length; i++)
            {
                _pooledMessageEntries[i] = new SingleProducerSingleConsumerCircularQueue<WebSocketMessageEntry>(1024);
            }
            for (int i = 0; i < _activeMessages.Length; i++)
            {
                _activeMessages[i] = new SingleProducerSingleConsumerCircularQueue<WebSocketMessageEntry>(1024);
            }
        ...
        internal class JsonContextPoolHelpers //TODO: Merge this with https://github.com/ravendb/ravendb/pull/10179
        {
            internal const string LIBC_6 = "libc";

            [DllImport(LIBC_6, EntryPoint = "sched_getcpu", CallingConvention = CallingConvention.Cdecl, SetLastError = false)]
            private static extern int sched_getcpu();

            [DllImport("kernel32.dll", SetLastError = true)]
            private static extern int GetCurrentProcessorNumber();

            public static int CurrentProcessNumber
            {
                get
                {
                    if (PlatformDetails.RunningOnPosix)
                    {
                        int currentProcessNumber = sched_getcpu();
        ...
        public void Log(ref LogEntry entry, TaskCompletionSource<object> tcs = null)
        {
            ...
            int currentProcessNumber = JsonContextPoolHelpers.CurrentProcessNumber;
            var pool = _pooledMessageEntries[currentProcessNumber% _pooledMessageEntries.Length];
            ...
            _activeMessages[currentProcessNumber%_activeMessages.Length].Enqueue(item, timeout: 128);
                    ...
                    try
                    {
                        ...
                        using (currentFile)
                        {
                            ...
                            while (sizeWritten < maxFileSize)
                            {
                                ...
                                foundEntry = false;
                                for (var index = 0; index < _activeMessages.Length; index++)
                                {
                                    var messages = _activeMessages[index];
                                    for (var limit = 0; limit < 16; limit++)
                                    {
                                        ...
                                        _pooledMessageEntries[index].Enqueue(item);
                                    }
                                }
                            }
                        }

...
public static partial class StringUtils
{
    ...
    // RavenDB-25738 Control characters are invalid for new databases
    ...
    {
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static unsafe int FindMaxEscapePositionAndControlCharSize(byte* str, int size, out int escapedCount)
        {
            var count = 0;
            var controlCount = 0;
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static unsafe void FindEscapedPositionsAndEscapeControls(FastList<int> buffer, byte* str, ref int len, int previousComputedMaxSize)
        {
            var originalLen = len;
            buffer.Clear();
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static unsafe void FindEscapedPositionsAndEscapeControlsForBackwardCompatibility(this JsonParserState state, byte* str, ref int len, int previousComputedMaxSize)
        => Compatibility.FindEscapedPositionsAndEscapeControls(state.EscapePositions, str, ref len, previousComputedMaxSize);

    public static unsafe LazyStringValue GetLazyStringForBackwardCompatibility(this JsonOperationContext context, byte* ptr, int size, bool longLived = false)
    {
        var state = new JsonParserState();
        var maxByteCount = Encodings.Utf8.GetMaxByteCount(size);

        int escapePositionsSize = Compatibility.FindMaxEscapePositionAndControlCharSize(ptr, size, out _);

        int memorySize = maxByteCount + escapePositionsSize;
        var memory = longLived ? context.GetLongLivedMemory(memorySize) : context.GetMemory(memorySize);
        ...
        LazyStringValue result = longLived == false ? context.AllocateStringValue(null, address, size) : new LazyStringValue(null, address, size, context);

...
///
/// A routing pointer (like the legacy widget-index), here for the same reason: the
/// public <c>/embed/{token}</c> URL is unauthenticated and the bridge's default
...
/// bound parameters). Written at mint and <b>left in place on revoke</b> — the embed
/// path must still resolve the (now-revoked) <see cref="EmbedLink"/> to return
/// 410 Gone, so the pointer survives until both self-expire via <c>@expires</c>.
/// </summary>

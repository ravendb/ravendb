 ...
 *  learns the origin check rather than trusting any parent-frame message. `onReady` is the stack's own
 *  line: every re-minted document boots with the operator's default appearance, so a host that drives it
 *  by prop has to resend the current value there. */
function envelopeHandlerLines(onReady: string) {
    return [
        ...
        "        break;",
        '    case "expired":',
        "        void openSession(); // mint a fresh link and keep chatting",
        "        break;",
        '    case "error":',
...
const APPEARANCE_MESSAGE_LINES = [
...
export function buildHtmlHostSnippet(embedOrigin: string) {
    return [
        ...
        ...indent([
            ...
            ...indent(envelopeHandlerLines("// the widget has mounted — hide your own loader here")),
            "});",
            "",
            ...
            ...indent(APPEARANCE_MESSAGE_LINES, 2),
            "    );",
            "}",
            "",
            "void openSession();",
...
export function buildReactHostSnippet(embedOrigin: string) {
    return [
        ...
        `const widgetOrigin = "${embedOrigin}";`,
        "",
        "function postAppearance(frame, appearance) {",
        ...
        "        postAppearance(frameRef.current, appearance);",
        "    }, [appearance]);",
        "",
        "    useEffect(() => {",
        ...indent(OPEN_SESSION_COMMENT, 2),
        "        async function openSession() {",
        '            const response = await fetch("/api/quill-session", { method: "POST" });',
        "            if (!response.ok) throw new Error(`Could not start the assistant: ${response.status}`);",
        "",
        ...
        ...indent(envelopeHandlerLines("postAppearance(frameRef.current, appearanceRef.current);"), 3),
        "        };",
        "",
...
export function buildVueHostSnippet(embedOrigin: string) {
    return [
        ...
        "}",
        "",
        "function postAppearance(appearance) {",
        ...
        ...indent(envelopeHandlerLines("postAppearance(props.appearance);")),
        "}",
        "",
        "onMounted(() => {",
        '    window.addEventListener("message", onMessage);',
        ...
        "",
        "watch(() => props.appearance, postAppearance);",
        "</script>",
    ].join("\n");
}
...
export function buildKotlinHostSnippet(embedOrigin: string) {
    return [
        ...
        "    }",
        "",
        ...indent(MOBILE_MINT_COMMENT),
        "    private fun fetchSessionUrl(): String {",
        '        val connection = URL("https://your.site/api/quill-session").openConnection() as HttpURLConnection',

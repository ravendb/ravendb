...
export function buildReactHostSnippet(embedOrigin: string) {
    return [
        ...
        "",
        "export function QuillAssistant() {",
        ...
        "    }",
        "",
        '    return <iframe ref={frameRef} title="Assistant" style={{ width: "100%", height: 600, border: 0 }} />;',
        "}",
    ].join("\n");
}
...
export function buildVueHostSnippet(embedOrigin: string) {
    return [
        "<template>",
        '    <iframe ref="frame" title="Assistant" style="width: 100%; height: 600px; border: 0"></iframe>',
        "</template>",
        "",
        "<script setup>",
...
export function buildKotlinHostSnippet(embedOrigin: string) {
    return [
        "class QuillAssistant(private val webView: WebView) {",
        "",
        "    init {",
...
export function buildSwiftHostSnippet(embedOrigin: string) {
    return [
        ...
        "    override init() {",
        "        super.init()",
        '        webView.configuration.userContentController.add(self, name: "quill")',
        "        openSession()",
        "    }",
        "",

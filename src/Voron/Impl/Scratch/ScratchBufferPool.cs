...
namespace Voron.Impl.Scratch
{
    ...
    public unsafe class ScratchBufferPool : ILowMemoryHandler, IDisposable
    {
        ...
        private readonly IScratchSpaceUsageMonitor _scratchSpaceUsageMonitor; // it tracks size of active and recycled scratches
        ...
        public ScratchBufferPool(StorageEnvironment env)
        {
            _disposeOnceRunner = new DisposeOnce<ExceptionRetry>(() =>
            {
                ...
                foreach (var scratch in _scratchBuffers)
                {
                    ...
                    _scratchSpaceUsageMonitor?.Decrease(scratch.Value.File.NumberOfAllocatedPages * Constants.Storage.PageSize);
                }
                ...
                while (_recycleArea.First != null)
                {
                    ...
                    if (recycledScratch.File.IsDisposed == false)
                    {
                        ...
                        _scratchSpaceUsageMonitor?.Decrease(recycledScratch.File.NumberOfAllocatedPages * Constants.Storage.PageSize);
                    }
            ...
            _scratchSpaceUsageMonitor = env.Options.ScratchSpaceUsageMonitor;
        ...
        private ScratchBufferItem NextFile(long minSize, long? requestedSize)
        {
            ...
            while (current != null)
            {
                    ...
                    recycled.File.HasActivelyUsedBytes(oldestTx) == false)
                {
                    ...
                    _scratchSpaceUsageMonitor?.Increase(recycled.File.NumberOfAllocatedPages * Constants.Storage.PageSize);
            ...
            _scratchSpaceUsageMonitor?.Increase(item.File.NumberOfAllocatedPages * Constants.Storage.PageSize);
        ...
        public PageFromScratchBuffer Allocate(LowLevelTransaction tx, int numberOfPages)
        {
            ...
            if (current.File.Size < _options.MaxScratchBufferSize)
            {
                ...
                if (_scratchSpaceUsageMonitor != null)
                    numberOfPagesBeforeAllocate = current.File.NumberOfAllocatedPages;

                var page = current.File.Allocate(tx, numberOfPages, size);

                if (numberOfPagesBeforeAllocate != -1)
                    _scratchSpaceUsageMonitor?.Increase((current.File.NumberOfAllocatedPages - numberOfPagesBeforeAllocate) * Constants.Storage.PageSize);
        ...
        private void RemoveInactiveScratches(ScratchBufferItem except)
        {
            foreach (var item in _scratchBuffers)
            {
                ...
                if (_recycleArea.Contains(scratchBufferItem) == false)
                {
                    ...
                    _scratchSpaceUsageMonitor?.Decrease(scratchBufferItem.File.NumberOfAllocatedPages * Constants.Storage.PageSize);
                }
            }
        }

                ...
                using (var txw = compactedEnv.WriteTransaction(context))
                {
                    ...
                    if (schema.Key == null || schema.Key.IsGlobal) 
                    {
                        ...
                        if (variableSizeIndex != null)
                        {
                            ...
                            var currentKey = lastSlice;                                 // a resumed run continues the previous batch's key
                            var committedUnderCurrentKey = committedUnderLastSlice;      // ... including the rows already committed for it

...
namespace Voron
{
    public abstract class StorageEnvironmentOptions : IDisposable
    {
        ...
        public class DirectoryStorageEnvironmentOptions : StorageEnvironmentOptions
        {
            ...
            private AbstractPager GetMemoryMapPagerWithPossibleFileRename(string name, long initialSize, Func<StorageEnvironmentOptions, long?, VoronPathSetting, bool, bool, AbstractPager> getMemoryMapPagerFunc)
            {
                ...
                for (int i = 0; i < 15; i++)
                {
                    ...
                    try
                    {
                        // note : GetMemoryMapPagerWithPossibleFileRename calls with deleteOnClose = true
                        return getMemoryMapPagerFunc(this, initialSize, tempFile, true, false);
                    }
            ...
            {
                return GetMemoryMapPagerWithPossibleFileRename(name, initialSize, GetMemoryMapPager);
            }
            ...
            {
                var pager = GetMemoryMapPagerWithPossibleFileRename(name, initialSize, GetMemoryMapPagerInternal);

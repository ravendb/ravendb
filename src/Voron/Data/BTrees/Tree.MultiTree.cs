    ...
    public unsafe partial class Tree
    {
        ...
        public void MultiAdd(Slice key, Slice value)
        {
            ...
            if (item->Flags == TreeNodeFlags.MultiValuePageRef)
            {
                ...
                var lastSearchPosition = page.LastSearchPosition;
                page = ModifyPage(page);
                item = page.GetNode(lastSearchPosition);
                existingTree = OpenMultiValueTree(key, item);
                existingTree.DirectAdd(value, 0, out _).Dispose();
                return;
            }
            ...
            var lastNestedPageSearchPosition = nestedPage.LastSearchPosition;
            ...
            if (nestedPage.LastMatch != 0)
            {
                ...
                var lastPageSearchPosition = page.LastSearchPosition;
                page = ModifyPage(page);
                item = page.GetNode(lastPageSearchPosition);
            ...
            if (existingItem != null)
            {
                ...
                nestedPage.RemoveNode(lastNestedPageSearchPosition);
            }

            if (nestedPage.HasSpaceFor(_llt, value, 0))
            {
                // we are now working on top of the modified root page, we can just modify the memory directly
                nestedPage.AddDataNode(lastNestedPageSearchPosition, value, 0);
                return;
            }

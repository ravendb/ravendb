...
namespace Voron.Data.BTrees
{
    public unsafe partial class Tree : IDisposable
    {
        ...
        private TreePage SearchForPage(Slice key, out TreeNodeHeader* node)
        {
            ...
            if (p.IsLeaf == false)
                throw new VoronUnrecoverableErrorException(_llt.Environment, "Index points to a non leaf page " + p.PageNumber);
        ...
        private TreePage SearchForPage(Slice key, out Func<Slice, TreeCursor> cursorConstructor, out TreeNodeHeader* node)
        {
            ...
            if (p.IsLeaf == false)
                throw new VoronUnrecoverableErrorException(_llt.Environment, "Index points to a non leaf page");

...
namespace Voron.Data.BTrees
{
    public unsafe partial class Tree
    {
        ...
        private bool TryAddInlineStream(Slice key, Stream stream, Slice? tag, out int preReadBytes, out byte* preReadBuffer)
        {
            ...
            var maxInlineSize = Constants.Tree.NodeMaxSize - Constants.Tree.NodeHeaderSize - InlineStreamHeader.SizeOf - tagSize;
            if (maxInlineSize <= 0)
                return false;

            var nodeMaxSize = Constants.Tree.NodeMaxSize;
        ...
        public bool IsInlineStream(Slice key, out byte* data, out int dataSize, out TreePage treePage)
        {
            ...
            if ((ReadHeader().Flags & TreeFlags.Streams) == 0)
                return false;
        ...
        public (int Version, long Size) DeleteStream(Slice key)
        {
            ...
            if (IsInlineStream(key, out var inlineData, out _, out _))
            {
                var inlineHeader = (InlineStreamHeader*)inlineData;
                version = inlineHeader->Info.Version;
                size = inlineHeader->Info.TotalSize;

        ...
        private async Task<TResult> WaitForResult<TResult>(RavenCommand<TResult> command, Dictionary<Task, (RavenCommand<TResult> Command, int Index)> tasks, CancellationTokenSource broadcastCts)
        {
            while (tasks.Count > 0)
            {
                ...
                if (completed.IsCanceled || completed.IsFaulted)
                {
                    ...
                    command.FailedNodes[node] = completed.Exception.ExtractSingleInnerException();
...
#pragma warning restore 4014
        ...
        private class ServerCertificateCustomValidationCallbackTranslator
        {
        ...
        public static event Func<HttpRequestMessage, X509Certificate2, X509Chain, SslPolicyErrors, bool> ServerCertificateCustomValidationCallback
        {
            add
            {
                lock (_locker)
                {
                    var callbackTranslator = new ServerCertificateCustomValidationCallbackTranslator
                    {
                        Callback = value
                    };
            ...
            remove
            {
                lock (_locker)
                {
                    ...
                    {
                        if (callback.Target is ServerCertificateCustomValidationCallbackTranslator ct && ct.Callback == value)
                        {

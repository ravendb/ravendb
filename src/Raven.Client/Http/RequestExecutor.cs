...
namespace Raven.Client.Http
{
    public class RequestExecutor : IDisposable
    {
        ...
        public string Url => _nodeSelector?.GetCurrentNode().Item2?.Url;
        ...
        protected virtual async Task UpdateClientConfigurationAsync(int? sessionId)
        {
            if (_disposed)
                return;
            ...
            try
            {
                ...
                using (ContextPool.AllocateOperationContext(out JsonOperationContext context))
                {
                    ...
                    var (currentIndex, currentNode) = ChooseNodeForRequest(sessionId);
        ...
        public virtual async Task<bool> UpdateTopologyAsync(ServerNode node, int? nodeIndex, int timeout)
        {
            if (_disposed)
                return false;
        ...
        public Task ExecuteAsync<TResult>(
            ...
            int? sessionId = null)
        {
            if (command.GetType().Name.Contains("Query"))
            {
                Debugger.Break();
            }
        ...
        public (int, ServerNode) ChooseNodeForRequest(int? sessionId)
        {
        ...
        private void UpdateTopologyCallback(object _)
        {
            ...
            var (serverIndex, serverNode) = _nodeSelector.GetCurrentNode();
            GC.KeepAlive(Task.Run(async () =>
            {
                try
                {
                    await UpdateTopologyAsync(serverNode, serverIndex, 0).ConfigureAwait(false);
                }
                catch (Exception e)
                {
                ...
                finally
                {
                    ...
                    if (refreshTopology || refreshClientConfiguration)
                    {
                        ...
                        tasks[1] = refreshClientConfiguration
                            ? UpdateClientConfigurationAsync(sessionId ?? 0)
                            : Task.CompletedTask;
        ...
        private async Task<bool> HandleUnsuccessfulResponse<TResult>(ServerNode chosenNode, int? nodeIndex, JsonOperationContext context, RavenCommand<TResult> command, HttpRequestMessage request, HttpResponseMessage response, string url, int? sessionId)
        {
            switch (response.StatusCode)
            {
                ...
                case HttpStatusCode.InternalServerError:
                    ...
                    //try again, maybe it's transient error?
                    if (command.DidRetryAfterInternalServerError == false)
                    {
                        command.DidRetryAfterInternalServerError = true;
                        await ExecuteAsync(chosenNode, nodeIndex, context, command).ConfigureAwait(false);
                    }
                    else
                    {

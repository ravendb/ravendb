...
{
    public abstract class AbstractSubscriptionWorker<T> : IAsyncDisposable, IDisposable
    {
        ...
        internal CancellationTokenSource _processingCts = new CancellationTokenSource();
        internal readonly SubscriptionWorkerOptions _options;
        internal (Func<SubscriptionBatch<T>, Task> Async, Action<SubscriptionBatch<T>> Sync) _subscriber;
        internal TcpClient _tcpClient;
        internal bool _disposed;
        internal Task _subscriptionTask;
        ...
        internal ServerNode _redirectNode;
        internal RequestExecutor _subscriptionLocalRequestExecutor;
        ...
        internal abstract SubscriptionBatch<T> GetSubscriptionBatch();
        internal abstract Task HandleBatchConfirm();
        internal abstract Task HandleSubscriptionError(Exception e);
        ...
        private async Task<TcpConnectionHeaderMessage.SupportedFeatures> NegotiateProtocolVersionForSubscriptionAsync(string chosenUrl, TcpConnectionInfo tcpInfo,
            Stream stream, JsonOperationContext context, List<string> _)
        {
        ...
        private async ValueTask<int> ReadServerResponseAndGetVersionAsync(JsonOperationContext context, AsyncBlittableJsonTextWriter writer, Stream stream,
        ...
        internal async Task ProcessSubscriptionAsync()
        {
            try
            {
                ...
                using (var tcpStream = await ConnectToServer(_processingCts.Token).ConfigureAwait(false))
                {
                    ...
                    var batch = GetSubscriptionBatch();
                    ...
                    while (_processingCts.IsCancellationRequested == false)
                    {
                        ...
                        //      notifiedSubscriber = Task.Run(async () => await HandleAndAckBatch(batch, context, incomingBatch.ReturnContext, tcpStreamCopy, lastReceivedChangeVector).ConfigureAwait(false));
                ...
                while (endOfBatch == false && _processingCts.IsCancellationRequested == false)
                {
                    ...
                    switch (receivedMessage.Type)
                    {
                        ...
                        case SubscriptionConnectionServerMessage.MessageType.Confirm:
                            ...
                            incomingBatch.Clear();
                            batch.Items.Clear();
                            await HandleBatchConfirm().ConfigureAwait(false);
                            break;
        ...
        private bool ShouldTryToReconnect(Exception ex)
        {
            ...
            switch (ex)
            {
                ...
                case SubscriptionChangeVectorUpdateConcurrencyException e:
                    HandleSubscriptionError(e).Wait(_processingCts.Token);
                    return true;
        ...
        private void CloseTcpClient(bool disposeReqExecutor = true)
        {
            if (_stream != null)
            {
                try

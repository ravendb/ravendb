...
namespace Raven.Client.Documents.Operations
{
    ...
    {
        public Operation Send(IOperation<OperationIdResult> operation, int? sessionId = null, bool isServerOperation = false)
        {
            return AsyncHelpers.RunSync(() => SendAsync(operation, default(CancellationToken), sessionId, isServerOperation));
        }

        public async Task<Operation> SendAsync(IOperation<OperationIdResult> operation, CancellationToken token = default(CancellationToken), int? sessionId = null, bool isServerOperation = false)
        {

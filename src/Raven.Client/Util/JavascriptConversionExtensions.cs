...
namespace Raven.Client.Util
{
    internal sealed class JavascriptConversionExtensions
    {
        ...
        internal sealed class EnumConversionExtension : JavascriptConversionExtension
        {
            ...
            public EnumConversionExtension(bool saveEnumsAsIntegers)
            {
                opts = saveEnumsAsIntegers ? EnumOptions.UseNumbers : EnumOptions.UseStrings;
            }
            ...
            {
                var isDictionaryKey = writer.ToString().EndsWith("[");

                if ((this.opts & EnumOptions.UseStrings) != 0 || isDictionaryKey)
                {
                    if (inStringAlready)
                    {
                        if (!isDictionaryKey && (this.opts & EnumOptions.UseNumbers) != 0)

...
public partial class IndexSearcher
{
        ...
        where TInner : IQueryMatch
    {
        if (typeof(TValueType) == typeof(string))
        {
            ...
            return BuildUnaryMatch<TInner, Slice>(in set, field, slice, @operation, take, token);
        }
        ...
        where TInner : IQueryMatch
    {
        if (typeof(TValueType) == typeof(string))
        {
            ...
            return BuildBetween<TInner, Slice>(in set, field, leftValueSlice, rightValueSlice, UnaryMatchOperation.GreaterThanOrEqual, UnaryMatchOperation.LessThanOrEqual, isNegated, take, token);
        }
        else if (typeof(TValueType) == typeof(Slice))
        {
        ...
        where TInner : IQueryMatch
    {
        if (typeof(TValueType) == typeof(string))
        {
            ...
            return BuildBetween<TInner, Slice>(in set, field, leftValueSlice, rightValueSlice, leftSide, rightSide, isNegated, take, token);
        }
        else if (typeof(TValueType) == typeof(Slice))
        {
            ApplyAnalyzer(field, field.Analyzer,(Slice)(object)leftValue, out var leftValueSlice);

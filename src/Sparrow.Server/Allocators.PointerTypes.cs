...
namespace Sparrow.Server
{
    ...
    public unsafe struct Pointer<T> : Meta.IDescribe, IPointerType<T> where T : struct
    {
        ...
        static Pointer()
        {
            ...
            // We apply the NoOptimization attribute because we need to ensure the JIT wont optimize away
            // the call to Marshal.SizeOf<T> for correctness either now or in the future. 
            Marshal.SizeOf<T>();
        }
        ...
        {
            return new ReadOnlySpan<T>(Address, (int)Size);
        }
        ...
        {
            if (ptr.Size % Unsafe.SizeOf<T>() != 0)
                throw new InvalidCastException($"Pointer of size {ptr.Size} is not a multiple of {nameof(T)} size.");

            return new Pointer<T>(ptr.Address, ptr.Size / Unsafe.SizeOf<T>());
        }
    }
    ...
    public unsafe struct BlockPointer : IPointerType, Meta.IDescribe
    {
        ...
        public readonly int BlockSizeAsBytes;

#if VALIDATE
        public uint Generation;        
#endif

        public BlockPointer(void* ptr, int blockSizeAsBytes, int size)
        {
            this.Address = ptr;
            this.Size = size;
            this.BlockSizeAsBytes = blockSizeAsBytes;
        ...
        {
            if (this.IsValid)
                return $"{{{(long)this.Address:X32}|{this.Size}|{this.BlockSizeAsBytes}}}";
    ...
    public unsafe struct BlockPointer<T> : Meta.IDescribe, IPointerType<T> where T : struct
    {
        ...
        public BlockPointer(BlockPointer ptr)
        {
            this.Address = ptr.Address;
            this._size = ptr.Size / Unsafe.SizeOf<T>();
            this.BlockSizeAsBytes = ptr.BlockSizeAsBytes;
        ...
        {
            if (this.IsValid)
                return $"{{{(long)this.Address:X64}|{this.Size}|{this.BlockSizeAsBytes}|{this.SizeAsBytes}b}}";
        ...
        {
            return new BlockPointer(ptr.Address, ptr.BlockSizeAsBytes, ptr.SizeAsBytes);
        }

        public static implicit operator BlockPointer<T>(Pointer<T> ptr)
        {
            return new BlockPointer<T>(ptr.Address, ptr.SizeAsBytes, ptr.Size);
        }

        public static explicit operator BlockPointer<T>(BlockPointer ptr)
        {
            return new BlockPointer<T>(ptr.Address, ptr.BlockSizeAsBytes, ptr.Size / Unsafe.SizeOf<T>());
        }
    }
}

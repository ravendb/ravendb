...
namespace Sparrow.Server
{
        ...
        where TPointerType : struct, IPointerType
    {
        ...
        /// <summary>
        /// This is the total amount of memory currently either InUse or on hold on internal buffers since the last Reset Cycle.
        /// </summary>
        long Allocated { get; }

        /// <summary>
        /// This the is actual amount of memory that is on the customer hands at the current moment.
        /// </summary>
        long InUse { get; }
        ...
        where TAllocator : struct, IAllocator<TAllocator, Pointer>
    {
        ...
        private readonly SingleUseFlag _initialized = new SingleUseFlag();

        // The generation is composed of 2 values. The allocator ID which are the first 12 bits and the 
        // actual allocator generation. The first one is used to check for pointer allocate-release crossovers
        // and the second part to check for memory used after renewing or resetting the allocator.
        // The allocator ID will be assigned on creation and remain fixed. 
        ...
        where TAllocator : struct, IAllocator<TAllocator, BlockPointer>, IAllocator
    {
        ...
        /// <summary>
        /// A renewable allocator will explicitly implement the renewable interface. Allocators may choose to
        /// implement the ability of not releasing to the parent allocator the memory they own. In those cases,
        /// is where calling renew makes sense. While Reset will return the memory to the parent, renew will
        /// try to keep as much backend memory as possible to amortize the allocation code. 
        /// </summary>
        /// <remarks>Not all allocators will implement but those that do, will try to clean after themselves. </remarks>
        public void Renew()
        {
            if (_allocator is IRenewable<TAllocator> a)
                a.Renew(ref _allocator);
        ...
        /// <summary>
        /// Every allocator when asked to reset will get rid of as much memory as it possibly can. Allocators,
        /// when asked to `Reset`, wont try to speculatively hoard on memory. They will release as much as they
        /// can to the parent allocator.
        /// </summary>
        public void Reset()
        {
            _allocator.Reset(ref _allocator);

...
public static partial class MemoryInformation
{
    ...
    private static LightWeightMemoryInfoResult GetEarlyOutOfMemoryInfo()
    {
        if (_failedToGetAvailablePhysicalMemory)
        {
            if (Logger.IsDebugEnabled)
                Logger.Debug("Because of a previous error in getting available memory, we are now lying and saying we have 256MB free");
        ...
        catch (Exception e)
        {
            if (Logger.IsWarnEnabled)
                Logger.Warn("Error while trying to get available memory, will stop trying and report that there is 256MB free only from now on", e);
    ...
    private static unsafe LightWeightMemoryInfoResult GetEarlyOutOfMemoryInfoWindows()
    {
        ...
        {
            if (Logger.IsDebugEnabled)
                Logger.Debug("Failure when trying to read memory info from Windows, error code is: " + Marshal.GetLastWin32Error());
            return FailedLightWeightMemoryResult;
        }
        ...
        if (Win32MemoryMethods.IsProcessInJob(ProcessHandle, IntPtr.Zero, out var isInJob) && isInJob)
        {
            ...
            {
                if (_reportedQueryJobObjectFailure == false && Logger.IsDebugEnabled)
                {

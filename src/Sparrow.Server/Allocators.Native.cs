...
namespace Sparrow.Server
{
        ...
        where TOptions : struct, INativeOptions
    {
        ...
        private void CheckForLeaks(ref NativeAllocator<TOptions> allocator)
        {
            ...
            {
#if VALIDATE || DEBUG
                var stackFrameLines = allocator._initializeStackTrace.ToString().Split(Environment.NewLine.ToCharArray(), StringSplitOptions.None);
                string stackFrame = $"{Environment.NewLine}Construction Stack Trace: {Environment.NewLine}{stackFrameLines.Take(3)}{Environment.NewLine}End Construction Stack Trace";                
#else

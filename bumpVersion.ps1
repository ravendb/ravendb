...
function UpdateFileInGitHub($fileUri, $newFileContent, $commitMessage, $branch) {
    ...
    write-host "GET $fileUri`?ref=$branch properties"
    
    $response = Invoke-RestMethod -TimeoutSec 120 "$fileUri`?ref=$branch"
...
function GetAssemblyInfoWithBumpedVersion ($fileUri, $branch, $newVersion, $newInformationalVersion) {
    $assemblyInfoFileContents = (GetAssemblyInfoContent $fileUri $branch) |

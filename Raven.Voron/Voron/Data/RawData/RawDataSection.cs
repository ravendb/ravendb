...
namespace Voron.Data.RawData
{
    ...
    public unsafe class RawDataSmallSection
    {
        ...
        public bool TryWrite(long id, byte* data, int size)
        {
            ...
            if (sizes[0] < sizes[1])
                throw new InvalidDataException("Asked to load a value that where the allocated size is smaller than the used size: " + id + " from page " +
                                               pageHeader->PageNumber);
        ...
        {
            size += sizeof(short)/*allocated size */ + sizeof(short) /*actual size*/;// we need to have the size value here, so we add that

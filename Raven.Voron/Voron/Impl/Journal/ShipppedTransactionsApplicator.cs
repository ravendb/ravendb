...
namespace Voron.Impl.Journal
{
    public unsafe class ShipppedTransactionsApplicator : IDisposable
    {
        ...
        public void ApplyShippedLogs(TransactionToShip shippedTransactions)
        {
            if (shippedTransactions == null)
                throw new ArgumentNullException();

            //todo: uncompress the data & validate it
            ...
            {
                var buffer = ReadFromShippedTransaction(tx, shippedTransactions.Header, shippedTransactions.PreviousTransactionCrc, shippedTransactions.CopiedPages);
                tx.WriteDirect(buffer);
                tx.Commit();
            }
        }

        protected PageFromScratchBuffer ReadFromShippedTransaction(Transaction tx, TransactionHeader txHeader, uint previousTransactionCrc, byte[] txPages)
        {   
            var compressedPages = (txHeader.CompressedSize / AbstractPager.PageSize) + (txHeader.CompressedSize % AbstractPager.PageSize == 0 ? 0 : 1);

            fixed (byte* txPagesPtr = txPages)
            {
                ...
                if (_previousTransactionCrc != previousTransactionCrc)

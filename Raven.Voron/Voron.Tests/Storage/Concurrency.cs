...
namespace Voron.Tests.Storage
{
    ...
    public class Concurrency : StorageTest
    {
        ...
        public void MissingEntriesShouldReturn0Version()
        {
            ...
            {
                Assert.Equal(0, tx.Root.ReadVersion("key/1"));
            }
        }
        ...
        public void SimpleVersion()
        {
            using (var tx = Env.NewTransaction(TransactionFlags.ReadWrite))
            {
                tx.Root.Add			("key/1", StreamFor("123"));
                Assert.Equal(1, tx.Root.ReadVersion("key/1"));
                tx.Root.Add			("key/1", StreamFor("123"));
                Assert.Equal(2, tx.Root.ReadVersion("key/1"));
        ...
        public void VersionOverflow()
        {
            using (var tx = Env.NewTransaction(TransactionFlags.ReadWrite))
            {
                for (uint i = 1; i <= ushort.MaxValue + 1; i++)
                {
                    ...
                    Assert.Equal(expected, tx.Root.ReadVersion("key/1"));
                }
        ...
        public void Missing()
        {
            using (var tx = Env.NewTransaction(TransactionFlags.ReadWrite))
            {
                tx.Root.Add			("key/1", StreamFor("123"), 0);
                Assert.Equal(1, tx.Root.ReadVersion("key/1"));
            ...
            using (var tx = Env.NewTransaction(TransactionFlags.ReadWrite))
            {
                var e = Assert.Throws<ConcurrencyException>(() => tx.Root.Add			("key/1", StreamFor("321"), 0));
        ...
        public void ConcurrencyExceptionShouldBeThrownWhenVersionMismatch()
        {
            ...
            using (var tx = Env.NewTransaction(TransactionFlags.ReadWrite))
            {
                var e = Assert.Throws<ConcurrencyException>(() => tx.Root.Add			("key/1", StreamFor("321"), 2));
                Assert.Equal("Cannot add 'key/1' to 'Root' tree. Version mismatch. Expected: 2. Actual: 1.", e.Message);
            }

            using (var tx = Env.NewTransaction(TransactionFlags.ReadWrite))
            {
                var e = Assert.Throws<ConcurrencyException>(() => tx.Root.Delete("key/1", 2));
        ...
        public void ConcurrencyExceptionShouldBeThrownWhenVersionMismatchMultiTree()
        {
            ...
            using (var tx = Env.NewTransaction(TransactionFlags.ReadWrite))
            {
                var e = Assert.Throws<ConcurrencyException>(() => tx.Root.MultiAdd("key/1", "321", version: 2));
                Assert.Equal("Cannot add value '321' to key 'key/1' to 'Root' tree. Version mismatch. Expected: 2. Actual: 0.", e.Message);
            }

            using (var tx = Env.NewTransaction(TransactionFlags.ReadWrite))
            {
                var e = Assert.Throws<ConcurrencyException>(() => tx.Root.MultiDelete("key/1", "123", 2));

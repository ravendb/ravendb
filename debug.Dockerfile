FROM mcr.microsoft.com/dotnet/core/sdk:10.0 AS build
WORKDIR /app
...
RUN dotnet build ./src/Raven.Server/Raven.Server.csproj && \
    echo '{}' > ./src/Raven.Server/bin/Debug/net10.0/settings.json
...
ENTRYPOINT [ "dotnet", "./src/Raven.Server/bin/Debug/net10.0/Raven.Server.dll" ]

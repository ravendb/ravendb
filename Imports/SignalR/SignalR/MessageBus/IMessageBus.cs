...
namespace Raven.Imports.SignalR
{
    ...
    public interface IMessageBus
    {
        ...
        /// <param name="cancel">A <see cref="CancellationToken"/> that represents cancellation of the wait operation when there's no messages.</param>
        /// <returns>A <see cref="Task{MessageResult}"/> that completes when data is received on the <see cref="IMessageBus"/>.</returns>
        Task<MessageResult> GetMessages(IEnumerable<string> eventKeys, string cursor, CancellationToken cancel);

...
{
	public unsafe class ExternalHttpListenerServer : RoutingHost
	{
		...
		/// <summary>
		/// Initializes new instance of <see cref="ExternalHttpListenerServer"/>.
		/// </summary>
		/// <param name="url">The url to host the server on.</param>
		/// <param name="listener">The external listener to use </param>
		public ExternalHttpListenerServer(string url, HttpListener listener)
			: this(url, GlobalHost.DependencyResolver, listener)
		{
		...
		/// <param name="listener">The external listener to use </param>
		public ExternalHttpListenerServer(string url, IDependencyResolver resolver, HttpListener listener)
			: base(resolver)
		{
			_url = url.Replace("*", @".*?").Replace("+",".+?");
			...
			{
				_requestQueueHandle = (CriticalHandle)requestQueueHandleField.GetValue(listener);
			}
		}

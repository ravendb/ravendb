...
# RECOMMENDED: Server-only (5-10 minutes)
./build.ps1 -LinuxX64 -DontBuildStudio  # Linux x64, skip Studio
./build.ps1 -WinX64 -DontBuildStudio    # Windows x64, skip Studio
...
npm run prettier && npm run lint         # Studio
...
./build.ps1 -LinuxX64 -DontBuildStudio  # Skip Studio build
...
npm run prettier-fix && npm run lint-fix   # Studio code style
```
...
1. `./build.ps1 -LinuxX64 -DontBuildStudio`
...
- Studio builds are slowest (10+ min) - use `-DontBuildStudio` when possible

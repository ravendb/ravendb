    ...
    public class WorkingWithConflictsTests : RavenFsTestBase
	{
		...
		public async void Should_be_able_to_get_conflicts()
		{
			...
			for (int i = 0; i < 25; i++)
			{
				...
				{
                    await  destination.Config.SetConfig("test" + i, new RavenJObject { { "foo", "bar" } });
				}
		...
		public void Should_detect_conflict_on_destination()
		{
			...
			var request =
                (HttpWebRequest)WebRequest.Create(destination.Url + "/" + destination.FileSystemName + "/synchronization/updatemetadata/" + fileName);
		...
		public void Should_detect_conflict_on_renaming_synchronization()
		{
            ...
            sourceClient.UploadAsync("test.bin", new RavenJObject { { "key", "value" } }, content).Wait();
			content.Position = 0;
            destinationClient.UploadAsync("test.bin", new RavenJObject { { "key", "value" } }, content).Wait();
		...
		public void Should_successfully_get_finished_and_conflicted_synchronization()
		{
            ...
            var webRequest = (HttpWebRequest)WebRequest.Create(destinationClient.Url + "/" + destinationClient.FileSystemName + "/synchronization/updatemetadata/test.bin");

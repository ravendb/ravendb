...
namespace RavenFS.Tests.Auth
{
    public class ClientWithoutAuthenticationSetup : RavenFsTestBase
    {
        ...
        {
            var server = CreateRavenDbServer(Ports[0], fileSystemName: "WillUseDefaultNetworkCredentialsWhenServerRequiresAuthentication", enableAuthentication: true); // enable authentication

            using (var client = new RavenFileSystemClient(GetServerUrl(false, server.SystemDatabase.ServerUrl), "WillUseDefaultNetworkCredentialsWhenServerRequiresAuthentication"))
            {

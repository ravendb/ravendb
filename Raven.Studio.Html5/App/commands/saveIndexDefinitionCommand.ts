...
class saveIndexDefinitionCommand extends commandBase {
    ...
    execute(): JQueryPromise<any> {
        ...
        // 1. POSTing the index priority to /indexes/set-priority/[indexname]?priority=[priority]
        // 2. PUTing the index definition to /indexes/[index name]?definition=yes

        return $.when(this.savePriority(), this.saveDefinition())
            .done(() => this.reportSuccess("Saved " + this.index.Name))
            .fail(response => this.reportError("Failed to save " + this.index.Name, JSON.stringify(response)));
    }

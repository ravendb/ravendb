...
class getDatabasesCommand extends commandBase {
    ...
    execute(): JQueryPromise<database[]> {
        ...
        var resultsSelector = (databases: databaseDto[]) => databases.map((db: databaseDto) => new database(db.Name, db.Disabled, db.Bundles, db.IndexingDisabled, db.RejectClientsEnabled));

	...
	public class LiveTestResourceCleanerStartupTask : IServerStartupTask
	{
		...
		private void CleanupDatabases(DatabasesLandlord databaseLandlord)
		{
			...
			var databaseDocuments = systemDatabase
				.Documents
				.GetDocumentsWithIdStartingWith(Constants.RavenDatabasePathPrefix, null, null, 0, int.MaxValue, CancellationToken.None, ref nextStart);
			...
			foreach (var databaseId in databaseIds)
			{
				...
				{
					var key = databaseId;
					if (key.StartsWith(Constants.RavenDatabasePathPrefix))
						key = key.Substring(Constants.RavenDatabasePathPrefix.Length);
					...
					var docKey = Constants.RavenDatabasePathPrefix + key;
					systemDatabase.Documents.Delete(docKey, null, null);

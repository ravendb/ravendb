...
function GetVersionInfoWithBumpedVersion ($projectDir, $newVersion, $srcFileContent) {
    ...
    $pattern = [regex]'\[assembly: RavenVersion\(Build = "40", CommitHash = "([^"]*)", Version = "4.0", FullVersion = "[^"]*"\)\]'
    $m = $pattern.Match($result)
    $commit = $m.Groups[1]
    $result = $pattern.Replace(
        $result,
        "[assembly: RavenVersion(Build = ""40"", CommitHash = ""$commit"", Version = ""4.0"", FullVersion = ""$newVersion-custom-40"")]")

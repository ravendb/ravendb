...
function GetVersionInfoWithBumpedVersion ($projectDir, $newVersion, $srcFileContent) {
    ...
    $pattern = [regex]'\[assembly: RavenVersion\(Build = "51", CommitHash = "([^"]*)", Version = "5.1", FullVersion = "[^"]*"\)\]'
    $m = $pattern.Match($result)
    $commit = $m.Groups[1]
    $result = $pattern.Replace(
        $result,
        "[assembly: RavenVersion(Build = ""51"", CommitHash = ""$commit"", Version = ""5.1"", FullVersion = ""$newVersion-custom-51"")]")

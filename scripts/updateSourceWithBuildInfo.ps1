...
function UpdateRavenVersion ( $projectDir, $buildNumber, $version, $commit, $versionInfoFile ) {
    ...
    $content = (Get-Content $versionInfoFile) |
        Foreach-Object { 
            $_ -replace `
                'RavenVersion\(Build = ".*", CommitHash = ".*", Version = "5.4", FullVersion = ".*", ReleaseDateString = ".*"\)', `
                "RavenVersion(Build = ""$buildNumber"", CommitHash = ""$commit"", Version = ""5.4"", FullVersion = ""$version"", ReleaseDateString = ""$releaseDate"")" 
            }

...
function LayoutServerPackage ( $packageDir, $projectDir, $packOpts ) {
    ...
    #    # TODO get 'net10.0' from somewhere
    #    $buildDir = [io.path]::Combine($projectDir, "src", "Raven.Server", "bin", "Release", "net10.0", $target.Runtime)
...
function CopyDirectoryContents ( $tag, $src, $dst, [switch]$WarnOnDllConflict ) {
    ...
    if ($WarnOnDllConflict) {
        try {
            ...
            if ($_.Exception -and $_.Exception.Message -match 'Could not copy file .+System\.Diagnostics\.EventLog\.Messages\.dll.+Source hash:.+Destination hash:') {

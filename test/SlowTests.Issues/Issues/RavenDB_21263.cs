...
namespace SlowTests.Issues
{
    public class RavenDB_21263 : ReplicationTestBase
    {
        ...
        // Three generations of the dump's DatabaseRecord:
        //   Old (pre-RavenDB-21263)         : no SupportedFeatures field at all.
        //   Mid (RavenDB-21263 / pre-22358) : SupportedFeatures = [ThrowRevisionKeyTooBigFix].
        //   New (RavenDB-22358 +)           : SupportedFeatures = [HashedRevisionPk].
        // Derived from OldJsonRecord by splicing the SupportedFeatures fragment right after `"Encrypted": false,`.
        public enum JsonRecordVersion { Old, Mid, New }

        private static readonly string MidJsonRecord = WithSupportedFeatures(Raven.Client.Constants.DatabaseRecord.SupportedFeatures.ThrowRevisionKeyTooBigFix);
        private static readonly string NewJsonRecord = WithSupportedFeatures(Raven.Client.Constants.DatabaseRecord.SupportedFeatures.HashedRevisionPk);

        private static string WithSupportedFeatures(string flag) =>
            OldJsonRecord.Replace("\"Encrypted\": false,\r\n",
                $"\"Encrypted\": false,\r\n        \"SupportedFeatures\": [\"{flag}\"],\r\n");

        private static string JsonRecordFor(JsonRecordVersion version) => version switch
        {
            JsonRecordVersion.Old => OldJsonRecord,
            JsonRecordVersion.Mid => MidJsonRecord,
            JsonRecordVersion.New => NewJsonRecord,
            _ => throw new ArgumentOutOfRangeException(nameof(version), version, null)
        };



        // Import always treats the database as a fresh one, so AdminDatabasesHandler.Put has already
        // stamped SupportedFeatures = [HashedRevisionPk] before the dump arrives. The dump's own
        // SupportedFeatures (none, the legacy ThrowRevisionKeyTooBigFix flag, or the new HashedRevisionPk)
        // does NOT downgrade the live record.
        [RavenTheory(RavenTestCategory.Smuggler | RavenTestCategory.BackupExportImport)]
        [RavenData(JsonRecordVersion.Old)]
        [RavenData(JsonRecordVersion.Mid)]
        [RavenData(JsonRecordVersion.New)]
        public async Task ImportRevisionsWithLargeChangeVector_NowSucceeds(Options options, JsonRecordVersion version)
        {
            using (var store = GetDocumentStore(options))
            {
                var jsonString = JsonRecordFor(version);
        ...
        [RavenFact(RavenTestCategory.Smuggler | RavenTestCategory.BackupExportImport)]
        public async Task RestoreFromBackupWithDatabaseVersion_NowSucceeds()
        {
            using (var store = GetDocumentStore())
            {
                ...
                await using (var zipStream = new GZipStream(ms, CompressionMode.Compress))
                {
                    ...
                    var databaseName = "test111_NewBitsRestore_" + Guid.NewGuid().ToString("N");
                    var restoreConfig = new RestoreBackupConfiguration { BackupLocation = backupPath, DatabaseName = databaseName };
        ...
        public async Task RestoreFromSnapshotShouldNotThrow_Cluster()
        {
            ...
            }))
            {
                ...
                foreach (var node in cluster.Nodes)
                {
                    // validate that each cluster node does not contain 'HashedRevisionPk'
                    // (snapshot restore preserves the source's SupportedFeatures, which was empty)
                    ...
                    Assert.False(record.SupportedFeatures.Contains(Raven.Client.Constants.DatabaseRecord.SupportedFeatures.HashedRevisionPk));
                }
            }
        }

        [RavenFact(RavenTestCategory.Smuggler | RavenTestCategory.BackupExportImport | RavenTestCategory.Replication)]
        public async Task LargeChangeVectorReplicatedToNewBits_PutSucceedsWithoutPruning()
        {
            using (var store = GetDocumentStore())
            {
        ...
        private async Task AssertSupportedFeaturesAsync(DocumentStore store, bool oldVersion)
        {
            ...
            if (oldVersion)
                ...
                Assert.True(databaseRecord.SupportedFeatures.Contains(Raven.Client.Constants.DatabaseRecord.SupportedFeatures.HashedRevisionPk));
        }

...
public class RavenDB_27035 : RavenTestBase
{
    ...
    // An entry without the field has a row in the field's entries -> terms tree as well as in the non-existing posting
    // list, so adding both counted it twice and the surplus hid the entry whose empty list produced no term at all.
    [RavenTheory(RavenTestCategory.Querying | RavenTestCategory.Corax)]
    [RavenData(DatabaseMode = RavenDatabaseMode.Single, SearchEngineMode = RavenSearchEngineMode.Corax)]
    public void OrderByShouldNotDropAnEntryThatProducedNoTerm(Options options)
    {
        using (var store = GetDocumentStore(options))
        {
            using (var session = store.OpenSession())
            {
                session.Store(new Movie { Id = "movies/1", Title = "Alpha", Year = 2001 });
                session.Store(new Movie { Id = "movies/2", Title = new object[] { }, Year = 2002 });
                session.SaveChanges();
            }

            using (var bulk = store.BulkInsert())
                bulk.Store(new { Year = 2003 }, "movies/3", new MetadataAsDictionary { [Constants.Documents.Metadata.Collection] = "Movies" });
            ...
            using (var session = store.OpenSession())
            {
                var results = session.Advanced.RawQuery<Movie>("from index 'Movies/ByTitleAndYear' order by Title include timings()")
                    .Timings(out var timings)
                    .ToList();

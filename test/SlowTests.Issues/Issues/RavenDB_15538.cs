            ...
            using (var store = GetDocumentStore(options))
            {
                ...
                using (var session = store.OpenAsyncSession())
                {
                    ...
                    using (documentsContext.OpenReadTransaction())
                    {
                        ...
                        using (RevisionsStorage.GetRevisionKeyHashSlice(documentsContext.Allocator, rawCvSlice, out Slice hashSlice, out _))
                        {
                            Assert.StartsWith($"user/1\u001er\u001e{hashSlice}\u001e1\u001e{revisionsAttachment.Base64Hash}\u001eimage/png", revisionsAttachment.Key);
                        }
                    }
                }
            }

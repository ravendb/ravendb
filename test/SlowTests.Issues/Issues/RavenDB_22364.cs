...
public class RavenDB_22364 : RavenTestBase
{
    ...
    public void LinqSearchOptionsJoinToThePreviousClauseAndNotToTheNextOne()
    {
        ...
        // a group of searches joins the clause before it with the first search's flag - and, without one, like any
        // other search statement: OR after a search, the default operator after anything else
        ...
        Assert.Equal("from 'Employees' where search(Name, $p0) and (search(Category, $p1) or search(Surname, $p2))",
            session.Query<Employee>().Search(x => x.Name, "ma").Search(x => x.Category, "a", options: SearchOptions.And).Search(x => x.Surname, "ko").ToString());

        Assert.Equal("from 'Employees' where search(Name, $p0) or (search(Category, $p1) or search(Surname, $p2))",
            session.Query<Employee>().Search(x => x.Name, "ma").Search(x => x.Category, "a", options: SearchOptions.Or).Search(x => x.Surname, "ko").ToString());

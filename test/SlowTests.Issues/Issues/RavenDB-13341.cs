        ...
        public async Task CanReplicateMissingAttachment(Options options)
        {
            ...
            using (var destination = GetDocumentStore(options))
            {
                ...
                using (var tx = context.OpenWriteTransaction())
                {
                    ...
                    using (var stream = new MemoryStream(new byte[] { 1, 2, 3 }))
                    {
                        attachmentStorage.PutAttachment(context, documentId, attachmentName, contentType, attachment.Base64Hash.ToString(), stream.Length, remoteParams: null, expectedChangeVector: null, stream, updateDocument: false);
                    }

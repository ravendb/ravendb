    ...
    public class RavenDB_27347 : StorageTest
    {
        ...
        public void CompressedLeavesSurviveSequentialChurnWithDeletesAndOverflowValues(int seed)
        {
            ...
            using (var tx = Env.ReadTransaction())
            {
                ...
                Assert.Equal(branchPages, tree.ReadHeader().BranchPages);
                Assert.Equal(leafPages, tree.ReadHeader().LeafPages);
                ...
                // Assert.Equal(expected.Count, tree.ReadHeader().NumberOfEntries);
                // Assert.Equal(overflowPages, tree.ReadHeader().OverflowPages);
                // Assert.Equal(branchPages + leafPages + overflowPages, tree.ReadHeader().PageCount);
        ...
        private static unsafe long CountOverflowPages(Transaction tx, TreePage page)
        {
            ...
            for (int i = 0; i < page.NumberOfEntries; i++)
            {
                ...
                if (node->Flags == TreeNodeFlags.PageRef)
                    count += Paging.GetNumberOfOverflowPages(GetPage(tx, node->PageNumber).OverflowSize);
            }

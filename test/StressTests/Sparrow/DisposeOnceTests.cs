...
namespace StressTests.Sparrow
{
    ...
    public sealed class QueuedTaskScheduler : TaskScheduler, IDisposable
    {
        ...
        private static LightThreadLocal<bool> _taskProcessingThread = new LightThreadLocal<bool>();
    ...
    public class DisposeOnceTests : NoDisposalNeeded
    {
        ...
        [InlineData(8)]
        [InlineData(16)]
        public void WithRetrySingleThreadException(int numRetries)
        {
            int counter = 0;
        ...
        [InlineData(4, 10000)]
        [InlineData(8, 100000)]
        [InlineData(16, 1000000)]
        public void SingleAttemptMultiThreadNoErrors(int numThreads, int numAttempts)
        {
            RunWithScheduler(numThreads, scheduler =>
            {
                long counter = 0;
                var disposer = new DisposeOnce<SingleAttempt>(() => Interlocked.Increment(ref counter));

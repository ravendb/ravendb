    ...
    public class ClusterDatabaseMaintenance : ReplicationTestBase
    {
        ...
        public async Task ReshuffleAfterPromotion()
        {
            ...
            using (var store = new DocumentStore { Urls = new[] { cluster.Leader.WebUrl } }.Initialize())
            {
                ...
                for (int i = 0; i < numberOfDatabases; i++)
                {
                    ...
                    await Cluster.WaitForRaftIndexToBeAppliedInCluster(databaseResult.RaftCommandIndex, TimeSpan.FromSeconds(10));
                }
        ...
        public async Task RemoveNodeFromClusterWhileDeletion()
        {
            ...
            })
            {
                ...
                await Cluster.WaitForRaftIndexToBeAppliedInCluster(index, TimeSpan.FromSeconds(30));
        ...
        public async Task ReduceChangeVectorWhenRemovingNode()
        {
            ...
            }))
            {
                ...
                await Cluster.WaitForRaftIndexToBeAppliedInCluster(deleteResult.RaftCommandIndex, TimeSpan.FromSeconds(10));
        ...
        public async Task CanRemoveChangeVector()
        {
            using (var store = GetDocumentStore())
            {
                ...
                await Cluster.WaitForRaftCommandToBeAppliedInLocalServer(nameof(UpdateUnusedDatabaseIdsCommand));
        ...
        public async Task KeepDatabaseIdOnSoftDelete()
        {
            ...
            }))
            {
                ...
                await Cluster.WaitForRaftIndexToBeAppliedInCluster(result.RaftCommandIndex + 1, TimeSpan.FromSeconds(15));
                var record = await store.Maintenance.Server.SendAsync(new GetDatabaseRecordOperation(store.Database));
                Assert.Equal(1, record.UnusedDatabaseIds.Count);

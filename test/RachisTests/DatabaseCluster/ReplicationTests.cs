...
namespace RachisTests.DatabaseCluster
{
    public class ReplicationTests : ReplicationTestBase
    {
        ...
        [RavenTheory(RavenTestCategory.ClusterTransactions | RavenTestCategory.Certificates)]
        [InlineData(false)]
        [InlineData(true)]
        ...
        public async Task ExternalReplicationFailover(Options source, Options destination)
        {
            var clusterSize = 3;
            ...
            using (var dst = GetDocumentStore(destination))
            {
                ...
                using (var dstSession = src.OpenSession())
                {

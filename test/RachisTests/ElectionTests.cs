...
namespace RachisTests
{
    public class ElectionTests : RachisConsensusTestBase
    {
        ...
        [Fact]
        public async Task RavenDB_13228_2sec()
        {
            Console.WriteLine("Start test 13228");
            DebuggerAttachedTimeout.DisableLongTimespan = true;
            var numberOfNodes = 2;
            var firstLeader = await CreateNetworkAndGetLeader(numberOfNodes);
            var follower = GetFollowers().Single();

            var timeToWait = TimeSpan.FromMilliseconds(2000);
            await IssueCommandsAndWaitForCommit(10, "test", 1);
            var currentTerm = firstLeader.CurrentTerm;
            Console.WriteLine($"url {firstLeader.Url} - {follower.Url}");
            ...
            Console.WriteLine("After reconnect");
            ...
            Console.WriteLine("after fail test 13228");
            ...
            Console.WriteLine("end test 13228");
        }

        [Fact]
        public async Task RavenDB_13228_3sec()
        {
            Console.WriteLine("Start test 13228");
            DebuggerAttachedTimeout.DisableLongTimespan = true;
            var numberOfNodes = 2;

            ...
            InterversionTestBase.ProcessNode node, string dbName, List<DocSnapshot> docs, string label)
        {
            ...
            using (ctx.OpenReadTransaction())
            {
                foreach (var doc in docs)
                {
                    ...
                    using (DocumentIdWorker.GetLowerIdSliceAndStorageKey(ctx, doc.DocId, out Slice lowerIdSlice, out _))
                    {
                    ...
                    using (DocumentIdWorker.GetLowerIdSliceAndStorageKey(ctx, doc.DocId, out Slice docIdSlice, out _))
                    {

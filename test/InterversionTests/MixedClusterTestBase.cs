...
namespace InterversionTests
{
    public abstract class MixedClusterTestBase : InterversionTestBase
    {
        ...
        protected async Task<List<ProcessNode>> CreateCluster(string[] peers, X509Certificate2 certificate = null, bool watcherCluster = false, Dictionary<string, string> customSettings = null)
        {
            var processes = new List<ProcessNode>();
            foreach (var peer in peers)
            {
                processes.Add(await GetServerAsync(peer, customSettings: customSettings));
            }

        ...
        public async Task PeriodicExport_should_work_with_long_intervals()
        {
            ...
            using (var store = await GetDocumentStore())
            {
                ...
                typeof(PeriodicExportRunner)
                    .GetField(nameof(PeriodicExportRunner.MaxTimerTimeout), BindingFlags.Instance | BindingFlags.NonPublic)
                    .SetValue(periodicExportRunner, 5);
            ...
            using (var store = await GetDocumentStore())
            {
                await store.Smuggler.ImportAsync(new DatabaseSmugglerOptions(), new File());

...
{
    public class AbstractJavaScriptIndexTests : RavenTestBase
    {
        [Fact]
        ...
        [Fact]
        public void CanReduceNullValues2()
        {
            using (var store = GetDocumentStore())
            {
                store.ExecuteIndex(new UsersReducedByName2());
                using (var session = store.OpenSession())
                {
                    ...
                    session.SaveChanges();
                    WaitForIndexing(store);
                    var res = session.Query<User>("UsersReducedByName2").OfType<ReduceResults>().Single(x => x.Count == 3);
        ...
        private class UsersReducedByName2 : AbstractJavaScriptIndexCreationTask
        {
            public UsersReducedByName2()
            {
                Maps = new HashSet<string>
                {
                    @"map('Users', function (u){ return { Name: u.Name, Count: 1};})",

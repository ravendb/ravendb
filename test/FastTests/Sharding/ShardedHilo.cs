...
{
    public class ShardedHilo : ShardedTestBase
    {
        public ShardedHilo(ITestOutputHelper output) : base(output)
        {
        }
...
*/
        ...
        private static async Task<List<ModifyOngoingTaskResult>> SetupReplicationAsync(IDocumentStore fromStore, params IDocumentStore[] toStores)
        {
        ...
        private static async Task<ModifyOngoingTaskResult> AddWatcherToReplicationTopology<T>(
        ...
        private static T WaitForDocumentToReplicate<T>(IDocumentStore store, string id, int timeout)
            where T : class
        {

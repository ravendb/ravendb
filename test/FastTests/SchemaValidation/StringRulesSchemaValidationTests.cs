...
public class StringRulesSchemaValidationTests : SchemaValidationTestsBase
{
    ...
    public void SchemaValidation_WhenValidateMinStringLength()
    {
        ...
        () =>
        {
            ...
            Assert.StartsWith("The length of the value at 'root.prop' should be at least 5, but its actual length is 4.", errors);
        });
    }
    ...
    public void SchemaValidation_WhenValidateMaxStringLength()
    {
        ...
        () =>
        {
            ...
            Assert.False(schemaValidator.Validate(invalidObj, out var errors));
            Assert.StartsWith("The length of the value at 'root.prop' should not exceed 5, but its actual length is 6.", errors);
        });
    }
    ...
    public void SchemaValidation_WhenValidateForRegexPattern()
    {
        ...
        () =>
        {
            ...
            Assert.False(schemaValidator.Validate(invalidObj, out var errors));
            Assert.StartsWith("The value 'i3' at 'root.prop' does not match the required pattern '[A-Za-z]{2,3}\\d'.", errors);
        });
    }
}

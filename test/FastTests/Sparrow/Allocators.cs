...
namespace FastTests.Sparrow
{
    public unsafe class AllocatorsTests : NoDisposalNeeded
    {
        ...
        [Fact]
        public void Alloc_Pointer_ImplicitConversion()
        {
            var allocator = new Allocator<NativeAllocator<NativeAllocator.DefaultZero>>();
            allocator.Initialize(default(NativeAllocator.DefaultZero));

            var intPtr = allocator.Allocate<int>(1000);

            Pointer ptr = intPtr;
            Assert.Equal((long)intPtr.Address, (long)ptr.Address);
            Assert.Equal(1000 * sizeof(int), ptr.Size);
            Assert.Equal(1000 * sizeof(int), intPtr.SizeAsBytes);

            Pointer<int> iPtr = intPtr;
            Assert.Equal((long)ptr.Address, (long)iPtr.Address);
            Assert.Equal(1000, iPtr.Size);
            Assert.Equal(1000 * sizeof(int), iPtr.SizeAsBytes);

            BlockPointer<int> bPtr = intPtr;
            Assert.Equal((long)bPtr.Address, (long)intPtr.Address);
            Assert.Equal(1000 * sizeof(int), bPtr.BlockSizeAsBytes);
            Assert.Equal(1000 * sizeof(int), bPtr.SizeAsBytes);
            Assert.Equal(1000, bPtr.Size);

            allocator.Release(ref intPtr);
        
        }

        [Fact]
        public void Alloc_BlockPointer_ImplicitConversion()
        {
            using (var allocator = new BlockAllocator<PoolAllocator<PoolAllocator.Default>>())
            {
                allocator.Initialize(default(PoolAllocator.Default));

                var intPtr = allocator.Allocate<int>(1000);

                BlockPointer bPtr = intPtr;
                Assert.Equal((long)intPtr.Address, (long)bPtr.Address);
                Assert.Equal(intPtr.SizeAsBytes, bPtr.Size);
                Assert.Equal(intPtr.SizeAsBytes, bPtr.BlockSizeAsBytes);
                Assert.True(bPtr.IsValid);

                BlockPointer<int> bcPtr = (BlockPointer<int>)bPtr;
                Assert.Equal((long)intPtr.Address, (long)bcPtr.Address);
                Assert.Equal(intPtr.Size, bcPtr.Size);
                Assert.Equal(intPtr.SizeAsBytes, bcPtr.SizeAsBytes);
                Assert.Equal(intPtr.BlockSizeAsBytes, bcPtr.BlockSizeAsBytes);
                Assert.True(bcPtr.IsValid);
        ...
        public void Alloc_PoolDefaultByBytes()
        {
            using (var allocator = new BlockAllocator<PoolAllocator<PoolAllocator.Static>>())
            {
                ...
                var ptr = allocator.Allocate(1000);
                Assert.Equal(1000, ptr.Size);
                Assert.Equal(1000, ptr.BlockSizeAsBytes);
                Assert.True(ptr.IsValid);
        ...
        public void Alloc_PoolReturnUsedBytes()
        {
            using (var allocator = new BlockAllocator<PoolAllocator<PoolAllocator.Static>>())
            {
                ...
                Assert.Equal(1024, ptr.BlockSizeAsBytes);
                Assert.True(ptr.IsValid);
        ...
        public void Alloc_PoolReturnBlockBytes()
        {
            using (var allocator = new BlockAllocator<PoolAllocator<PoolAllocator.Static>>())
            {
                ...
                Assert.Equal(size, nonReusedPtr.BlockSizeAsBytes);

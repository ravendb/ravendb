...
namespace FastTests.Voron
{
    public class CommitFinalizationRecovery : StorageTest
    {
        ...
        public void FailedCommitAfterJournalWriteDivergesFromRecovery()
        {
            ...
            using (Slice.From(tx.Allocator, "key", out Slice key))
                visibleBeforeRestart = tx.ReadTree("tree")?.TryRead(key, out _) == true;
            ...
            using (var tx = Env.ReadTransaction())
            using (Slice.From(tx.Allocator, "key", out Slice key))
                visibleAfterRestart = tx.ReadTree("tree")?.TryRead(key, out _) == true;

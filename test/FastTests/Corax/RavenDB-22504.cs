...
public class RavenDB_22504 : RavenTestBase
{
    ...
    private void Test(int numberOfDocuments, Options options)
    {
        ...
        using (var bulkInsert = store.BulkInsert())
        {
            ...
            {
                var id = $"fields-{new Random().Next(numberOfDocuments)}-A";
        ...
        for (int i = 0; i < documentsById.Length - 1; i++)
        {
            ...
            Assert.True(int.Parse(document1[7..][..^2]) <= int.Parse(document2[7..][..^2]),
                $"{int.Parse(document1[7..][..^2])} <= {int.Parse(document2[7..][..^2])} OR {document1} <= {document2}");
        }
    }

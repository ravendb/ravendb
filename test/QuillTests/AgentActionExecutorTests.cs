...
public class AgentActionExecutorTests(ITestOutputHelper output) : NoDisposalNeeded(output)
{
    ...
    public async Task The_delivered_body_is_the_model_arguments_and_nothing_else()
    {
        ...
        Assert.Equal("""{"ticketId":"T-1"}""", result);
    ...
    public async Task Webhook_non_2xx_is_reported_to_the_model()
    {
        ...
        Assert.Equal("""action failed: webhook returned 500 body: {"error":"boom"}""", result);
    }
    ...
    public async Task An_oversized_webhook_response_is_capped_and_says_so()
    {
        ...
        Assert.EndsWith(WebhookActionExecutor.TruncationMarker, result);
        Assert.Equal(
            WebhookActionExecutor.MaxResponseBytes + WebhookActionExecutor.TruncationMarker.Length, result.Length);
    }
    ...
    {
        await using var receiver = await MockQuillServices.StartAsync();
        // '€' is 3 bytes, so the byte cut lands inside a character rather than between two
        receiver.WebhookResponse = (200, new string('€', 4 * 1024));

        var result = await WebhookExecutor().ExecuteAsync(ActionRequest(), Webhook(receiver.WebhookUrl), CancellationToken.None);

        Assert.DoesNotContain('�', result);
        Assert.EndsWith(WebhookActionExecutor.TruncationMarker, result);
        Assert.Equal(WebhookActionExecutor.MaxResponseBytes / 3, result.Length - WebhookActionExecutor.TruncationMarker.Length);
    }
    ...
    public async Task A_webhook_that_succeeds_with_no_content_still_answers_the_model()
    {
        ...
        Assert.Equal("action succeeded: with no content", result);
    }

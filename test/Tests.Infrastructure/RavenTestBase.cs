...
namespace FastTests
{
    public abstract class RavenTestBase : TestBase
    {
        ...
        protected async Task<T> WaitForGreaterThanAsync<T>(Func<Task<T>> act, T expectedVal, int timeout = 15000, int interval = 100) where T : IComparable
        {
            return await WaitForPredicateAsync(t => t.CompareTo(expectedVal) > 0, act, timeout, interval);
        }
        ...
        protected class DummyWebSocket : WebSocket
        {
            private static readonly WebSocketReceiveResult Result = new WebSocketReceiveResult(1, WebSocketMessageType.Text, true);
            private readonly TaskCompletionSource<WebSocketReceiveResult> _completionSource = new TaskCompletionSource<WebSocketReceiveResult>(TaskCreationOptions.RunContinuationsAsynchronously);

...
namespace Tryouts
{
    public class Program
    {
        public static void Main(string[] args)
        {
            ...
            if (args.Length > 0 && args[0].Equals("--bulkdocs"))
            {
                if (args.Length != 4)
                {
                    System.Console.WriteLine("Usage : --bulkdocs <serverUrl> <dbname> <numOfDocs>");
                    return;
                }
                using (var store = new DocumentStore
                {
                    Url = args[1],
                    DefaultDatabase = args[2]
                })
                {
                    store.Initialize();
                    Util.BulkDoc(store, Convert.ToInt32(args[3]));
                }
                return;
            }
            ...
            if (args.Length > 0 && args[0].Equals("--createdb"))
            {
                if (args.Length != 3)
                {
                    System.Console.WriteLine("Usage : --createdb <serverUrl> <dbname>");
                    return;
                }
                using (var store = new DocumentStore
                {
    ...
    public class Util
    {
        ...
        public static void BulkDoc(DocumentStore store, int numOfDocs)
        {

...
namespace SlowTests.Issues
{
    public class RavenDB_7170 : NoDisposalNeeded
    {
        ...
        {
            var ex = Assert.Throws<IndexCompilationException>(() => IndexCompiler.Compile(cacheKey: null, new Bad_index_1().CreateIndexDefinition()));
            Assert.Contains("Indexing function must return an anonymous object", ex.Message);

            ex = Assert.Throws<IndexCompilationException>(() => IndexCompiler.Compile(cacheKey: null, new Bad_index_2().CreateIndexDefinition()));
            Assert.Contains("Indexing function must return an anonymous object", ex.Message);

...
public abstract class FilteredPullReplicationClusterConvergenceTestBase : ReplicationTestBase
{
    ...
    private sealed class ReplicationLab : IAsyncDisposable
    {
        ...
        private const int ReplicationWaitTimeoutMs = 25_000;
        private const int HandlerWaitTimeoutMs = 15_000;
        private const int NodeStateWaitTimeoutSeconds = 20;
        ...
        public static async Task<ReplicationLab> CreateAsync(
            FilteredPullReplicationClusterConvergenceTestBase owner,
            Options options,
            ScenarioDefinition definition,
            BridgeTicketMutationMode mutationMode)
        ...
        private async Task PrimeClusterAsync()
        {
            foreach (var node in ScenarioCatalog.OrderedNodes)
            {
                ...
                {
                    Assert.True(
                        _owner.WaitForDocument(_stores[otherClusterNode], docId, timeout: ReplicationWaitTimeoutMs),
        ...
        private async Task InflateTargetAsync(int stepIndex, ClusterNode source, ClusterNode target)
        {
            ...
            try
            {
                ...
                Assert.True(
                    _owner.WaitForDocument(sinkStore, GetTicketDocumentId(stepIndex), timeout: ReplicationWaitTimeoutMs),
                ...
                Assert.True(
                    _owner.WaitForDocument(_stores[target], GetTicketDocumentId(stepIndex), timeout: ReplicationWaitTimeoutMs),
        ...
        private async Task VerifyReplicationAliveAsync()
        {
            ...
            Assert.True(
                _owner.WaitForDocument(_stores[markerTargetClusterNode], markerId, timeout: ReplicationWaitTimeoutMs),
        ...
        {
            var timeout = Debugger.IsAttached ? TimeSpan.FromMinutes(5) : TimeSpan.FromSeconds(NodeStateWaitTimeoutSeconds);

...
public abstract class NonDocumentDbCvProtectionTestBase : ReplicationTestBase
{
    ...
    protected sealed class NonDocumentLab : IAsyncDisposable
    {
        ...
        public async Task ApplyHeartbeatChangeVectorAsync(LineageNode source, LineageNode target)
        {
            ...
            ReplicationDebugTrace.Write(
                "NonDocumentDbCvProtectionTestBase",
                $"ApplyHeartbeatChangeVectorAsync source='{source}', target='{target}', sourceChangeVector='{sourceChangeVector ?? "<null>"}', adjustedSourceChangeVector='{adjustedSourceChangeVector ?? "<null>"}', sourceLastDatabaseEtag={sourceLastDatabaseEtag}.");
    ...
    protected sealed class InternalLinkBlocker : IDisposable
    {
        ...
        public InternalLinkBlocker(DatabaseOutgoingReplicationHandler handler)
        {
            ...
            ReplicationDebugTrace.Write(
                "TestBlocker",
                $"Attached blocker handlerId='{handler.HandlerId}', destination='{handler.Destination?.FromString() ?? "<null>"}'.");
        }
        ...
        public void Release()
        {
            ReplicationDebugTrace.Write(
                "TestBlocker",
                $"Release blocker handlerId='{_handler.HandlerId}', destination='{_handler.Destination?.FromString() ?? "<null>"}'.");
            _gate.Set();
        }
        ...
        public void Dispose()
        {
            ...
            ReplicationDebugTrace.Write(
                "TestBlocker",
                $"Dispose blocker handlerId='{_handler.HandlerId}', destination='{_handler.Destination?.FromString() ?? "<null>"}'.");
            _gate.Set();
            _gate.Dispose();
        }
        ...
        private void WaitForFetch()
        {
            ReplicationDebugTrace.Write(
                "TestBlocker",
                $"WaitForFetch enter handlerId='{_handler.HandlerId}', destination='{_handler.Destination?.FromString() ?? "<null>"}', isSet={_gate.IsSet}.");
            _previousFetch?.Invoke();
            _gate.Wait();
            ReplicationDebugTrace.Write(
                "TestBlocker",
                $"WaitForFetch exit handlerId='{_handler.HandlerId}', destination='{_handler.Destination?.FromString() ?? "<null>"}'.");
        }
    }
}

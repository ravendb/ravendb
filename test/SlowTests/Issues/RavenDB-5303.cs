    ...
    public class RavenDB_5303 : RavenTestBase
    {
        ...
        public void can_load_documents_with_transformer_with_load_document_streaming()
        {
            using (var store = GetDocumentStore())
            {
                ...
                using (var session = store.OpenSession())
                {
                    using (var enumerator = session.Advanced.Stream<TestDocumentTransformer.Output>(startsWith: "", transformer: transformer.TransformerName))
                    {
                        var count = 0;
                ...
                using (var session = store.OpenSession())
                {
                    using (var enumerator = session.Advanced.Stream<TestDocumentTransformer2.Output>(startsWith:"", transformer: transformer2.TransformerName))
                    {
                        var count = 0;
                        while (enumerator.MoveNext())
                        {

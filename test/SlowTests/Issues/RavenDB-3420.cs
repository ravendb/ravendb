    ...
    public class RavenDB_3420 : RavenTestBase
    {
        ...
        public void bulk_insert_sharded(string databaseName1, string databaseName2)
        {
            ...
            using (var server2 = GetNewServer())
            {
                ...
                {
                    Url = server1.Server.Configuration.Core.ServerUrl,
                ...
                {
                    Url = server2.Server.Configuration.Core.ServerUrl,
                ...
                using (var store1 = new DocumentStore { Url = server1.Server.Configuration.Core.ServerUrl, DefaultDatabase = databaseName1 }.Initialize())
                {
                ...
                using (var store2 = new DocumentStore { Url = server2.Server.Configuration.Core.ServerUrl, DefaultDatabase = databaseName2 }.Initialize())
                {
        ...
        public void bulk_insert_sharded_when_specifying_default_database(string databaseName)
        {
            ...
            using (var server2 = GetNewServer())
            {
                ...
                using (var store1 = new DocumentStore { Url = server1.Server.Configuration.Core.ServerUrl, DefaultDatabase = databaseName }.Initialize())
                {
                    using (var session = store1.OpenSession())
                    {
                        var docs = session.Load<Profile>("Shard1/bulk1");
                ...
                using (var store2 = new DocumentStore { Url = server2.Server.Configuration.Core.ServerUrl, DefaultDatabase = databaseName }.Initialize())
                {
                    using (var session = store2.OpenSession())
                    {
                        var docs = session.Load<Profile>("Shard2/profiles/1");

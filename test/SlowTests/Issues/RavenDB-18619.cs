...
namespace SlowTests.Issues
{
    public class RavenDB_18619 : ClusterTestBase
    {
        ...
        /*
        [Fact]
        public async Task IncludeExplanationsShouldWorkCorrectly()
        {
            using (var store = GetDocumentStore())
            {
                using (var session = store.OpenSession())
                ...
                var index = new Item_List();
                await index.ExecuteAsync(store);
                Indexes.WaitForIndexing(store);

                using (var session = store.OpenSession())
                {
                    var query = session.Query<Item, Item_List>().Search(p => p.List, "null*", boost: 10);
                    var docQuery = query
                        .ToDocumentQuery()
                        .IncludeExplanations(out var explanations2);
                    var queryResults = docQuery.GetQueryResult();
                    ...
                    Assert.NotNull(explanations1);
                    //Assert.Same(explanations1, explanations2.GetExplanations());
                    var x = 1;
                }
            }
        }
        ...
        class Item_List : AbstractIndexCreationTask<Item>
        {
            public Item_List()
            {
                Map = (items) =>

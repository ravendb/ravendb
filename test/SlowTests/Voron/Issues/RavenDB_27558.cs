...
namespace SlowTests.Voron.Issues
{
    public class RavenDB_27558 : RavenTestBase
    {
        ...
        // Archived documents are rewritten through CompressedDocsSchema while regular writes to the same collection use DocsSchema.
        // When the transaction merger puts an ArchiveDocumentsCommand and regular puts into one transaction, the same Voron table is
        ...
        public async Task Archival_merged_with_regular_writes_must_not_corrupt_the_documents_table()
        {
            ...
            using (database.DocumentsStorage.ContextPool.AllocateOperationContext(out DocumentsOperationContext context))
            {
                ...
                // 1. regular write of a large document: opens the DocsSchema table and its section object without touching the section header
                ...
                // 2. the archiver rewrites every due document through CompressedDocsSchema, filling the small-row pages
                ...
                // 3. regular write of a small document: allocates from the section through the stale header

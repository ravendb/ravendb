    ...
    public class RavenDB_26746 : StorageTest
    {
        ...
        private void AssertTreeMatchesShadowSet(HashSet<string> live, int cycle)
        {
            using (var tx = Env.ReadTransaction())
            {
                ...
                Assert.True(live.Count == tree.ReadHeader().NumberOfEntries,
                    $"cycle {cycle}: tree header says {tree.ReadHeader().NumberOfEntries} entries but {live.Count} keys should be live (deletes silently missed keys or subtrees were lost)");

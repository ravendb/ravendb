    ...
    public class RavenDB_27347 : StorageTest
    {
        ...
        public void FailedRecompressionMustNotConsumeCompressionTombstoneTwice()
        {
            ...
            using (var tx = Env.WriteTransaction())
            {
                ...
                var root = GetPage(tx, tree.ReadHeader().RootPageNumber);

                Assert.True(root.IsCompressed);
                Assert.Equal(1, tree.ReadHeader().Depth);
                Assert.Equal(16, tree.ReadHeader().NumberOfEntries);
                Assert.Equal(2, tree.ReadHeader().OverflowPages);
                Assert.Equal(3, tree.ReadHeader().PageCount);
            ...
            using (var tx = Env.WriteTransaction())
            {
                ...
                Assert.Equal(16, tree.ReadHeader().NumberOfEntries);
                Assert.Equal(1, tree.ReadHeader().OverflowPages);
        ...
        public void SplittingDecompressedPageMustKeepOverflowEntryInTheMiddle()
        {
            ...
            using (var tx = Env.WriteTransaction())
            {
                ...
                Assert.True(root.IsCompressed);
                Assert.Equal(1, tree.ReadHeader().Depth);
                Assert.Equal(15, tree.ReadHeader().NumberOfEntries);
                Assert.Equal(1, tree.ReadHeader().OverflowPages);
                Assert.Equal(2, tree.ReadHeader().PageCount);
            ...
            using (var tx = Env.WriteTransaction())
            {
                ...
                var page = tree.ModifyPage(tree.ReadHeader().RootPageNumber);
        ...
        public void DecompressedPageThatConsumedTombstonesMustBeWrittenBackEvenIfItCannotBeCompressed()
        {
            ...
            using (var tx = Env.WriteTransaction())
            {
                ...
                Assert.True(GetPage(tx, tree.ReadHeader().RootPageNumber).IsCompressed);
                Assert.Equal(16, tree.ReadHeader().NumberOfEntries);
                Assert.Equal(2, tree.ReadHeader().OverflowPages);
        ...
        public void SuccessfulRecompressionMustResetMustBeWrittenBackOfTheCachedDecompressedPage()
        {
            ...
            {
                var tree = tx.ReadTree("tree");
                var rootPageNumber = tree.ReadHeader().RootPageNumber;

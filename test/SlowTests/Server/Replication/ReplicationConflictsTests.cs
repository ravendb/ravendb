    ...
    public class ReplicationConflictsTests : ReplicationTestBase
    {
        ...
        public async Task Should_not_resolve_conflcit_with_script_when_they_from_different_collection()
        {
            ...
            }))
            {
                ...
                await SetupReplicationAsync(store1, store2);
                var db2 = (await Server.ServerStore.DatabasesLandlord.TryGetOrCreateResourceStore(store2.Database)).NotificationCenter;
        ...
        public async Task ConflictResolutionThrowsMeaningfulExceptionOnUsingForbiddenMethod_OnReplication(string script)
        {
            ...
            Assert.Equal(alert.Reason, AlertReason.Replication);
        ...
        public async Task ConflictResolutionThrowsMeaningfulExceptionOnUsingForbiddenMethod_WithScript(string script)
        {
            ...
            using (db.NotificationCenter.TrackActions(notificationsQueue, null))
            {
                ...
                var alert = await notificationsQueue.DequeueUntilAsync(static item =>
                    {
                        var alertReason = item[nameof(AlertRaised.Reason)];
                        var severity = item[nameof(AlertRaised.Severity)];
                        var message = item[nameof(AlertRaised.Message)];

                        if (alertReason == null || severity == null || message == null)
                        {
                            return false;
                        }

                        return alertReason.ToString() == nameof(AlertReason.Replication) &&

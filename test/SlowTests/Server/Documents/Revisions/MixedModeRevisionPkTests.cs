        ...
        public async Task CompactionPreservesMixedPkForms()
        {
            ...
            }))
            {
                ...
                using (readCtx.OpenReadTransaction())
                {
                    ...
                    using (DocumentIdWorker.Compatibility.GetLowerIdSliceAndStorageKey(readCtx, "users/1", out Slice lowerIdSlice, out _))
                    ...
                    using (RevisionsStorage.BuildRevisionKey(readCtx.Allocator, parentRevCv, out var parentRevKey))
                    {
                        ...
                        unsafe
                        {
                            using (DocumentIdWorker.Compatibility.GetLowerIdSliceAndStorageKey(readCtx, "att-b", out Slice lowerNameSlice, out _))
                            using (DocumentIdWorker.Compatibility.GetLowerIdSliceAndStorageKey(readCtx, "text/plain", out Slice lowerContentTypeSlice, out _))
        ...
        public async Task AttachmentTombstone_DualFormReadByKey()
        {
            ...
            }))
            {
                ...
                using (readCtx.OpenReadTransaction())
                {
                    var parentRevCv = readCtx.GetChangeVector(parentRevCvString);
                    using (DocumentIdWorker.Compatibility.GetLowerIdSliceAndStorageKey(readCtx, "users/1", out Slice lowerIdSlice, out _))
                    using (DocumentIdWorker.Compatibility.GetLowerIdSliceAndStorageKey(readCtx, "x", out Slice lowerNameSlice, out _))
                    using (DocumentIdWorker.Compatibility.GetLowerIdSliceAndStorageKey(readCtx, "text/plain", out Slice lowerCtSlice, out _))

...
{
    public partial class RetireAttachmentsSlowTests : ReplicationTestBase
    {
         ...
         *
         */
        public RetireAttachmentsSlowTests(ITestOutputHelper output) : base(output)
        {
        }
        ...
        {
            await using (var holder = new RetiredAttachmentsHolder(this))
            {
                int docsCount = RetiredAttachmentsHolder.GetDocsAndAttachmentCount(attachmentsCount, out int attachmentsPerDoc);
                var ids = new List<(string Id, string Collection)>();

                using (var store = GetDocumentStore())
                {
                    await CanUploadRetiredAttachmentToS3AndGetInternal(attachmentsCount, size, store, holder, docsCount, ids, attachmentsPerDoc, collections);
                }
            }
        }
        ...
        private async Task CanUploadRetiredAttachmentToS3AndDeleteInternal(int attachmentsCount, int size, List<string> collections = null)
        {
            await using (var holder = new RetiredAttachmentsHolder(this))
            {
                ...
                {
                    await CanUploadRetiredAttachmentToS3AndGetInternal(attachmentsCount, size, store, holder, docsCount, ids, attachmentsPerDoc, collections);
                    foreach (var attachment in holder.Attachments)
                    {
                    ...
                    await RetiredAttachmentsHolder.GetBlobsFromS3AndAssertForCount(holder.Settings, 0);
                }
            }
        }

        private async Task CanUploadRetiredAttachmentToS3AndGetInternal(int attachmentsCount, int size, DocumentStore store, RetiredAttachmentsHolder holder, int docsCount,
            List<(string Id, string Collection)> ids, int attachmentsPerDoc, List<string> collections = null, RavenServer server = null)
        {
            await RetiredAttachmentsHolder.PutRetireAttachmentsConfiguration(store, holder.Settings, collections);
            await RetiredAttachmentsHolder.CreateDocs(store, docsCount, ids, collections);
            await holder.PopulateDocsWithRandomAttachments(store, size, ids, attachmentsPerDoc);

            var database = await Databases.GetDocumentDatabaseInstanceFor(server ?? Server, store);
            holder.GetStorageAttachmentsMetadataFromAllAttachments(database);
            Assert.Equal(attachmentsCount, holder.Attachments.Count);
            ...
            var cloudObjects = await RetiredAttachmentsHolder.GetBlobsFromS3AndAssertForCount(holder.Settings, attachmentsCount, 15_000);

            await holder.AssertAllRetiredAttachments(store, cloudObjects, size);
        }
        ...
        public async Task CanUploadRetiredAttachmentToS3AndDeleteInTheSameTime(int attachmentsCount, int size, int attachmentsPerDoc)
        {
            await using (var holder = new RetiredAttachmentsHolder(this))
            {
                ...
                using (var store = GetDocumentStore())
                {
                    ...
                    await holder.PopulateDocsWithRandomAttachments(store, size, ids, attachmentsPerDoc / 2, start: 0);
                    ...
                    await database.RetireAttachmentsSender.RetireAttachments(int.MaxValue, int.MaxValue);
                    await RetiredAttachmentsHolder.GetBlobsFromS3AndAssertForCount(holder.Settings, attachmentsCount / 2);

                    var list = holder.Attachments.ToList();
                    var t1 = Task.Run(async () =>
                    {
                        await holder.PopulateDocsWithRandomAttachments(store, size, ids, attachmentsPerDoc / 2, start: 1000);
                    });
                    ...
                    holder.GetStorageAttachmentsMetadataFromAllAttachments(database);
                    var list2 = holder.Attachments.ToList();
                    ...
                    {
                        await database.RetireAttachmentsSender.RetireAttachments(int.MaxValue, int.MaxValue);
                        var objs = await RetiredAttachmentsHolder.GetBlobsFromS3AndAssertForCount(holder.Settings, 0, 1000);
                        return objs.Count;
                    }, 0);
                }
            }
        ...
        public async Task ShouldAddRetireAtToAttachmentMetadata()
        {
            await using (var holder = new RetiredAttachmentsHolder(this))
            {
                using (var store = GetDocumentStore())
                {
                    await RetiredAttachmentsHolder.PutRetireAttachmentsConfiguration(store, holder.Settings);
        ...
        public async Task ShouldThrowUsingRegularAttachmentsApiOnRetiredAttachment()
        {
            await using (var holder = new RetiredAttachmentsHolder(this))
            {
                using (var store = GetDocumentStore())
                {
                    ...
                    var cloudObjects = await RetiredAttachmentsHolder.GetBlobsFromS3AndAssertForCount(holder.Settings, 1);
                   // Assert.Contains($"{holder.Settings.RemoteFolderName}/{store.Database}/Orders/b3JkZXJzLzMeZB50ZXN0LnBuZx5FY0RubTNIRGwyek5EQUxSTVE0bEZzQ08zSjJMYjFmTTFvRFdPazJPY3RvPR5pbWFnZS9wbmc=", cloudObjects[0].FullPath);
                    Assert.Contains($"{holder.Settings.RemoteFolderName}/Orders/b3JkZXJzLzMeZB50ZXN0LnBuZx5FY0RubTNIRGwyek5EQUxSTVE0bEZzQ08zSjJMYjFmTTFvRFdPazJPY3RvPR5pbWFnZS9wbmc=", cloudObjects[0].FullPath);
        ...
        private async Task CanUploadRetiredAttachmentsToS3AndGetInBulkInternal(int attachmentsCount, int size, int attachmentsPerDoc, List<string> collections = null)
        {
            await using (var holder = new RetiredAttachmentsHolder(this))
            {
                ...
                {
                    await CanUploadRetiredAttachmentToS3AndGetInternal(attachmentsCount, size, store, holder, docsCount, ids, attachmentsPerDoc, collections);
                    await AssertRetiredAttachmentsInBulk(holder, store);
                }
            }
        }

        private static async Task AssertRetiredAttachmentsInBulk(RetiredAttachmentsHolder holder, DocumentStore store)
        {
            ...
            while (attachmentsEnumerator.MoveNext())
            {
                ...
                var tuple = holder.Attachments.FirstOrDefault(x => x.DocumentId == current.Details.DocumentId && x.Name == current.Details.Name);
        ...
        public async Task CanUploadRetiredAttachmentToS3IfItAlreadyExists()
        {
            await using (var holder = new RetiredAttachmentsHolder(this))
            {
                ...
                {
                    await RetiredAttachmentsHolder.PutRetireAttachmentsConfiguration(store, holder.Settings);
                    await RetiredAttachmentsHolder.PutRetireAttachmentsConfiguration(store2, holder.Settings);
                    ...
                    var expiredDocumentsCleaner = database.RetireAttachmentsSender;
                    await expiredDocumentsCleaner.RetireAttachments(int.MaxValue, int.MaxValue);
                    List<S3FileInfoDetails> cloudObjects = await RetiredAttachmentsHolder.GetBlobsFromS3AndAssertForCount(holder.Settings, 1);
                   // Assert.Contains($"{holder.Settings.RemoteFolderName}/{store.Database}/Orders/b3JkZXJzLzMeZB50ZXN0LnBuZx5FY0RubTNIRGwyek5EQUxSTVE0bEZzQ08zSjJMYjFmTTFvRFdPazJPY3RvPR5pbWFnZS9wbmc=", cloudObjects[0].FullPath);
                    Assert.Contains($"{holder.Settings.RemoteFolderName}/Orders/b3JkZXJzLzMeZB50ZXN0LnBuZx5FY0RubTNIRGwyek5EQUxSTVE0bEZzQ08zSjJMYjFmTTFvRFdPazJPY3RvPR5pbWFnZS9wbmc=", cloudObjects[0].FullPath);

                    await RetiredAttachmentsHolder.GetAndCompareRetiredAttachment(store, id, "test.png", "EcDnm3HDl2zNDALRMQ4lFsCO3J2Lb1fM1oDWOk2Octo=", "image/png", profileStream, 3);
                    ...
                    List<S3FileInfoDetails> cloudObjects2 = await RetiredAttachmentsHolder.GetBlobsFromS3AndAssertForCount(holder.Settings, 1);
                   // Assert.Contains($"{holder.Settings.RemoteFolderName}/{store2.Database}/Orders/b3JkZXJzLzMeZB50ZXN0LnBuZx5FY0RubTNIRGwyek5EQUxSTVE0bEZzQ08zSjJMYjFmTTFvRFdPazJPY3RvPR5pbWFnZS9wbmc=", cloudObjects2[0].FullPath);
                    Assert.Contains($"{holder.Settings.RemoteFolderName}/Orders/b3JkZXJzLzMeZB50ZXN0LnBuZx5FY0RubTNIRGwyek5EQUxSTVE0bEZzQ08zSjJMYjFmTTFvRFdPazJPY3RvPR5pbWFnZS9wbmc=", cloudObjects2[0].FullPath);

                    Assert.NotEqual(cloudObjects[0].LastModified, cloudObjects2[0].LastModified);

                    await RetiredAttachmentsHolder.GetAndCompareRetiredAttachment(store2, id, "test.png", "EcDnm3HDl2zNDALRMQ4lFsCO3J2Lb1fM1oDWOk2Octo=", "image/png", profileStream, 3);
                }
            ...
            await using (var holder = new RetiredAttachmentsHolder(this))
            {
                using (var store = GetDocumentStore())
                {
                  ...
                  //  Assert.Contains($"{holder.Settings.RemoteFolderName}/{store.Database}/Orders/b3JkZXJzLzMeZB50ZXN0LnBuZx5FY0RubTNIRGwyek5EQUxSTVE0bEZzQ08zSjJMYjFmTTFvRFdPazJPY3RvPR5pbWFnZS9wbmc=", cloudObjects[0].FullPath);
                    ...
                    await holder.DeleteObjects(holder.Settings);

                    var e = await Assert.ThrowsAsync<RavenException>(async () => await RetiredAttachmentsHolder.GetAndCompareRetiredAttachment(store, id, "test.png", "EcDnm3HDl2zNDALRMQ4lFsCO3J2Lb1fM1oDWOk2Octo=", "image/png", profileStream, 3));
        ...
        public async Task CanUploadRetiredAttachmentToS3InClusterAndGet(int attachmentsCount, int size)
        {
            ...
            using (var src = new DocumentStore { Urls = srcNodes.Servers.Select(s => s.WebUrl).ToArray(), Database = srcDb, }.Initialize())
            {
                ...
                await using (var holder = new RetiredAttachmentsHolder(this))
                {
                    ...
                    await RetiredAttachmentsHolder.PutRetireAttachmentsConfiguration(store, holder.Settings);
                    await RetiredAttachmentsHolder.CreateDocs(store, docsCount, ids);
                    await holder.PopulateDocsWithRandomAttachments(store, size, ids, attachmentsPerDoc);
...
//var z = database.ReplicationLoader.OutgoingHandlers.FirstOrDefault(x=>x.Destination.NodeTag);
            ...
            await using (var holder = new RetiredAttachmentsHolder(this))
            {
                ...
                using (var store = GetDocumentStore())
                {
                           ...
                           }))
                    {
                        ...
                        holder.GetStorageAttachmentsMetadataFromAllAttachments(database2);
                        Assert.Equal(attachmentsCount, holder.Attachments.Count);
                        // move in time & start retire
                        database2.Time.UtcDateTime = () => DateTime.UtcNow.AddMinutes(10);
                        ...
                        {
                            await holder.AssertAllRetiredAttachments(restored, cloudObjects, size);
                        }
                    }
                }
            }
        ...
        public async Task CanExternalReplicateRetiredAttachmentAndThenUploadToS3AndGet(int attachmentsCount, int size)
        {
            ...
            using (var store2 = GetDocumentStore())
            {
                ...
                await using (var holder = new RetiredAttachmentsHolder(this))
                {
                    ...
                    var ids = new List<(string Id, string Collection)>();
                    List<string> collections = null;
                    await RetiredAttachmentsHolder.PutRetireAttachmentsConfiguration(store1, holder.Settings);
                    await RetiredAttachmentsHolder.CreateDocs(store1, docsCount, ids);
                    await holder.PopulateDocsWithRandomAttachments(store1, size, ids, attachmentsPerDoc);
                    ...
                    await database2.RetireAttachmentsSender.RetireAttachments(int.MaxValue, int.MaxValue);
                    var cloudObjects = await RetiredAttachmentsHolder.GetBlobsFromS3AndAssertForCount(holder.Settings, attachmentsCount, 15_000);
                    await holder.AssertAllRetiredAttachments(store2, cloudObjects, size);
        ...
        public async Task CanBackupRetiredAttachments(int attachmentsCount, int size)
        {
            await using (var holder = new RetiredAttachmentsHolder(this))
            {
                ...
                using (var store = GetDocumentStore())
                {
                    await CanUploadRetiredAttachmentToS3AndGetInternal(attachmentsCount, size, store, holder, docsCount, ids, attachmentsPerDoc);
        ...
        public async Task CanExportImportWithRetiredAttachment(int attachmentsCount, int size)
        {
            await using (var holder = new RetiredAttachmentsHolder(this))
            {
                ...
                using (var store2 = GetDocumentStore())
                {
                    await CanUploadRetiredAttachmentToS3AndGetInternal(attachmentsCount, size, store1, holder, docsCount, ids, attachmentsPerDoc);
        ...
        public async Task CanEtlWithRetiredAttachmentAndRetireOnDestination(int attachmentsCount, int size)
        {
            await using (var holder = new RetiredAttachmentsHolder(this))
            {
                ...
                using (var replica = GetDocumentStore())
                {
                    ...
                    Assert.True(holder.Attachments.TrueForAll(x => x.RetireAt != null), "holder.Attachments.TrueForAll(x => x.RetireAt != null)");


                    await RetiredAttachmentsHolder.PutRetireAttachmentsConfiguration(replica, holder.Settings);
                    ...
                    await database2.RetireAttachmentsSender.RetireAttachments(int.MaxValue, int.MaxValue);
                    var cloudObjects = await RetiredAttachmentsHolder.GetBlobsFromS3AndAssertForCount(holder.Settings, attachmentsCount, 15_000);
                    await holder.AssertAllRetiredAttachments(replica, cloudObjects, size);
        ...
        public async Task CanEtlRetiredAttachmentsToDestination(int attachmentsCount, int size)
        {
            await using (var holder = new RetiredAttachmentsHolder(this))
            {
                ...
                using (var replica = GetDocumentStore())
                {
                    ...
                    await AssertRetiredAttachmentsInBulk(holder, replica);
                }
            }
        }

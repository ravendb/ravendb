...
{
    public class S3RemoteAttachmentsBackupRestoreTests : RemoteAttachmentsS3Base
    {
        public S3RemoteAttachmentsBackupRestoreTests(ITestOutputHelper output) : base(output)
        {
        }
                    ...
                    using (var _documentInfoHelper = new DocumentInfoHelper(context))
                    {
                        ...
                        foreach (var attachment in attachments)
                        {
                            ...
                            using (var docId = _documentInfoHelper.GetDocumentId(attachment.Key))
                            {
                                ...
                                newAttachment.RemoteParameters = attachment.RemoteParameters;
        ...
        public async Task CanExportAndImportRemoteAttachmentsAsync(int attachmentsCount, int size)
        {
            await using (var holder = CreateCloudSettings())
            {
                int docsCount = GetDocsAndAttachmentCount(attachmentsCount, out int attachmentsPerDoc);
                var ids = new List<(string Id, string Collection)>();
                using (var store = GetDocumentStore(new Options { ModifyDatabaseName = s => $"{s}_source" }))
                {
                    var identifier = await CanUploadRemoteAttachmentToCloudAndGetInternal(attachmentsCount, size, store, docsCount, ids, attachmentsPerDoc);
                    ...
                    // Import
                    using (var importedStore = GetDocumentStore(new Options { ModifyDatabaseName = s => $"{s}_dest" }))
                    {
                        ...
                        using (context.OpenReadTransaction())
                        {
                            ...
                            await Assert.AllAsync(attachments, async attachment =>
                            {
                                ...
                                Assert.NotNull(a);
                                attachment.Stream = a.Stream;
                                await GetAndCompareRemoteAttachment(importedStore, a.DocumentId, attachment.Name, attachment.Base64Hash.ToString(),
                                    attachment.ContentType, (MemoryStream)attachment.Stream, size, identifier);
                            });
                        }
                    }
                }

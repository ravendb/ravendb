...
{
    public class RetiredAttachmentSchemaUpdateTests : RetiredAttachmentsS3Base
    {
        public RetiredAttachmentSchemaUpdateTests(ITestOutputHelper output) : base(output)
        {
        }
        ...
        public async Task Can_Open_v71_Data()
        {
            ...
            {
                await using (var stream = typeof(RetiredAttachmentSchemaUpdateTests).Assembly.GetManifestResourceStream($"SlowTests.Data.Attachments.RavenDB_24543.{snapshot}"))
                {
            ...
            using (var store = GetDocumentStore(new Options { CreateDatabase = false, RunInMemory = false, }))
            {
                    ...
                    foreach (var id in ids)
                    {
                        ...
                        foreach (var attachmentInfo in attachmentsArray)
                        {
                            ...
                            using (var session2 = store2.OpenAsyncSession())
                            {
                                ...
                                Assert.Null(attachmentResult.Details.RetireParameters);
                                Assert.False(attachment.RetireParameters.IsExplicitNull); // RetireParameters should not exist for existing attachments
                                ...
                                Assert.Equal(attachmentResult.Details.RetireParameters, attachmentResult2.Details.RetireParameters);
                ...
                Assert.Equal(0, finalStats.CountOfRetiredAttachments);
        ...
        [RavenFact(RavenTestCategory.Attachments)]
        public async Task Schema_Update_From62000_Should_Add_RetiredAttachment_Fields()
        {
            // Test that the schema update properly adds the new fields for retired attachments
            var dest = "Northwind";
            var snapshot = $"{dest}.zip";
            var backupPath = NewDataPath(forceCreateDir: true);
            var fullBackupPath = Path.Combine(backupPath, snapshot);
            ...
            using (var store = GetDocumentStore(new Options { CreateDatabase = false, RunInMemory = false, }))
            {
                ...
                using (context.OpenReadTransaction())
                {
                    ...
                    Assert.Equal(nameof(RetiredAttachmentsStorage.GenerateFlagAndHashForAttachments), dynamicIndex.GenerateKey.Method.Name);
                    var tree = attachmentTable.GetTree(dynamicIndex);
                    Assert.Equal(16, tree.State.Header.NumberOfEntries);

                    // Test 2: Verify all existing attachments have RetiredAttachmentFlags.None
                    foreach (var result in attachmentTable.SeekForwardFrom(Raven.Server.Documents.Schemas.Attachments.AttachmentsSchemaBase.FixedSizeIndexes[Raven.Server.Documents.Schemas.Attachments.AttachmentsEtagSlice], 0, 0))
                    {
                        ...
                        // Verify all migrated attachments have RetiredAttachmentFlags.None
                        Assert.Equal(RetiredAttachmentFlags.None, attachment.Flags);
                        ...
                        Assert.Null(attachment.RetireAt);
                    }
                }
            }
        }
        ...
        [AmazonS3RetryFact]
        public async Task Schema_Update_From62000_Should_Allow_New_Retired_Attachment_Operations()
        {
            // Test that retired attachment operations work after the schema update
            ...
            using (var store = GetDocumentStore(new Options { CreateDatabase = false, RunInMemory = false, }))
            {
                ...
                // Test that retired attachment functionality works after migration
                ...
                // Add an attachment that can be retired
                var content = "This is test attachment content for retirement";
                ...
                using (var stream = new MemoryStream(contentBytes))
                {
                    ...
                    //verify that we can retire the attachment
                    await using (var holder = CreateCloudSettings())
                    {
                        var retireAt = DateTime.UtcNow.AddMinutes(3);
                        var identifier = await PutRetireAttachmentsConfiguration(store, Settings, collections: null);

                        stream.Position = 0;
                        store.Operations.Send(new PutAttachmentOperation(testDocId, "test-attachment.txt", stream, "text/plain",
                            retireParameters: new RetireAttachmentParameters(identifier, retireAt)));
                        ...
                        using (var session = store.OpenAsyncSession())
                        {
                            ...
                            Assert.NotNull(attachmentResult.Details.RetireParameters);
                            Assert.Equal(identifier, attachmentResult.Details.RetireParameters.Identifier);
                            Assert.Equal(retireAt, attachmentResult.Details.RetireParameters.At);
                            Assert.Equal(RetiredAttachmentFlags.Retired, attachmentResult.Details.RetireParameters.Flags);
                        }
                    }
                }
            }
        ...
        internal static unsafe AttachmentTableValueReader TableValueToAttachmentTableValueReader(DocumentsOperationContext context, ref TableValueReader tvr)
        {
            var result = new AttachmentTableValueReader
            {
                ...
                RetireAt = DocumentsStorage.TableValueToNullableDateTime((int)Raven.Server.Documents.Schemas.Attachments.AttachmentsTable.RetireAt, ref tvr),

...
{
    public class AzureRetireAttachmentsSlowTests : AzureRetiredAttachmentsHolder
    {
        ...
        public AzureRetireAttachmentsSlowTests(ITestOutputHelper output) : base(output)
        {
            _output = output;
        }
        ...
        {
            await CanUploadRetiredAttachmentToAzureAndGetInternal(attachmentsCount, size);
        }
        ...
        public async Task CanUploadRetiredAttachmentFromDifferentCollectionsToAzureAndGet(int attachmentsCount, int size)
        {
            ...
            await CanUploadRetiredAttachmentToAzureAndGetInternal(attachmentsCount, size, collections);
        }

        internal async Task CanUploadRetiredAttachmentToAzureAndGetInternal(int attachmentsCount, int size, List<string> collections = null)
        {
            await using (var holder = CreateCloudSettings())
            {
                ...
                {
                    await CanUploadRetiredAttachmentToAzureAndGetInternal(attachmentsCount, size, store, docsCount, ids, attachmentsPerDoc, collections);
                }
            }
        }
        ...
        public async Task CanUploadRetiredAttachmentFromDifferentCollectionsToAzureAndDelete(int attachmentsCount, int size)
        {
            Assert.True(attachmentsCount > 32, "this test meant to have more than 32 attachments so we will have more than one document");
            var collections = new List<string> { "Orders", "Products" };
            await CanUploadRetiredAttachmentToAzureAndDeleteInternal(attachmentsCount, size, collections);
        }
        ...
        {
            await CanUploadRetiredAttachmentToAzureAndDeleteInternal(attachmentsCount, size);
        }

        private async Task CanUploadRetiredAttachmentToAzureAndDeleteInternal(int attachmentsCount, int size, List<string> collections = null)
        {
            await using (var holder = CreateCloudSettings())
            {
                int docsCount = GetDocsAndAttachmentCount(attachmentsCount, out int attachmentsPerDoc);
        ...
        private async Task CanUploadRetiredAttachmentToAzureAndGetInternal(int attachmentsCount, int size, DocumentStore store, int docsCount,
        ...
        {
            var collections = new List<string> { "Orders", "Products" };
            await CanUploadRetiredAttachmentsToAzureAndGetInBulkInternal(attachmentsCount, size, attachmentsPerDoc, collections);
        }
        ...
        {
            await CanUploadRetiredAttachmentsToAzureAndGetInBulkInternal(attachmentsCount, size, attachmentsPerDoc);
        }

        private async Task CanUploadRetiredAttachmentsToAzureAndGetInBulkInternal(int attachmentsCount, int size, int attachmentsPerDoc, List<string> collections = null)
        {
            await using (var holder = CreateCloudSettings())
            {
                int docsCount = attachmentsCount / attachmentsPerDoc;
        ...
        public async Task UploadRetiredAttachmentToAzureThenManuallyDeleteAndGetShouldThrow()
        {
            await using (var holder = CreateCloudSettings())
            {
                using (var store = GetDocumentStore())
                {
                    ...
                    var e = await Assert.ThrowsAsync<RavenException>(async () => await GetAndCompareRetiredAttachment(store, id, "test.png", "EcDnm3HDl2zNDALRMQ4lFsCO3J2Lb1fM1oDWOk2Octo=", "image/png", profileStream, 3));
        ...
        //[InlineData(64, 3)]
        ////TODO: egor this doesn't work yet since there is no replication for retired attachment :(
        //public async Task CanUploadRetiredAttachmentToAzureInClusterAndGet3(int attachmentsCount, int size)
        //{
        //    var srcDb = GetDatabaseName();
        //    var srcRaft = await CreateRaftCluster(3);
        //    var leader = srcRaft.Leader;
        ...
        public async Task CanBackupRetiredAttachments(int attachmentsCount, int size)
        {
            await using (var holder = CreateCloudSettings())
            {
                ...
                using (var store = GetDocumentStore())
                {
                    await CanUploadRetiredAttachmentToAzureAndGetInternal(attachmentsCount, size, store, docsCount, ids, attachmentsPerDoc);
        ...
        public async Task CanExportImportWithRetiredAttachment(int attachmentsCount, int size)
        {
            await using (var holder = CreateCloudSettings())
            {
                ...
                using (var store2 = GetDocumentStore())
                {
                    await CanUploadRetiredAttachmentToAzureAndGetInternal(attachmentsCount, size, store1, docsCount, ids, attachmentsPerDoc);

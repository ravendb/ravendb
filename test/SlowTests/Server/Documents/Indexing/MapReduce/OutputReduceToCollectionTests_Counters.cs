...
{
    public class OutputReduceToCollectionTests_Counters : RavenTestBase
    {
        public OutputReduceToCollectionTests_Counters(ITestOutputHelper output)
            : base(output)
        {
        }

        [RavenFact(RavenTestCategory.Indexes | RavenTestCategory.Counters)]
        ...
        private async Task CreateDataAndIndexes(IDocumentStore store)
        {
            ...
            using (var session = store.OpenAsyncSession())
            {
                for (int i = 0; i < 30; i++)
                {
                    ...
                    session.CountersFor(invoice).Increment("Views", 6);
                }
        ...
        private class DailyInvoicesIndex : AbstractCountersIndexCreationTask<Invoice, DailyInvoice>
        {
        ...
        public class DailyInvoicesIndexLoadDocument : AbstractCountersIndexCreationTask<InvoiceHolder, DailyInvoice>
        {
        ...
        private class MonthlySelfReduceIndex : AbstractCountersIndexCreationTask<DailyInvoice, DailyInvoice>
        {
        ...
        private class InvoiceSelfReduceLoadDocumentIndex : AbstractCountersIndexCreationTask<InvoiceHolder, Invoice>
        {
        ...
        private class YearlyToDailyInfiniteLoopIndex : AbstractCountersIndexCreationTask<MonthlyInvoice, Invoice>
        {

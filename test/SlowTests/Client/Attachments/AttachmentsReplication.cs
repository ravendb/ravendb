...
namespace SlowTests.Client.Attachments
{
    public class AttachmentsReplication : ReplicationTestBase
    {
        ...
        [Fact]
        public async Task AttachmentsShouldBeProperlyResolvedToLatestAfterConflicts()
        {
            using (var store1 = GetDocumentStore())
            using (var store2 = GetDocumentStore())
            {
        ...
        [Fact]
        public async Task AttachmentWithDifferentStreamAndSameNameShouldBeResolvedAfterConflict_ResolveToLatestFalse_RenameAttachment()
        {
            using (var store1 = GetDocumentStore(options: new Options
            {
                ModifyDatabaseRecord = record =>
            ...
            }))
            {
                ...
                {
                    var user = await session.LoadAsync<User>("users/1");
                    Assert.Equal("EGR_RESOLVED", user.Name);
                    var attachments = session.Advanced.Attachments.GetNames(user);
                    Assert.Equal(2, attachments.Length);
                    ...
                    Assert.Equal("newName", attachments[1].Name);
        ...
        [Fact]
        public async Task MultipleAttachmentWithDifferentStreamAndSameNameShouldBeResolvedAfterConflict_ResolveToLatestFalse_RenameAttachment()
        {
            using (var store1 = GetDocumentStore(options: new Options
            {
                ModifyDatabaseRecord = record =>
            ...
            }))
            {
                ...
                using (var session = store1.OpenAsyncSession())
                {
                    ...
                    Assert.Equal("newName", attachments[2].Name);
        ...
        [Fact]
        public async Task CanRemoveAttachmentUsingScriptResolverAfterConflict()
        {
            using (var store1 = GetDocumentStore(options: new Options
            {
                ModifyDatabaseRecord = record =>
        ...
        [Fact]
        public async Task AddAttachmentsUsingScriptResolverShouldRaiseConflict()
        {
            using (var store1 = GetDocumentStore(options: new Options
            {
                ModifyDatabaseRecord = record =>
            ...
            }))
            {
                ...
                await SetupReplicationAsync(store1, store2);
                await SetupReplicationAsync(store2, store1);
                Assert.Equal(2, WaitForValue(() => store1.Commands().GetConflictsFor("users/1").Length, 2));
                Assert.Equal(2, WaitForValue(() => store2.Commands().GetConflictsFor("users/1").Length, 2));
            }
        }
    }
}

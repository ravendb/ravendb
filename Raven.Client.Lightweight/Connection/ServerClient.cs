...
namespace Raven.Client.Connection
{
	...
	public class ServerClient : IDatabaseCommands//, IAdminDatabaseCommands
	{
		...
		internal void ExecuteWithReplication(string method, Action<OperationMetadata> operation)
		{
		...
		{
			ExecuteWithReplication("PUT", operationMetadata => DirectPutAttachment(key, metadata, etag, data, operationMetadata));
		}
		...
		{
			ExecuteWithReplication("POST", operationMetadata => DirectUpdateAttachmentMetadata(key, metadata, etag, operationMetadata));
		}
		...
		{
			return ExecuteWithReplication("GET", operationMetadata => DirectGetAttachment("GET", key, operationMetadata));
		}
		...
		{
			return ExecuteWithReplication("HEAD", operationMetadata => DirectGetAttachment("HEAD", key, operationMetadata));
		}
		...
		private Attachment DirectGetAttachment(string method, string key, OperationMetadata operationMetadata)
		{
			...
			try
			{
				...
				HandleReplicationStatusChanges(webRequest.ResponseHeaders[Constants.RavenForcePrimaryServerCheck], Url, operationMetadata.Url);
		...
		{
			ExecuteWithReplication("DELETE", operationMetadata => DirectDeleteAttachment(key, etag, operationMetadata));
		}
		...
		{
			EnsureIsNotNullOrEmpty(name, "name");
			ExecuteWithReplication("DELETE", operationMetadata => DirectDeleteTransformer(name, operationMetadata));
		}
		...
		public string PutTransformer(string name, TransformerDefinition indexDef)
		{
			...
			return ExecuteWithReplication("PUT", operationMetadata => DirectPutTransformer(name, operationMetadata, indexDef));
		...
		public string PutIndex(string name, IndexDefinition definition, bool overwrite)
		{
			...
			return ExecuteWithReplication("PUT", operationMetadata => DirectPutIndex(name, operationMetadata, overwrite, definition));
		}
		...
		{
			EnsureIsNotNullOrEmpty(name, "name");
			ExecuteWithReplication("DELETE", operationMetadata => DirectDeleteIndex(name, operationMetadata));
		}
		...
		public long NextIdentityFor(string name)
		{
			...
			{
				var request = jsonRequestFactory.CreateHttpJsonRequest(
					new CreateHttpJsonRequestParams(this, url + "/identity/next?name=" + Uri.EscapeDataString(name), "POST", operationMetadata.Credentials, convention)
						.AddOperationHeaders(OperationsHeaders));
		...
		public long SeedIdentityFor(string name, long value)
		{
			...
			{
				var request = jsonRequestFactory.CreateHttpJsonRequest(
					new CreateHttpJsonRequestParams(this, url + "/identity/seed?name=" + Uri.EscapeDataString(name) + "&value=" + Uri.EscapeDataString(value.ToString(CultureInfo.InvariantCulture)), "POST", operationMetadata.Credentials, convention)
						.AddOperationHeaders(OperationsHeaders));

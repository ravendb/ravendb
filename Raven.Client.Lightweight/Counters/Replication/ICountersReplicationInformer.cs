...
{
	public interface ICountersReplicationInformer : IReplicationInformerBase<CounterStore>
	{
		Task UpdateReplicationInformationIfNeededAsync();

		//TODO: finish refactoring and remove this overload
		Task ExecuteWithReplicationAsync<T>(HttpMethod method, CounterStore client, Func<OperationMetadata, Task<T>> operation);
		...
		//TODO: finish refactoring and remove this overload
		Task<T> ExecuteWithReplicationAsyncWithReturnValue<T>(HttpMethod method, CounterStore client, Func<OperationMetadata, Task<T>> operation);

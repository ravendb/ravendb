...
namespace Raven.Client.Counters
{
    public class RavenCountersClient : IDisposable, IHoldProfilingInformation
    {
        ...
        {
            get { return string.Format("{0}/fs/{1}", ServerUrl, CounterStorageName); }
        }
        ...
        public Task ResetAsync(string counterName)
        {
            ...
            {
				var requestUriString = operation.Url + "/counters/{counterName}/reset/" + Uri.EscapeDataString(counterName);
        ...
        public ReplicationClient Replication
        {
            get
            {
                return new ReplicationClient(this, Convention);
            }
        }
        ...
        public class ConfigurationClient : IHoldProfilingInformation
        {
			...
			public ConfigurationClient(RavenCountersClient countersClient, CounterConvention convention)
            {
			...
			public Task<T> GetConfig<T>(string name)
			{
				return ravenFileSystemClient.ExecuteWithReplication("GET", async operation =>
				{
					...
					var request = ravenFileSystemClient.jsonRequestFactory.CreateHttpJsonRequest(new CreateHttpJsonRequestParams(this, requestUriString, "GET", operation.Credentials, convention));
			...
			public Task<ConfigSearchResults> SearchAsync(string prefix, int start = 0, int pageSize = 25)
			{
				return ravenFileSystemClient.ExecuteWithReplication("GET", async operation =>
				{
					...
					var request =
					ravenFileSystemClient.jsonRequestFactory.CreateHttpJsonRequest(new CreateHttpJsonRequestParams(this, requestUriBuilder.ToString(),
						"GET", operation.Credentials, convention));
        ...
        public class ReplicationClient : IHoldProfilingInformation
        {
			...
			public ReplicationClient(RavenCountersClient countersClient, CounterConvention convention)
            {
                ...
                this.convention = convention;
				FileSystemUrl = countersClient.FileSystemUrl;
				this.countersClient = countersClient;
            }
            ...
            /*public Task<SynchronizationReport> StartAsync(string fileName, RavenCountersClient destination)
            {
        ...
        public class StorageClient : IHoldProfilingInformation
        {
			...
			public StorageClient(RavenCountersClient countersClient, CounterConvention convention)
            {
				this.countersClient = countersClient;
				this.convention = convention;
            }
            ...
            public Task CleanUp()
            {
				return countersClient.ExecuteWithReplication("POST", async operation =>
                {
                    var requestUriString = String.Format("{0}/storage/cleanup", operation.Url);

					var request = countersClient.jsonRequestFactory.CreateHttpJsonRequest(
                                            new CreateHttpJsonRequestParams(this, requestUriString,
                                                                            "POST", operation.Credentials, convention));
	    ...
	    public class CounterClient// : ICounterClient, IHoldProfilingInformation
	    {
			private readonly RavenCountersClient countersClient;
			private readonly CounterConvention convention;
			...
			public async void Change(string group, string counterName, long delta)
		    {
				var requestUriString = string.Format("{0}/counters/{1}/change?group={2}&counterName={3}&delta={4}",
					countersClient.ServerUrl, countersClient.CounterStorageName, group, counterName, delta);

				var request =
					countersClient.jsonRequestFactory.CreateHttpJsonRequest(new CreateHttpJsonRequestParams(this, requestUriString,
						"GET", countersClient.PrimaryCredentials, convention));
			...
			public async void Reset(string group, string counterName)
		    {
				var requestUriString = string.Format("{0}/counters/{1}/change?group={2}&counterName={3}",
					countersClient.ServerUrl, countersClient.CounterStorageName, group, counterName);

				var request =
					countersClient.jsonRequestFactory.CreateHttpJsonRequest(new CreateHttpJsonRequestParams(this, requestUriString,
						"POST", countersClient.PrimaryCredentials, convention));
			...
			public long GetFinalValue(string group, string counterName)
		    {
			    return 0;
		    }

		    public CounterData GetServersValues(string name)
		    {

		    }

		    public ICounterBatch CreateBatch()
		    {
		...
		public class AdminClient : IHoldProfilingInformation
        {
            ...
            {
				var requestUriString = string.Format("{0}/counterStorage/conterStorages", countersClient.ServerUrl);
			...
			public async Task<List<FileSystemStats>> GetCounterStoragesStats() //TODO: change to CounterStoragesStats after it's implemented
            {
				var requestUriString = string.Format("{0}/counterStorage/stats", countersClient.ServerUrl);
			...
			{
				var requestUriString = string.Format("{0}/counterstorage/admin/{1}", countersClient.ServerUrl,
													 newCounterStorageName ?? countersClient.CounterStorageName);

				var request = countersClient.jsonRequestFactory.CreateHttpJsonRequest(
										new CreateHttpJsonRequestParams(this, requestUriString,
																		"PUT", countersClient.PrimaryCredentials, convention));
			...
			{
				var requestUriString = string.Format("{0}/counterstorage/admin/{1}?update=true", countersClient.ServerUrl,
													 newCounterStorageName ?? countersClient.CounterStorageName);
			...
			public async Task DeleteCounterStorageAsync(string counterStorageName = null, bool hardDelete = false)
			{
				var requestUriString = string.Format("{0}/counterstorage/admin/{1}?hard-delete={2}", countersClient.ServerUrl, counterStorageName ?? countersClient.CounterStorageName, hardDelete);

				var request = countersClient.jsonRequestFactory.CreateHttpJsonRequest(
										new CreateHttpJsonRequestParams(this, requestUriString,
																		"DELETE", countersClient.PrimaryCredentials, convention));

            ...
            )
        {
            ...
            else
            {
                ...
                using (var tx = env.WriteTransaction())
                {
                    ...
                    while (tree.ReadHeader().NumberOfEntries < generationTreeSize)
                    {
                        int deletions = 0;
                        ...
                        ref readonly var stateHeader = ref tree.ReadHeader();
                        if (stateHeader.NumberOfEntries > generationTreeSize)
                        {
                            while (stateHeader.NumberOfEntries > generationTreeSize)
                            {
                        ...
                        {
                            while (deletions > 0 && stateHeader.NumberOfEntries > 0)
                            {
                                var keyIndex = generator.Next(treeKeys.Count);
                                tree.Delete(treeKeys[keyIndex]);
                                treeKeys.RemoveAt(keyIndex);

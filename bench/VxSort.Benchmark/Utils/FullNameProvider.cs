...
namespace Bench.Utils
{
    ...
    {
        private static readonly IReadOnlyDictionary<Type, string> Aliases = new Dictionary<
            Type,
            string
        >
        {
        ...
        internal static string GetTypeName(Type type)
        {
            ...
            string args = string.Join(
                ", ",
                type.GetGenericArguments().Select(GetTypeName).ToArray()
            );
        ...
        private static string GetBenchmarkParameters(
            MethodInfo method,
            ParameterInstances benchmarkParameters
        )
        {
            var methodArguments = method.GetParameters();
            var benchmarkParams = benchmarkParameters.Items
            ...
            for (int i = 0; i < methodArguments.Length; i++)
            {
                ...
                parametersBuilder.Append(
                    GetArgument(
                        benchmarkParameters.GetArgument(methodArguments[i].Name).Value,

...
namespace Rachis
{
    public class RaftEngine : IDisposable
    {
        ...
        {
            var leaderWasConfirmed = _leaderConfirmedEvent.Wait(timeout, CancellationToken);
            
            if(leaderWasConfirmed && _leaderSelectedEvent.IsSet == false)
            {                
                if (_log.IsDebugEnabled)
                    _log.Debug("leader confirmed event set, but selected did not");                            
            }
            return leaderWasConfirmed;
        }

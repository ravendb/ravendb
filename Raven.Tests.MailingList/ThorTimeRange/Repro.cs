...
namespace Raven.Tests.MailingList.ThorTimeRange
{
	public class Repro : RavenTest
	{
		...
		public Repro()
		{
			_docStore = NewDocumentStore();
			_docStore.Conventions.DefaultQueryingConsistency = ConsistencyOptions.QueryYourWrites;
			Utility.CreateTestData(_docStore);
		}
		...
		public void CanPersistAndLoad_FAILS()
		{
			using (var session = _docStore.OpenSession())
			{
				...
				WaitForUserToContinueTheTest(_docStore);
